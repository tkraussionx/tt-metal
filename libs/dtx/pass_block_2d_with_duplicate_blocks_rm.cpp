...
vector<vector<int>> dim_order_counting_with_duplicates_(vector<int> shape, vector<int> dim_order, vector<int> block_shape, int num_duplicates, int dim_to_duplicate) {
    bool DEBUG = false;
...
bool block_2d_with_duplicate_blocks_rm(DataTransformations * dtx, vector<int> dim_order, vector<int> block_shape_yx, int num_duplicates, int dim_to_duplicate) {
    ...
    if (DEBUG) cout << "\n\nPASS: Block 2d matrix with duplicate blocks. RM transfers." << endl;
    ...
    for (int group_idx=0; group_idx<producer->groups.size(); group_idx++) {
        ...
        vector<vector<int>> list_of_counted_dims = dim_order_counting_with_duplicates_(shape_blocked,   dim_order, block_shape, num_duplicates, dim_to_duplicate);
        ...
        for (int i=0; i< list_of_counted_dims.size(); i++) {
            ...
            int str_y = str[Y(rank)];
            vector<int> consumer_str = {i, 0, 0};
            vector<int> consumer_end = {i, block_shape[Y(rank)]-1, block_shape[X(rank)]-1};
            for(int block_shape_y_i = 0; block_shape_y_i < block_shape[Y(rank)]; block_shape_y_i++) {

                str[Y(rank)] = str_y + block_shape_y_i;
                end[Y(rank)] = str[Y(rank)];
                consumer_str[Y(rank)] = block_shape_y_i;
                consumer_end[Y(rank)] = consumer_str[Y(rank)];
                assert(consumer_str[Y(rank)] < block_shape[Y(rank)]);
