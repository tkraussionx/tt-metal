...
bool convert_tensor_layout_3d_conv_act_to_2d_blocks(DataTransformations * dtx, vector<int> dim_order, vector<int> block_shape_yx) {
    ...
    if (DEBUG) cout << "\n\nPASS: Convert 3d conv activation to 2d Blocks" << endl;

    // Identify producer TX & Consumer
    TransformationNode * producer = dtx->transformations.back();
    TransformationNode * consumer = new TransformationNode("convert_3d_conv_act_to_2d_blocks", producer->groups.size());  // TODO: generalize for groups>1
    dtx->transformations.push_back(consumer);
    assert(producer->groups.size() == 1); // TODO: add support for groups > 1
