...
Tensor bmm_single_core_tilize_untilize(const Tensor &A, const Tensor &B, const uint32_t num_blocks, const uint32_t out_subblock_h, const uint32_t out_subblock_w); // single core, with tilize and untilize for activations (A)
