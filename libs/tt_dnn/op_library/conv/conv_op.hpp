...
struct Conv {
    ...
    bool use_address_map, use_fast_reader;
    Conv(uint32_t act_bh, uint32_t act_bw, uint32_t weight_bw, uint32_t out_sh, uint32_t out_sw, const std::vector<int>&c_params, uint32_t output_channels, bool address_map, bool fast_reader)
...
operation::ProgramWithCallbacks conv_single_core(const Tensor& A, const Tensor& B, const Tensor& bias, vector<int> conv_params, uint32_t act_block_h_ntiles, uint32_t act_block_w_ntiles, uint32_t weight_block_w_ntiles,
