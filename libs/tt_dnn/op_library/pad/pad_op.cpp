...
operation::ProgramWithCallbacks pad_cl(const Tensor &a, Tensor &output, const std::array<uint32_t, 4> &output_tensor_shape, const std::array<uint32_t, 4> &input_tensor_start, float pad_value) {
    ...
    uint32_t unpadded_channel_size_bytes = a.shape()[1] * a.element_size();
    uint32_t padded_channel_size_bytes = output_shape[1] * a.element_size();
    ...
    uint32_t src_stick_size = unpadded_channel_size_bytes;
    uint32_t dst_stick_size = padded_channel_size_bytes;
    ...
    vector<uint32_t> reader_kernel_args = {
        ...
        a.shape()[1],
        output_shape[1],
        unpadded_channel_size_bytes,
        padded_channel_size_bytes,
        padded_channel_size_bytes - unpadded_channel_size_bytes,
        packed_pad_value,
        cache_buffer_l1.address(),
        src_buffer_l1.address(),
        dst_buffer_l1.address()
...
void Pad::validate(const std::vector<Tensor> &input_tensors) const {
    ...
    TT_ASSERT(input_tensor.layout() == Layout::TILE || input_tensor.layout() == Layout::ROW_MAJOR || input_tensor.layout() == Layout::CHANNELS_LAST);
    TT_ASSERT(
...
operation::ProgramWithCallbacks Pad::create_program(const std::vector<Tensor>& input_tensors, std::vector<Tensor> &output_tensors) const {
    ...
    if (input_tensor.layout() == Layout::ROW_MAJOR) {
        ...
        return pad_cl(input_tensor, output_tensor, this->output_tensor_shape, this->input_tensor_start, this->pad_value);
    } else {
