...
def read_conv_weight_into_mm_act_block(mm_weight, weight_row_start_tile_id,
                                        weight_current_block_start_tile_id, weight_tile_stride_h,
                                        weight_block_stride_h, weight_block_h, weight_block_w):
    ...
    dst_address = 0
    for weight_tile_h in range(weight_block_h):
        weight_tile_id = weight_row_start_tile_id
        for weight_tile_w in range(weight_block_w):
            src_address = weight_tile_id * 1024; # Tile size = 1024
            for s in range(1024):
            ...
            weight_tile_id += 1
            dst_address += 1024
        weight_row_start_tile_id += weight_tile_stride_h
    weight_current_block_start_tile_id += weight_block_stride_h
    return (mm_weight_block.reshape(mm_weight_block_shape), weight_row_start_tile_id, weight_current_block_start_tile_id)
...
def blocked_mm_with_conv_act(conv_act,
                            ...
                            act_block_w,
                            weight_block_w,
                            weight_tile_stride_h,
                            weight_block_stride_h,
                            weight_block_stride_w):
    ...
    num_groups = address_map[act_address_map_index]
    ...
    for block_act_h in range(num_blocks_act_h):
        ...
        weight_start_tile_id = 0
        for block_weight_w in range(num_blocks_weight_w):
            weight_current_block_start_tile_id = weight_start_tile_id
            output_block = torch.zeros(mm_output_block_shape, dtype=torch.bfloat16).float()
            for block_act_w in range(num_blocks_act_w):
                weight_row_start_tile_id = weight_current_block_start_tile_id
                address_map_this_block_size = address_map[act_address_map_index]
                act_address_map_index += 1
                (mm_act_block, act_address_map_index) = read_conv_act_into_mm_act_block(conv_act, act_address_map_index,
                                                    address_map, address_map_this_block_size, act_block_h, act_block_w)
                (mm_weight_block, weight_row_start_tile_id, weight_current_block_start_tile_id) = read_conv_weight_into_mm_act_block(mm_weight, weight_row_start_tile_id,
                                                    weight_current_block_start_tile_id, weight_tile_stride_h,
                                                    weight_block_stride_h, weight_block_h, weight_block_w)
            ...
            weight_start_tile_id += weight_block_stride_w
