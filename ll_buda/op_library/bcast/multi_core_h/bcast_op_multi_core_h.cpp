...
Tensor bcast_multi_core_h(const Tensor &a, const Tensor &b, BcastOpMath::Enum bcast_math, BcastOpDim::Enum bcast_dim) {
    ...
    const char* reader_name = bcast::get_reader_name(bcast_dim, BcastOpParallelizationStrategy::MULTI_CORE_H);
    ...
    for (uint32_t i = 0; i < num_cores; i++){
    		...
    		void *hlk_args = new bcast::hlk_args_t { .B = NC, .Ht = Ht_per_core[i], .Wt = Wt };
