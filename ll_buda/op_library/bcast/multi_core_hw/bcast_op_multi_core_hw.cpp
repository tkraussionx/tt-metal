...
Tensor bcast_multi_core_hw(const Tensor &a, const Tensor &b, BcastOpMath::Enum bcast_math, BcastOpDim::Enum bcast_dim) {
    ...
    const char* reader_name = bcast::get_reader_name(bcast_dim, BcastOpParallelizationStrategy::MULTI_CORE_HW);
    ...
    for (uint32_t i = 0; i < num_cores; i++){
    		...
    		void *hlk_args = new bcast::hlk_args_t { .B = NC, .Ht = 1, .Wt = num_tiles_per_core[i] };
    ...
    //                      Execute Application
    ////////////////////////////////////////////////////////////////////////////
		for (uint32_t i = 0, num_tiles_read = 0; i < num_cores; num_tiles_read+=num_tiles_per_core[i], i++){
        tt_xy_pair core = {i / num_cores_y, i % num_cores_y};
        uint32_t num_tensor_tiles_per_core = num_tiles_per_core[i];
    ...
    ll_buda::dumpProfilerResults("multi_core_hw_" + std::to_string(num_cores), true);
