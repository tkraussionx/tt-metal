...
def test_falcon40B_attn_sequence(device, num_cores, seq_len):
    ...
    tiles_per_shard = head_dim * seq_len // num_slices // num_cores // 32
    if tiles_per_shard == 0:
        tiles_per_shard = 1

    print("Seq len is: ", seq_len, " tiles per shard is: ", tiles_per_shard)
...
@pytest.mark.parametrize("num_cores", [64])
def test_falcon40B_attn_sequence_experimental(device, num_cores, seq_len):
    compute_grid_size = device.compute_with_storage_grid_size()
    if num_cores > (compute_grid_size.x * compute_grid_size.y):
        pytest.skip(f"Need {num_cores} cores to run this test but core grid is {compute_grid_size}")
    grid_size = (8, 8)
...
@pytest.mark.parametrize(
    "seq_len",
    (32, 1024, 2048),
    ids=["seq_len_32", "seq_len_1024", "seq_len_2048"],
)
@pytest.mark.parametrize("num_cores", [64])
def test_falcon40B_layernorm(device, num_cores, seq_len):
    compute_grid_size = device.compute_with_storage_grid_size()
    if num_cores > (compute_grid_size.x * compute_grid_size.y):
        pytest.skip(f"Need {num_cores} cores to run this test but core grid is {compute_grid_size}")
    grid_size = (8, 8)
    ...
    layernorm_beta_shape = [1, 1, 256, 32]
    layernorm_epsilon = 1e-5

    torch_ln_input = torch.randn(layernorm_input_shape).bfloat16().float()
    torch_gamma = torch.randn(layernorm_gamma_shape).bfloat16().float()
    torch_beta = torch.randn(layernorm_beta_shape).bfloat16().float()
    ...
    tt_ln_input = torch2tt_tensor(
        torch_ln_input, device, ttl.tensor.Layout.TILE, dram_interleaved_memory_config, ttl.tensor.DataType.BFLOAT8_B
    )
    tt_ln_gamma = torch2tt_tensor(
        torch_gamma, device, ttl.tensor.Layout.ROW_MAJOR, dram_interleaved_memory_config, ttl.tensor.DataType.BFLOAT16
    )
