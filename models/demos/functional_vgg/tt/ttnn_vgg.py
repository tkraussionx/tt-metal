...
# from models.demos.functional_vgg.tt.vgg_preprocessing import preprocess_conv2d_weights, preprocess_conv2d_bias, preprocess_linear_weight, preprocess_linear_bias
...
def ttnn_vgg16(
    ...
    for itr, v in enumerate(cfgs["D"]):
        if v == "M":
            ...
            print("Iteration", itr)
            print("TTNN MAXPOOL2d", tt_x.shape)

            tt_x = ttnn.to_device(tt_x, device)
            ttact_d = ttnn.to_layout(tt_x, ttnn.ROW_MAJOR_LAYOUT)
            ...
            print("MAXPOOL2d COMPLETED", tt_x.shape, tt_x.layout)
            ...
            tt_weight = parameters[f"features." + str(conv_feature_ids[iter_conv_id]) + ".weight"]
            tt_bias = parameters[f"features." + str(conv_feature_ids[iter_conv_id]) + ".bias"]
            ...
            print("CONV COMPLETED", tt_x.shape, tt_x.layout)
    ...
    # Linear 1
    print("linear 1 input", tt_x.shape, tt_x.layout)
    print(
        "linear 1 weight",
        parameters["classifier." + str(classifier_ids[0]) + ".weight"].shape,
        parameters["classifier." + str(classifier_ids[0]) + ".weight"].layout,
    )
    print(
        "linear 1 bias",
        parameters["classifier." + str(classifier_ids[0]) + ".bias"].shape,
        parameters["classifier." + str(classifier_ids[0]) + ".bias"].layout,
    )

    # tt_x = ttnn.linear(tt_x, parameters["classifier"][classifier_ids[0]]["weight"], bias = parameters["classifier"][classifier_ids[0]]["bias"])
    tt_x = ttnn.linear(
        tt_x,
        parameters["classifier." + str(classifier_ids[0]) + ".weight"],
        bias=parameters["classifier." + str(classifier_ids[0]) + ".bias"],
    )
    tt_x = ttnn.relu(tt_x)

    # Linear 2
    # tt_x = ttnn.linear(tt_x, parameters["classifier"][classifier_ids[1]]["weight"], bias = parameters["classifier"][classifier_ids[1]]["bias"])
    tt_x = ttnn.linear(
        tt_x,
        parameters["classifier." + str(classifier_ids[1]) + ".weight"],
        bias=parameters["classifier." + str(classifier_ids[1]) + ".bias"],
    )
    tt_x = ttnn.relu(tt_x)

    # Linear 3
    # tt_x = ttnn.linear(tt_x, parameters["classifier"][classifier_ids[2]]["weight"], bias = parameters["classifier"][classifier_ids[2]]["bias"])
    tt_x = ttnn.linear(
        tt_x,
        parameters["classifier." + str(classifier_ids[2]) + ".weight"],
        bias=parameters["classifier." + str(classifier_ids[2]) + ".bias"],
    )
    return tt_x
...
def ttnn_vgg11(
    ...
    for itr, v in enumerate(cfgs["A"]):
        if v == "M":
            ...
            tt_x = ttnn.to_device(tt_x, device)
            ttact_d = ttnn.to_layout(tt_x, ttnn.ROW_MAJOR_LAYOUT)
            print("after row ,ajor", tt_x.shape, tt_x.layout)
            tt_x = ttnn.max_pool2d(
                input_tensor=ttact_d,
                batch_size=batch_size,
                input_h=int(math.sqrt(in_h / batch_size)),
            ...
            tt_weight = parameters[f"features." + str(conv_feature_ids_2[iter_conv_id]) + ".weight"]
            tt_bias = parameters[f"features." + str(conv_feature_ids_2[iter_conv_id]) + ".bias"]
