...
class Mamba(nn.Module):
    ...
    def from_pretrained(pretrained_model_name: str):
        ...
        args = ModelArgs(
            d_model=config_data["d_model"], n_layer=config_data["n_layer"], vocab_size=config_data["vocab_size"]
        )
        model = Mamba(args)
...
class MambaBlock(nn.Module):
    ...
    def ssm(self, x):
        ...
        (b, l, d_in, n) = deltaA.shape

        deltaA = deltaA.view(1, 1, b * l, d_in * n)
        deltaB_x = deltaB_x.view(1, 1, b * l, d_in * n)

        hidden_states = self.prefix_scan(deltaA, deltaB_x)

        hidden_states = hidden_states.view(b, l, d_in, n)

        y = einsum(hidden_states, C, "b l d_in n, b l n -> b l d_in")

        y = y + x * D

        return y

    def prefix_scan(self, A, x):
