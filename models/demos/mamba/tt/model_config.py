...
def get_tt_cache_path(model_version):
    tt_cache_path = Path("/mnt/MLPerf/tt_dnn-models/tt/Mamba") / model_version
    if tt_cache_path.exists():
        return tt_cache_path
    else:
        Path(f"models/demos/mamba/datasets/{model_version}").mkdir(parents=True, exist_ok=True)
        return Path(f"models/demos/mamba/datasets/{model_version}")
