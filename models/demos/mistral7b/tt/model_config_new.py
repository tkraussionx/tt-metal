...
class TtModelArgs:
    ...
    def __init__(self, device, model_base_path="/proj_sw/user_dev/hf_data/mistral/mistral-7B-v0.1/", instruct=False):
        ...
        self.model_config["KEYS_BATCHED_CONFIG"] = lambda padded_layer_past_len: ttnn.create_sharded_memory_config(
            shape=(8 * 128, padded_layer_past_len),
            core_grid=ttnn.CoreGrid(y=4, x=8),
        ...
        self.model_config["VALUES_BATCHED_CONFIG"] = lambda padded_layer_past_len: ttnn.create_sharded_memory_config(
            shape=(padded_layer_past_len, 8 * 128),
            core_grid=ttnn.CoreGrid(y=4, x=8),
            strategy=ttnn.ShardStrategy.HEIGHT,
            orientation=ttnn.ShardOrientation.ROW_MAJOR,
            use_height_and_width_as_shard_shape=True,
        ...
        self.model_config["QUERIES_BATCHED_CONFIG"] = ttnn.create_sharded_memory_config(
            shape=(32, 8 * 128),
            core_grid=ttnn.CoreGrid(y=4, x=8),
            strategy=ttnn.ShardStrategy.HEIGHT,
            orientation=ttnn.ShardOrientation.ROW_MAJOR,
