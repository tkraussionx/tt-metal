...
def run_2cq_model(device, tt_inputs, tt_resnet50, num_warmup_iterations, num_measurement_iterations):
    ...
    op_event = ttnn.experimental.device.CreateEvent()
    write_event = ttnn.experimental.device.CreateEvent()
    # Initialize the op event so we can write
    ttnn.experimental.device.RecordEvent(device, 0, op_event)

    profiler.start("compile")
    ttnn.experimental.device.WaitForEvent(device, 1, op_event)
    ttnn.experimental.tensor.write_tensor(tt_inputs, tt_image_res, 1)
    ttnn.experimental.device.RecordEvent(device, 1, write_event)
...
def run_trace_2cq_model(device, tt_inputs, tt_resnet50, num_warmup_iterations, num_measurement_iterations):
    ...
    ttnn.experimental.device.WaitForEvent(device, 0, write_event)
