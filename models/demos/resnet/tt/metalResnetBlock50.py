    ...
    def forward(self, x: ttnn.Tensor, write_event=None, op_event=None, final_out_mem_config=None) -> ttnn.Tensor:
        ...
        if not self.sharded:
            ...
            if write_event is not None:
                ttnn.experimental.device.WaitForEvent(self.device, 0, write_event)
            if x.storage_type() != ttnn.Tensor.StorageType.DEVICE:
            ...
            if op_event is not None:
                ttnn.experimental.device.RecordEvent(self.device, 0, op_event)
