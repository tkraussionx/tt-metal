...
class TtLlamaModelForGeneration:
    ...
    def capture_trace(self, tokens: torch.Tensor, start_pos: int):
        ...
        tt_inp_emb = ttnn.interleaved_to_sharded(tt_inp_emb, self.tt_model.core_model_config["EMBD_MEMCFG"])
        rot_mat = ttnn.to_device(rot_mat, self.mesh_device, memory_config=self.tt_model.core_model_config["ROT_MAT_MEMCFG"])
    ...
    def prefill_forward_single_user(
        ...
        tt_logits = self.tt_model(tt_inp_emb, rot_mat, start_pos, cache_idxs=None, attn_masks=attn_mask, user_id=user_id, mode="prefill")
