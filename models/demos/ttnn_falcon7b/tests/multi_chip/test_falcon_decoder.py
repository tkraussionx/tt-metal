...
@pytest.mark.parametrize("device_params", [{"trace_region_size": 114688}], indirect=True)
def test_falcon_decoder(
    ...
    logger.info("Done Capture")
    for i in range(0, 50):
        decoder_input, tt_decoder_input_updated = create_attention_input(
            llm_mode,
            dtype,
            batch,
            seq_len,
        ...
        ttnn.copy_host_to_device_tensor(tt_decoder_input_updated, tt_decoder_input)
        ttnn.execute_trace(pcie_device_mesh, trace_id, cq_id=0)
        tt_out_host = ttnn.to_torch(tt_out, mesh_composer=ConcatMeshToTensor(pcie_device_mesh, dim=shard_dim)).squeeze(
            1
        )
