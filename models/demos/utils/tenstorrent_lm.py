...
class TenstorrentLM(TemplateLM):
    ...
    def loglikelihood(self, requests, disable_tqdm: bool = False) -> List[Tuple[float, bool]]:
        ...
        if self.model_backend.continuous_batching:
            res = self.loglikelihood_continuous(requests, disable_tqdm)
        else:
            res = self.loglikelihood_batched(requests, disable_tqdm)
        return res

    def loglikelihood_continuous(self, requests, disable_tqdm: bool = False) -> List[Tuple[float, bool]]:
        res = []
        ...
        eval_logger.info(f"number of requests: {len(grouped_requests)}")
        for (group_key, req_list) in tqdm(grouped_requests.items(), disable=disable_tqdm):
            context_enc = self.tok_encode(req_list[0].args[0])
            continuation_enc = [self.tok_encode(req.args[1], add_special_tokens=False)[0] for req in req_list]
            cond_ids = torch.tensor(continuation_enc)
            logits = self.model_backend.loglikelihood(context_enc=context_enc, return_logits=True)
            # select 1st token in logits
            # next: set target indexes high for testing
            # target = req_list[0].doc["answer"]
            # target_id = cond_ids[target]
            # logits[0, target_id] = 1000.0
            # get log conditional likelihoods for all choices
            logits = logits.squeeze(0)
            probs = F.softmax(logits, dim=-1)
            greedy_id = probs.argmax(dim=-1)
            cond_probs = torch.gather(probs, 0, cond_ids)
            ll_list = torch.log(cond_probs).tolist()
            # set return values for each request by doc_id-choice
            is_greedy = (cond_ids == greedy_id).tolist()
            output = list(zip(ll_list, is_greedy))
            res.extend(output)
        return res

    def loglikelihood_batched(self, requests, disable_tqdm: bool = False) -> List[Tuple[float, bool]]:
        res = []
        # group by doc_id, task_name to support multiple tasks
        grouped_requests = defaultdict(list)
        for req in requests:
