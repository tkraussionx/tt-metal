...
class TenstorrentLM(TemplateLM):
    ...
    def loglikelihood(self, requests, disable_tqdm: bool = False) -> List[Tuple[float, bool]]:
        ...
        if self.model_backend.continuous_batching:
            res = self.loglikelihood_continuous(requests, disable_tqdm=True)
        else:
            res = self.loglikelihood_batched(requests, disable_tqdm)
        return res

    def loglikelihood_continuous(self, requests, disable_tqdm: bool = True) -> List[Tuple[float, bool]]:
        res = []
        ...
        for idx, (group_key, req_list) in tqdm(enumerate(grouped_requests.items()), disable=disable_tqdm):
