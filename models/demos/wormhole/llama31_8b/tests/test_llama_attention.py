...
def test_llama_attention_inference(mesh_device, use_program_cache, reset_seeds):
    ...
    for i in range(generation_length):
        ...
        # tt_out = tt_out[0]
        tt_output_torch = ttnn.to_torch(tt_out, mesh_composer=ttnn.ConcatMeshToTensor(mesh_device, dim=3))
        print("tt_output_torch shape: ", tt_output_torch.shape)
        # tt_output_torch = (
        #     tt_output_torch.view(1, -1, 4096).permute(1, 0, 2)[: model_args.max_batch_size, :, :]
        ...
        torch.save(tt_output_torch, "models/demos/wormhole/llama31_8b/outputs/output_TP.pth")
