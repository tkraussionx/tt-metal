...
def calc_tile_padding(dim, curr_tile_size, tile_size=32):
    """
    Function to pad a dimension to be a multiple of a tile size.
    Arguments:
        dim: Size of the dimension to be padded.
        curr_tile_size: Current tile size.
        tile_size: Tile size to pad to.
    """
    assert curr_tile_size <= tile_size

    c = dim / (curr_tile_size**2)
    pad_len = c * (tile_size**2) - dim
...
class TtLlamaConv2dPatch(LightweightModule):
    ...
    def __init__(
        ...
        pad_len = calc_tile_padding(weight.shape[-1], kernel_size)
    ...
    def forward(self, x: torch.Tensor):
        ...
        pad_len = calc_tile_padding(x.shape[-1], self.kernel_size[0])
