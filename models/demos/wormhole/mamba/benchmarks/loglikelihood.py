...
def compute_loglikelihood_given_prompt_and_target_for_wh(context_ids, target_ids, model: MambaDecode, vocab_size: int):
    # Reset the model hidden/conv states before decoding
    model.reset_states()
    ...
    for idx in range((input_ids.shape[-1])):
        ...
        tt_model_inputs = input_ids[:, idx].unsqueeze(1).repeat(32, 1)
        with torch.no_grad():
            out = model(tt_model_inputs)  # (B x 1) => (B x 1 x VOCAB)
            # select the first output from the batch
