...
def run_interactive_demo_inference_2(device, num_inference_steps, image_size=(256, 256)):
   disable_persistent_kernel_cache()
   device.enable_program_cache()
   ...
   prompt_file = "models/demos/wormhole/stable_diffusion/hf_prompts.txt"
   ...
   with open("metrics.txt", "w") as metrics_file:
       for i, prompt in enumerate(prompts):
           ...
           ttnn_output_path = f"{experiment_name}_ttnn.png"
           pil_images.save(ttnn_output_path)
           metrics_file.write(f"Prompt {i+1}: Time taken: {total_accum:.3f} seconds\n")
...
def test_sd_100_batch(device, num_inference_steps, image_size):
    return run_interactive_demo_inference_2(device, num_inference_steps, image_size)
