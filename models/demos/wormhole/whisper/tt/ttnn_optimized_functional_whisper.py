...
def calculate_key_values(
    config, device, key_value_states, *, parameters, whisper_memory_config, output_mesh_composer, inputs_mesh_mapper
):
    ...
    # #13672: Slice op doesn't support for 5d tensors.
    fused_qkv = ttnn.to_torch(fused_qkv, mesh_composer=output_mesh_composer)
    key_states, value_states = fused_qkv[..., 0, :, :], fused_qkv[..., 1, :, :]  #
    key_states = ttnn.from_torch(
        key_states, dtype=dtype, layout=ttnn.ROW_MAJOR_LAYOUT, device=device, mesh_mapper=inputs_mesh_mapper
    )
    value_states = ttnn.from_torch(
        value_states, dtype=dtype, layout=ttnn.ROW_MAJOR_LAYOUT, device=device, mesh_mapper=inputs_mesh_mapper
    )
    ...
    *,
    ...
    if is_cross_attention:
        ...
        key_states, value_states = calculate_key_values(
            ...
            device,
            key_value_states,
            parameters=parameters.key_value,
...
def encoder(
    config, device, inputs_embeds, *, parameters, whisper_memory_config, output_mesh_composer, inputs_mesh_mapper
):
    bs, _, _ = inputs_embeds.shape
    weights = parameters.embed_positions.weight
    weights = ttnn.concat([weights] * bs, dim=0)
    hidden_states = ttnn.add(inputs_embeds, weights)
    hidden_states = dropout(hidden_states, p=0, training=False)
    ...
    *,
    ...
    def ttnn_conv1d(
        ...
        *,
        ...
        weights = ttnn.from_torch(weights, dtype=ttnn.float32, mesh_mapper=weights_mesh_mapper)
        bias = ttnn.from_torch(
            bias.unsqueeze(0).unsqueeze(0).unsqueeze(0), dtype=ttnn.float32, mesh_mapper=weights_mesh_mapper
        )
    ...
    if parameters.conv1.weight.shape[0] == 512:
        input_features = ttnn.from_torch(
            input_features, dtype=ttnn.bfloat16, device=device, layout=ttnn.TILE_LAYOUT, mesh_mapper=inputs_mesh_mapper
        )
        ...
        conv1 = ttnn.from_torch(
            conv1, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device, mesh_mapper=inputs_mesh_mapper
        )
    ...
    input_embeds = ttnn.to_torch(input_embeds, mesh_composer=output_mesh_composer)
    ...
    conv = ttnn.from_torch(
        conv, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device, mesh_mapper=inputs_mesh_mapper
    )
...
def preprocess_decoder_inputs(config, input_ids, attention_mask, *, parameters, device, inputs_mesh_mapper):
    ...
    attention_mask = ttnn.from_torch(
        attention_mask, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device, mesh_mapper=inputs_mesh_mapper
    )
...
def whisper_for_audio_classification(
    config,
    inputs_embeds,
    *,
    parameters,
...
def custom_preprocessor(torch_model, name):
    ...
    if isinstance(torch_model, transformers.models.whisper.modeling_whisper.WhisperAttention):
        ...
        embeddings = torch_model.weight.unsqueeze(0)
        embeddings = ttnn.from_torch(embeddings, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
