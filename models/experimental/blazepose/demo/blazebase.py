...
class BlazeDetector(BlazeBase):
    ...
    def predict_on_batch(self, x):
        ...
        with torch.no_grad():
            out = self.__call__(x)
        torch.save(out[0], "output1.pt")
        torch.save(out[1], "output2.pt")
    ...
    def _tensors_to_detections(self, raw_box_tensor, raw_score_tensor, anchors):
        ...
        print(
            "Shape of mask and detection boxes pytorch:",
            mask.shape,
            " ",
            detection_boxes.shape,
            " ",
            raw_box_tensor.shape,
        )
        # Shape of mask and detection shape: ttnn.Shape([1, 896])   ttnn.Shape([1, 896, 12])   ttnn.Shape([1, 896, 12])
        output_detections = []
        for i in range(raw_box_tensor.shape[0]):
            print("What is i", i, " ", mask[i].shape)
    ...
    def _decode_boxes(self, raw_boxes, anchors):
        ...
        torch.save(raw_boxes, "raw_boxes.pt")

        x_center = raw_boxes[..., 0] / self.x_scale * anchors[:, 2] + anchors[:, 0]
        torch.save(x_center, "x_center.pt")
        y_center = raw_boxes[..., 1] / self.y_scale * anchors[:, 3] + anchors[:, 1]
        torch.save(y_center, "y_center.pt")
    ...
    def _weighted_non_max_suppression(self, detections):
        ...
        while len(remaining) > 0:
            ...
            torch.save(other_boxes, "other_boxes.pt")
            ious = overlap_similarity(first_box, other_boxes)
            torch.save(ious, "ious.pt")
            ...
            print("SHape of remaining Pytorch:", remaining)
            ...
            if len(overlapping) > 1:
                ...
                print(
                    "Shape of scores and coordinates in torch:",
                    coordinates.shape,
                    " ",
                    scores.shape,
                    " ",
                    weighted_detection.shape,
                )
...
def intersect(box_a, box_b):
    ...
    """
    print("Shape of boxes torch:", box_a.shape, " ", box_b.shape)
    A = box_a.size(0)
    B = box_b.size(0)
    max_xy = torch.min(
...
def jaccard(box_a, box_b):
    ...
    print("Shape of box a and b torch:", box_a.shape, " ", box_b.shape, " ", inter.shape)
