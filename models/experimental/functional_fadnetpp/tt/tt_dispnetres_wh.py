...
from models.experimental.functional_fadnetpp.tt.ttnn_resblock_wh import TtResBlock
import tt_lib
...
class TtDispNetRes(nn.Module):
    def __init__(
        self, device, parameters, in_planes, model, resBlock=True, input_channel=3, encoder_ratio=16, decoder_ratio=16
    ):
        super(TtDispNetRes, self).__init__()
        print("DispNetRes model:", model)
        ...
        conv1_weight = self.parameters["conv1"]["weight"]
        conv1_bias = self.parameters["conv1"]["bias"]
        ...
        if resBlock:
            self.conv2 = TtResBlock(parameters.conv2, self.basicE, self.basicE * 2, model.conv2, stride=2)
            self.conv3 = TtResBlock(parameters.conv3, self.basicE * 2, self.basicE * 4, model.conv3, stride=2)
            self.conv3_1 = TtResBlock(parameters.conv3_1, self.basicE * 4, self.basicE * 4, model.conv3_1)
            self.conv4 = TtResBlock(parameters.conv4, self.basicE * 4, self.basicE * 8, model.conv4, stride=2)
            self.conv4_1 = TtResBlock(parameters.conv4_1, self.basicE * 8, self.basicE * 8, model.conv4_1)
            ...
            self.conv5_1 = TtResBlock(parameters.conv5_1, self.basicE * 16, self.basicE * 16, model.conv5_1)
            self.conv6 = TtResBlock(parameters.conv6, self.basicE * 16, self.basicE * 32, model.conv6, stride=2)
            self.conv6_1 = TtResBlock(parameters.conv6_1, self.basicE * 32, self.basicE * 32, model.conv6_1)
        else:
    ...
    def __call__(self, device, input_tensor, flows, get_features=False):
        ...
        conv1 = ttnn.leaky_relu(conv1, slope=0.1)
        ...
        pr6_res = ttnn.to_torch(pr6_res)
        base_flow_6 = base_flow[6]
        base_flow_6 = ttnn.to_torch(base_flow_6)
        pr6 = pr6_res + base_flow_6
        ...
        pr5_res = ttnn.to_torch(pr5_res)
        base_flow_5 = base_flow[5]
        base_flow_5 = ttnn.to_torch(base_flow_5)
        pr5 = pr5_res + base_flow_5
        ...
        pr4_res = ttnn.to_torch(pr4_res)
        base_flow_4 = base_flow[4]
        base_flow_4 = ttnn.to_torch(base_flow_4)
        pr4 = pr4_res + base_flow_4
        ...
        pr3_res = ttnn.to_torch(pr3_res)
        base_flow_3 = base_flow[3]
        base_flow_3 = ttnn.to_torch(base_flow_3)
        pr3 = pr3_res + base_flow_3
        ...
        pr2_res = ttnn.to_torch(pr2_res)
        base_flow_2 = base_flow[2]
        base_flow_2 = ttnn.to_torch(base_flow_2)
        pr2 = pr2_res + base_flow_2
        ...
        pr1_res = ttnn.to_torch(pr1_res)
        base_flow_1 = base_flow[1]
        base_flow_1 = ttnn.to_torch(base_flow_1)
        pr1 = pr1_res + base_flow_1
        ...
        pr0_res = ttnn.to_torch(pr0_res)
        base_flow_0 = base_flow[0]
        base_flow_0 = ttnn.to_torch(base_flow_0)
        pr0 = pr0_res + base_flow_0
