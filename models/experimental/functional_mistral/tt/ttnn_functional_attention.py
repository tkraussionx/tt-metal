...
def apply_rotary_emb(xq, xk, bcast_freq_xq, bcast_freq_xk, device, mem_config):
    ...
    xq = ttnn.complex_tensor(xq_real, xq_img)
    ...
    xk = ttnn.complex_tensor(xk_real, xk_img)

    xk_real.deallocate()
    xk_img.deallocate()

    xq_out = ttnn.mul(xq, bcast_freq_xq, memory_config=mem_config)

    xk_out = ttnn.mul(xk, bcast_freq_xk, memory_config=mem_config)
