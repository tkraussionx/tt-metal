        ...
        if USE_DIFFUSER_SCHEDULER:
            ...
            for t in ttnn_scheduler.timesteps:
                ...
                with torch.no_grad():
                    ...
                    print(f"Sample: {iter}")
                    # noise_pred = unet(
                    #    ttnn.to_torch(ttnn_latent_model_input).float(), t, ttnn.to_torch(ttnn_text_embeddings).float()
                    # ).sample
            ...
            for i in range(len(time_step_list)):
                # expand the latents if we are doing classifier-free guidance to avoid doing two forward passes.
                ttnn_latent_model_input = tt_latent_expansion(
                    ttnn_latents, ttnn_scheduler, float(ttnn_sigma[i]), device
                )
