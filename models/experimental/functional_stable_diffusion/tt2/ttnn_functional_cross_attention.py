...
class cross_attention:
    def __init__(self, device, parameters):
        ...
        attention_mask_64_96 = torch.ones((2, 1, 64, 96)) * -1e9
        ...
        self.out_subblock_hs = {8192: 4, 2048: 4, 512: 2, 128: 1}
    ...
    def time_sharded_attention(self, query, t_key, value, head_size):
        ...
        for j in range(2):
            for i in range(num_slices // 2):
                ...
                # torch_mm_slice = ttnn_to_torch(slice) @ ttnn_to_torch(k_slice)
                # mm_slice = ttnn.from_torch(torch_mm_slice, ttnn.bfloat8_b, layout=ttnn.TILE_LAYOUT)
                # mm_slice = ttnn.to_device(mm_slice, self.device)
                # mm_slice = ttnn.experimental.tensor.interleaved_to_sharded(
                #     mm_slice,
                #     grid_size,
    ...
    def sharded_attention(self, query, key, value, original_seq_len, head_size, index):
        ...
        compare(attention_scores, f"attn_qkt_{index}.pt")
        ...
        if num_cores_softmax > 64:
            ...
            end_grid = ttnn.experimental.tensor.CoreCoord(5, 7)
        ...
        input_shape = attention_scores.shape
        ...
        attention_scores = ttnn.reshape(
            ...
            (
                ...
                1,
                attention_scores.shape[1] * attention_scores.shape[2],
                attention_scores.shape[3],
            ),
        )

        torch_attention_scores = ttnn_to_torch(attention_scores)
        torch_attention_scores *= 1 / math.sqrt(head_size)
        torch_attention_scores = torch_attention_scores.softmax(dim=-1)
        attention_scores = ttnn.from_torch(torch_attention_scores, dtype=ttnn.bfloat8_b, layout=ttnn.TILE_LAYOUT)
        attention_scores = ttnn.to_device(attention_scores, self.device)
        attention_scores = ttnn.experimental.tensor.interleaved_to_sharded(
            attention_scores,
            grid_size,
            [attention_scores.shape[-2] // num_cores, attention_scores.shape[-1]],
            ttnn.experimental.tensor.TensorMemoryLayout.HEIGHT_SHARDED,
            ttnn.experimental.tensor.ShardOrientation.COL_MAJOR,
        )
        ...
        attention_scores = ttnn.reshape(attention_scores, input_shape)

        compare(attention_scores, f"attn_softmax_{index}.pt")
        ...
        compare(attention_scores, f"attn_value_{index}.pt")
    ...
    def get_attention_scores_opt(self, query, t_key, value, original_seq_len, head_size, index=-1):
        if query.shape[-2] == 4096 and t_key.shape[-1] == 4096:
            ...
            not (query.shape[-2] == 1024 and t_key.shape[-1] == 1024)
            and not (query.shape[-2] == 4096 and t_key.shape[-1] == 96)
            and not (query.shape[-2] == 96 and t_key.shape[-1] == 96)
        ):
    ...
    def out(self, hidden_states):
        ...
        grid_sizes = {4096: (8, 8), 1024: (8, 8), 256: (8, 5), 64: (8, 5)}
    ...
    def __call__(
        ...
        if self.fused_qkv:
            ...
            compare(query, f"attn_q_{index}.pt")
            compare(key, f"attn_k_{index}.pt", transpose=True)
            compare(value, f"attn_v_{index}.pt")
            ...
            compare(query, f"attn_q_{index}.pt")
            compare(key, f"attn_k_{index}.pt", transpose=True, unpad=True)
            compare(value, f"attn_v_{index}.pt", unpad=True)
        ...
        compare(hidden_states, f"attn_value_{index}.pt")
        ...
        compare(hidden_states, f"attn_out_{index}.pt")
