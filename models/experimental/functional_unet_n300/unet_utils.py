...
from models.experimental.functional_unet_n300.tt import unet_shallow_torch
from models.experimental.functional_unet_n300.tt import unet_shallow_ttnn


def create_unet_models(device, batch, groups, torch_input_tensor, weights_mesh_mapper=None, output_mesh_composer=None):
    ...
    # )
    parameters = {}
    ttnn_model = unet_shallow_ttnn.UNet(
        device,
        parameters,
        batch,
        torch_model,
...
def create_unet_input_tensors(device, batch, groups, inputs_mesh_mapper=None):
    ...
    ttnn_input_tensor = ttnn.from_torch(ttnn_input_tensor, dtype=ttnn.bfloat16, mesh_mapper=inputs_mesh_mapper)
...
def create_custom_preprocessor(device, groups=1):
    def custom_preprocessor(model, name, ttnn_module_args):
        ...
        if isinstance(model, unet_shallow_torch.UNet):
            ...
            #     {"act_block_h": 32}
            #     if groups > 1
            #     else ({"act_block_h": 8 * 32} if device.arch() == ttl.device.Arch.WORMHOLE_B0 else {"act_block_h": 64})
            # )
            ...
            # ttnn_module_args.c8_2["conv_blocking_and_parallelization_config_override"] = {"act_block_h": 8*32}
