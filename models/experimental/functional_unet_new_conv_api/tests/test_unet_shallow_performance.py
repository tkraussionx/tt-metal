...
def test_unet_model_performance(device_mesh, perf_mode, batch, groups, loop):
    with torch.no_grad():
...
@pytest.mark.parametrize("batch", [2])
@pytest.mark.parametrize("groups", [1])
def test_unet_device_performance_helper(device_mesh, perf_mode, batch, groups):
    with torch.no_grad():
        torch.manual_seed(0)
        num_devices = 1 if isinstance(device_mesh, ttnn.Device) else device_mesh.get_num_devices()
...
def test_unet_device_performance(num_iterations, margin, expected_perf, batch):
    ...
    command = f"pytest models/experimental/functional_unet_new_conv_api/tests/test_unet_shallow_performance.py::test_unet_device_performance_helper"
