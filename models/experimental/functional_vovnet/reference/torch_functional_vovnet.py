...
def _create_pool(
    ...
    num_pooled_features = num_features * adaptive_pool_feat_mult(pool_type)

    return global_pool


def adaptive_pool_feat_mult(pool_type: str) -> int:
    if pool_type in ("", "fast", "avg", "max"):
        return 1
    else:
        return 1
...
def osa_block(
    x, parameters, running_mean, running_var, in_chs, mid_chs, out_chs, layer_per_block, residual=False, depthwise=True
):
    output = [x]
    next_in_chs = in_chs
...
def osa_stage(
    ...
    x = osa_block(
        x, parameters.blocks[0], rm, rv, in_chs, mid_chs, out_chs, layer_per_block, residual=False, depthwise=True
    )
...
def vovnet(
    ...
    if depthwise:
        x = seperable_conv_norm_act(
            ...
            stem_chs[1],
            parameters.stem[1],
            running_mean.stem[1].bn.running_mean,
            running_var.stem[1].bn.running_var,
            stem_chs[0],
            kernel_size=3,
            stride=1,
            padding=1,
        ...
        x = seperable_conv_norm_act(
            ...
            stem_chs[2],
            parameters.stem[2],
            running_mean.stem[2].bn.running_mean,
            running_var.stem[2].bn.running_var,
            stem_chs[1],
            kernel_size=3,
            stride=2,
            padding=1,
