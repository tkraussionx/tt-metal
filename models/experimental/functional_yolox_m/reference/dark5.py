...
class Dark5(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x3_b = self.b3(x3)
        x3_m = self.silu(x3_b)
        input_tensor = ttnn.from_torch(x3_m, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
        ttnn.dump_tensor("tests/ttnn/integration_tests/yolox_m/dark5_conv4_inp_torch.pt", input_tensor)
