...
def test_llama2_attention(device, pcc, model_location_generator, reset_seeds):
    ...
    torch_model = facebook_research_reference_model.model.layers[0].attention
    ...
    Tt_model = TtAttention(ModelArgs, state_dict, base_address, device)
    ...
    if does_pass:
        logger.info("Llama2Attention Passed!")

    assert does_pass, f"Llama2Attention does not meet PCC requirement {pcc}."
