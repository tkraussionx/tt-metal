...
def test_mamba_model_inference(device, use_program_cache, pcc: float):
    ...
    tt_input = ttnn.from_torch(input_tensor, device=device, layout=ttnn.TILE_LAYOUT, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
    
    # Warmup
    input_tensor = ttnn.permute(tt_input, (3, 0, 1, 2))
    repeat_interleaved_output = ttnn.repeat_interleave(input_tensor, 32, dim=0)
    ...
    tt_output = ttnn.to_torch(repeat_interleaved_output)
