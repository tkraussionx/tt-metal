...
class TtMambaBlock(torch.nn.Module):
    def __init__(
        ...
        if self.args.d_model == self.hidden_size:
            ...
            self.ssm_proj = ttnn.as_tensor(ssm_proj, layout=ttnn.TILE_LAYOUT, device=self.device,
                                 memory_config=ttnn.DRAM_MEMORY_CONFIG, dtype=ttnn.bfloat8_b, cache_file_name=tt_cache_path + "ssm_proj_bfp8.bin")
        else:
        ...
        if self.args.d_model == self.hidden_size:
            ...
            self.mlp_proj = ttnn.as_tensor(mlp_proj, layout=ttnn.TILE_LAYOUT, device=self.device,
                                 memory_config=ttnn.DRAM_MEMORY_CONFIG, dtype=ttnn.bfloat8_b, cache_file_name=tt_cache_path + "mlp_proj_bfp8.bin")
        else:
        ...
        for i in range(4):
            if self.args.d_model == self.hidden_size:
                ...
                self.conv_wts.append(ttnn.as_tensor(conv_wts, layout=ttnn.TILE_LAYOUT, device=self.device,
                                 memory_config=ttnn.DRAM_MEMORY_CONFIG, dtype=ttnn.bfloat8_b, cache_file_name=tt_cache_path + f"conv_wts_{i}_bfp8.bin"))
            else:
        ...
        if self.args.d_model == self.hidden_size:
            ...
            self.conv_bias = ttnn.as_tensor(conv_bias, layout=ttnn.TILE_LAYOUT, device=self.device,
                                 memory_config=ttnn.DRAM_MEMORY_CONFIG, dtype=ttnn.bfloat8_b, cache_file_name=tt_cache_path + "conv_bias_bfp8.bin")
        else:
    ...
    def forward(self, x):
        ...
        ssm_proj = ttnn.to_memory_config(self.ssm_proj, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
        ttnn.deallocate(self.conv_states[0])
        x = ttnn.linear(x_input, ssm_proj, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
        ...
        # do the convolution
        conv_wts_tmp = ttnn.repeat_interleave(self.conv_wts[3], self.num_users, dim=2)
        conv_wts = ttnn.to_memory_config(conv_wts_tmp, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
        ttnn.deallocate(conv_wts_tmp)
        
        accum = []
        
        accum.append(ttnn.mul(conv_wts, self.conv_states[3], memory_config=ttnn.L1_MEMORY_CONFIG))
        prod = None
        for i in range(1,4):
            ttnn.deallocate(conv_wts)
            conv_wts_tmp = ttnn.repeat_interleave(self.conv_wts[i], self.num_users, dim=2)
            conv_wts = ttnn.to_memory_config(conv_wts_tmp, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
            ttnn.deallocate(conv_wts_tmp)
            ...
            accum.append(ttnn.add(accum[-1], prod, memory_config=ttnn.L1_MEMORY_CONFIG))
            ttnn.deallocate(accum[-2])
            
        ttnn.deallocate(conv_wts)
        conv_bias_tmp = ttnn.repeat_interleave(self.conv_bias, self.num_users, dim=2)
        conv_bias = ttnn.to_memory_config(conv_bias_tmp, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
        ttnn.deallocate(conv_bias_tmp)
        
        ttnn.deallocate(prod)
        x_conv = ttnn.add(accum[-1], conv_bias, memory_config=ttnn.L1_MEMORY_CONFIG)

        ttnn.deallocate(accum[-1])
        x_silu = ttnn.silu(x_conv, memory_config=ttnn.L1_MEMORY_CONFIG)
        ttnn.deallocate(ssm_proj)
        
        mlp_proj = ttnn.to_memory_config(self.mlp_proj, memory_config=ttnn.L1_MEMORY_CONFIG, dtype=ttnn.bfloat16)
        res = ttnn.linear(x_input, mlp_proj, memory_config=ttnn.L1_MEMORY_CONFIG)
        ttnn.deallocate(mlp_proj)  
        
        x = self.tt_ssm(x_silu)
        
        
      
        
        
        x_next = ttnn.mul(x, res, memory_config=ttnn.L1_MEMORY_CONFIG)
        ttnn.deallocate(res)
        ttnn.deallocate(x)
        x = x_next
        
        x_next = ttnn.linear(x, self.down_proj, memory_config=ttnn.L1_MEMORY_CONFIG)
        ttnn.deallocate(x)
        x = x_next
