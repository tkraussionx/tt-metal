...
class TtGPT(nn.Module):
    def __init__(self, config, device, tt_cache_path, dtype):
        ...
        self.out_mem_config_dram = tt_lib.tensor.MemoryConfig(
            tt_lib.tensor.TensorMemoryLayout.INTERLEAVED, tt_lib.tensor.BufferType.DRAM
        )
        self.beta = tt_lib.tensor.load_tensor(tt_cache_path + base_address + ".ln_f.bias" + str(dtype) + ".bin").to(
            self.device, self.out_mem_config_l1
        )
        self.gamma = tt_lib.tensor.load_tensor(tt_cache_path + base_address + ".ln_f.weight" + str(dtype) + ".bin").to(
            self.device, self.out_mem_config_l1
        )
    ...
    def forward(self, idx: torch.Tensor) -> tt_lib.tensor.Tensor:
        ...
        tt_tok_emb = format_tensor(tt_tok_emb, tt_lib.tensor.Layout.TILE, self.device, self.out_mem_config_l1)
        tt_pos_emb = format_tensor(tt_pos_emb, tt_lib.tensor.Layout.TILE, self.device, self.out_mem_config_l1)
        ...
        tt_x = format_tensor(tt_x, tt_lib.tensor.Layout.TILE, self.device, self.out_mem_config_l1)
