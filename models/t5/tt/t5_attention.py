...
def t5_unshape_pt(
    states: torch.FloatTensor, batch_size: int, inner_dim: int
) -> torch.FloatTensor:
...
class TtT5Attention(nn.Module):
    def __init__(
        ...
        self.key = TtLinear(
            self.q_weights.shape()[-1], self.q_weights.shape()[-2], self.k_weights
        )
        ...
        self.value = TtLinear(
            self.q_weights.shape()[-1], self.q_weights.shape()[-2], self.v_weights
        )
        ...
        self.output = TtLinear(
            self.q_weights.shape()[-2], self.q_weights.shape()[-1], self.o_weights
        )
    ...
    def compute_bias_const(self, query_length, key_length):
        ...
        values = tt_lib.tensor.permute(values, 0, 3, 1, 2)
        values = tt_to_torch_tensor(values)
        return values
    ...
    def forward(
        ...
        if (
            ...
            position_bias = tt_lib.fallback_ops.repeat(
                position_bias, ((batch_size, 1, 1, 1))
            )
            ...
            if mask is not None:
                ...
                position_bias = tt_lib.tensor.permute(position_bias, 0, 2, 3, 1)
