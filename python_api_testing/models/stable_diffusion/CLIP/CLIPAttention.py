...
class TtCLIPAttention(nn.Module):
    ...
    def __init__(self, device, config=None, hidden_size=None, num_attention_heads=None):
        super().__init__()
        self.config = config
        self.device = device
        self.embed_dim = config.hidden_size if config else hidden_size
        ...
        self.weights = torch.ones((1, 1, self.embed_dim, self.embed_dim)).flatten().tolist()
        self.bias = torch.zeros((1, 1, self.embed_dim, self.embed_dim)).flatten().tolist()

        self.k_proj = TtLinear(self.embed_dim, self.embed_dim, self.weights, self.bias, device)
        self.v_proj = TtLinear(self.embed_dim, self.embed_dim, self.weights, self.bias, device)
        self.q_proj = TtLinear(self.embed_dim, self.embed_dim, self.weights, self.bias, device)
        self.out_proj = TtLinear(self.embed_dim, self.embed_dim, self.weights, self.bias, device)
    ...
    def forward(
        ...
        attn_output = self.out_proj(attn_output)
        return attn_output  # output_attention is ignored since it is false and makes no difference in our case
...
#         self.num_heads = config.num_attention_heads
    ...
    hidden_state_shape = [1, 1, D, hidden_size]
