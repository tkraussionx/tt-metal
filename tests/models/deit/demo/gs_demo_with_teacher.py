...
from deit_for_image_classification_with_teacher import (
...
def test_gs_demo(hf_cat_image_sample_input, device):
    ...
    with torch.no_grad():
        ...
        tt_output_with_teacher = tt_to_torch_tensor(tt_output_with_teacher).squeeze(0)[
            :, 0, :
        ]
    ...
    logger.info(
        f"TT's prediction: {torch_model_with_teacher.config.id2label[predicted_label]}."
    )
