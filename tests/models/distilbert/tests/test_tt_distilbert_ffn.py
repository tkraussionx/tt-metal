# SPDX-FileCopyrightText: © 2023 Tenstorrent Inc.

# SPDX-License-Identifier: Apache-2.0

import pytest
from loguru import logger
import torch
import tt_lib
from models.utility_functions import torch_to_tt_tensor_rm, tt_to_torch_tensor
from models.utility_functions import (
    comp_allclose,
    comp_pcc,
)
from models.models_wip.distilbert.tt.distilbert_ffn import TtFFN
from transformers import (
    DistilBertForQuestionAnswering as HF_DistilBertForQuestionAnswering,
)


@pytest.mark.parametrize(
    "pcc",
    ((0.99),),
)
def test_distilbert_ffn_inference(pcc, reset_seeds):
    device = tt_lib.device.CreateDevice(0)

    tt_lib.device.SetDefaultDevice(device)

    HF_model = HF_DistilBertForQuestionAnswering.from_pretrained(
        "distilbert-base-uncased-distilled-squad"
    )
    LAYER_INDEX = 0
    torch_model = HF_model.distilbert.transformer.layer[LAYER_INDEX].ffn
    base_address = f"distilbert.transformer.layer.{LAYER_INDEX}"

    # Tt distilbert_ffn
    config = HF_model.config
    tt_model = TtFFN(
        config,
        state_dict=HF_model.state_dict(),
        base_address=base_address,
        device=device,
    )

    input = torch.rand(1, 19, 768)

    tt_input = torch_to_tt_tensor_rm(input, device, put_on_device=True)

    with torch.no_grad():
        torch_output = torch_model(input)
        tt_output = tt_model(tt_input)

    # Compare outputs
    tt_output_torch = tt_to_torch_tensor(tt_output).squeeze(0)

    does_pass, pcc_message = comp_pcc(torch_output, tt_output_torch, pcc)

    logger.info(comp_allclose(torch_output, tt_output_torch))
    logger.info(pcc_message)

    tt_lib.device.CloseDevice(device)

    if does_pass:
        logger.info("DistilBertFFN Passed!")

    assert does_pass, f"DistilBertFFN does not meet PCC requirement {pcc}."
