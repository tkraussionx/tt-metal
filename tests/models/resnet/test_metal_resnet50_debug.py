...
def test_run_resnet50_inference(use_program_cache, batch_size, imagenet_sample_input):
    ...
    with torch.no_grad():
        ...
        # Run once to get saved output
        ref_tt_output = tt_resnet50.run_forward(image)

        # run 4 times back to back
        for i in range(50):
            print(i)
            tt_output = tt_resnet50.run_forward(image)
            if not torch.equal(tt_output, ref_tt_output):
                print(tt_output)
                print("golden -")
                print(ref_tt_output)
                tt_lib.device.CloseDevice(device)

            assert torch.equal(tt_output, ref_tt_output)
