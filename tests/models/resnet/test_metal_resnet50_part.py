...
def test_run_resnet50_inference(use_program_cache, batch_size, imagenet_sample_input):
    ...
    with torch.no_grad():
        ...
        tt_conv1_output_utilize_out, tt_max_pool_output_with_untilize_out = tt_resnet50.run_first_part(image)

        print("Compare pytorch conv1 output with TT conv1 + untilize")
        passing, info = comp_allclose_and_pcc(torch_conv1_output, tt_conv1_output_utilize_out, pcc=0.985)
        logger.info(info)
        golden_pcc = 0.985
        passing_pcc_conv1, _ = comp_pcc(torch_conv1_output, tt_conv1_output_utilize_out, pcc=golden_pcc)

        print("Compare pytorch maxpool output with TT conv1 + untilize + maxpool")
        passing, info = comp_allclose_and_pcc(torch_max_pool_output, tt_max_pool_output_with_untilize_out, pcc=0.985)
        logger.info(info)
        golden_pcc = 0.985
        passing_pcc_maxpool, _ = comp_pcc(torch_max_pool_output, tt_max_pool_output_with_untilize_out, pcc=golden_pcc)
