...
def test_resnet50_conv(use_program_cache, device, N,K,C,H,W,R,S,stride_h,stride_w,pad_h,pad_w):
    ...
    for i in range(1): # increase num of iterations to test op caching
        ...
        if (is_1x1_conv):
            ...
            [act_block_w_datums, act_block_h_datums, weight_block_w_datums, out_subblock_h_datums, out_subblock_w_datums, out_block_h_datums, grid_size, per_core_out_matrix_h, per_core_weight_matrix_w] = hardcoded_conv_blocking_and_parallelization_config[N][(conv_as_mm_padded_act_height, K)]
            if R == 1 and S == 1:
            ...
            conv = resnet50_optimized_conv(conv_weight_pyt.reshape(-1).tolist(),
                                conv_params,
                                device,
                                [act_block_h_datums, act_block_w_datums], [act_block_w_datums, weight_block_w_datums],
