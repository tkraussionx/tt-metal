...
def test_resnet50_conv(use_program_cache, device, N,K,C,H,W,R,S,stride_h,stride_w,pad_h,pad_w, use_new_matmul):
    ...
    # )
        ...
        # NCHW to NHWC
        out_golden_r = torch.transpose(out_golden, 1, 2)
        out_golden_r = torch.transpose(out_golden_r, 2, 3)

        outN = N
        outC = conv_output_shape[3]
        outH = conv_output_shape[1]
        outW = conv_output_shape[2]
        assert out_result.shape == out_golden_r.shape
        assert list(out_result.size()) == [outN, outH, outW, outC]
        out_result_r = torch.reshape(out_result, (1, 1, outN*outH*outW, outC))
        out_golden_r = torch.reshape(out_golden_r, (1, 1, outN*outH*outW, outC))
        num_rows = outN*outH*outW
        num_cols = outC
        atol_tensor = torch.abs(out_golden_r - out_result_r)
        rtol_tensor = torch.abs(out_golden_r - out_result_r) / torch.abs(out_result_r)
        atol_boolean_tensor = torch.abs(out_golden_r - out_result_r) > 20
        atol_tensor = torch.reshape(atol_tensor, (num_rows, num_cols))
        rtol_tensor = torch.reshape(rtol_tensor, (num_rows, num_cols))
        atol_boolean_tensor = torch.reshape(atol_boolean_tensor, (num_rows, num_cols))
        torch.save(atol_tensor, "atol_tensor_passing_config.pt")
        torch.save(rtol_tensor, "rtol_tensor_passing_config.pt")
        torch.save(atol_boolean_tensor, "atol_boolean_tensor_passing_config.pt")
