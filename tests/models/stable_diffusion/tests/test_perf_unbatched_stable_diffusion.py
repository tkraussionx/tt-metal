...
def run_perf_unbatched_stable_diffusion(expected_inference_time, expected_compile_time):
    ...
    for t in tqdm(tt_scheduler.timesteps):
        ...
        tt_conditioned = tt_conditioned.to(device, ttl.tensor.MemoryConfig(ttl.tensor.TensorMemoryLayout.INTERLEAVED, ttl.tensor.BufferType.L1))
        tt_unconditioned = tt_unconditioned.to(device, ttl.tensor.MemoryConfig(ttl.tensor.TensorMemoryLayout.INTERLEAVED, ttl.tensor.BufferType.L1))
