...
def test_swin_intermediate_inference(pcc, reset_seeds):
    ...
    # Torch swinintermediate
    torch_model = (
        model.encoder.layers[INTERMEDIATE_LAYER_INDEX]
        .blocks[INTERMEDIATE_LAYER_INDEX]
        .intermediate
    )

    # Tt swinintermediate
    dim = 96
    tt_model = TtSwinIntermediate(
        config=model.config,
        dim=dim,
    ...
    if does_pass:
        ...
        logger.warning("SwinIntermediate Failed!")
