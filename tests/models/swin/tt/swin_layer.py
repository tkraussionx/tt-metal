...
from tests.models.swin.tt.swin_attention import TtSwinAttention
from tests.models.swin.tt.swin_intermediate import TtSwinIntermediate
from tests.models.swin.tt.swin_output import TtSwinOutput
from tests.models.swin.swin_utils import (
    window_partition,
    window_reverse,
)
...
class TtSwinLayer(nn.Module):
    ...
    def get_attn_mask(self, height, width, dtype):
        if self.shift_size > 0:
            ...
            img_mask = torch_to_tt_tensor_rm(img_mask, self.host)
            mask_windows = window_partition(img_mask, self.window_size, self.host)
            ...
            attn_mask = torch_to_tt_tensor_rm(attn_mask, self.host)
        else:
            attn_mask = None
    ...
    def forward(
        ...
        hidden_states_windows = window_partition(
            shifted_hidden_states, self.window_size, self.host
        ).to(self.device)
