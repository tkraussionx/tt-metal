...
def tt_batch_norm(
    ...
    if mode == "inference":
        ...
        running_mean_left = ttl.tensor.mul(
            ttl.tensor.sub(ones_tt, momentum_tt), running_mean
        )
        mean_reshaped = mean.view(1, 1, 32, 32)
        mean_tilized = tilize_to_list(mean_reshaped)
        mean_tt = ttl.tensor.Tensor(
            mean_tilized,
            [1, 1, H, W],
            ttl.tensor.DataType.BFLOAT16,
            ttl.tensor.Layout.TILE,
        ...
        running_var_left = ttl.tensor.mul(
            ttl.tensor.sub(ones_tt, momentum_tt), running_var
        )
...
def run_batchnorm_forward(device, bn_size):
    ...
    running_mean_tt = ttl.tensor.Tensor(
    ...
    running_var_tt = ttl.tensor.Tensor(
        running_var_tilized,
        [1, 1, 32, bn_size],
        ttl.tensor.DataType.BFLOAT16,
        ttl.tensor.Layout.TILE,
