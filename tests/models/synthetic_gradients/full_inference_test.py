...
### Layer 2
    ...
    # compare
    output_layer1_tt_untilized = untilize(torch.Tensor(output_layer1_tt.cpu().to_torch()).reshape(output_layer1_tt.shape()))
    output_layer1_tt_untilized = output_layer1_tt_untilized[0, 0, 0, :]

    output_layer2_tt_untilized = untilize(torch.Tensor(output_layer2_tt.cpu().to_torch()).reshape(output_layer2_tt.shape()))
