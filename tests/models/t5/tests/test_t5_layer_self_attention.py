...
def run_test_T5LayerSelfAttention_inference(pcc, device, model_name, input_h, input_w):
    ...
    assert (
        does_pass
    ), f"T5LayerSelfAttention output does not meet PCC requirement {pcc}."
...
def test_T5LayerSelfAttention_inference_t5_small(pcc, model_name, input_h, input_w):
...
def test_T5LayerSelfAttention_inference_t5_base(pcc, model_name, input_h, input_w):
    device = tt_lib.device.CreateDevice(tt_lib.device.Arch.GRAYSKULL, 0)
    tt_lib.device.InitializeDevice(device)
    run_test_T5LayerSelfAttention_inference(pcc, device, model_name, input_h, input_w)
    tt_lib.device.CloseDevice(device)
