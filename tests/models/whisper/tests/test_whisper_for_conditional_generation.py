...
from models.models_wip.whisper.tt.whisper_for_conditional_generation import (
...
def _prepare_decoder_input_ids_for_generation(
    ...
    if model_kwargs is not None and "decoder_input_ids" in model_kwargs:
        ...
        decoder_start_token_id = _get_decoder_start_token_id(
            generation_config, decoder_start_token_id, bos_token_id
        )
        if device is None:
            device = device
        return (
...
def _get_decoder_start_token_id(
    generation_config, decoder_start_token_id: int = None, bos_token_id: int = None
) -> int:
    decoder_start_token_id = (
        decoder_start_token_id
    ...
    bos_token_id = (
        bos_token_id if bos_token_id is not None else generation_config.bos_token_id
    )
...
def run_generate(sample, device):
    ...
    hf_reference_model = WhisperForConditionalGeneration.from_pretrained(
    ...
    logger.debug(
        f"input_features {input_features.shape} {input_features}"
    )  # 1, 80, 3000
    ...
    if torch_model:
        ...
        encoder_outputs_last_hidden_state = torch_model_kwargs[
            "encoder_outputs"
        ].last_hidden_state
        ...
        input_ids = pad_input_32(input_ids, hf_reference_model.config.pad_token_id).to(
            torch.long
        )
    ...
    if run_tt_model:
        ...
        # Pad decoder inputs to 32
        tt_input_ids = pad_input_32(
            tt_input_ids, hf_reference_model.config.pad_token_id
        ).to(torch.long)
    ...
    for i in range(32):
        if torch_model:
            ...
            torch_next_tokens_scores = torch_logits_processor(
                input_features, torch_next_token_logits
            )
        ...
        if run_tt_model:
            ...
            if torch_model:
                ...
                does_pass, pcc_message = comp_pcc(
                    torch_next_token_logits, next_token_logits, 0.98
                )
    ...
    if torch_model:
        torch_transcription = processor.batch_decode(
