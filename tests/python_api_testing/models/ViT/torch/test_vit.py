...
def test_vit_inference(fuse_ops):
    # image = imagenet_sample_input; imagenet_sample_input
    ...
    torch_model = transformers.ViTModel.from_pretrained("google/vit-base-patch16-224-in21k")
    dr = dir(torch_model)

    print("start", torch_model.get_head_mask, "end of this")
    print(dr)
    print("get_head_mask" in dr)
    ...
    tt_model = ViTModel(config=torch_model.config, state_dict=state_dict)
    tt_model.eval()
