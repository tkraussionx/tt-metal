...
def test_bloom_generation():
    # input_sentance = "It was a dark and stormy night"
    input_sentance = "Alice stepped through the small door"
    ...
    output_sentance = run_generate(input_sentance, run_tt_model=True, tokenizer=tokenizer, hf_reference_model=hf_reference_model, tt_model=tt_model, device=device)
    output_sentance = run_generate(input_sentance, run_tt_model=False, tokenizer=tokenizer, hf_reference_model=hf_reference_model, tt_model=tt_model, device=device)

    logger.info(f"Decoded output TT: {output_sentance}")
    logger.info(f"Decoded output PT: {output_sentance}")
