...
class TtCodeGenAttention(nn.Module):
    def __init__(self, config: CodeGenConfig(), state_dict, device):
        ...
        s#self.scale_attn = torch.sqrt(torch.tensor(self.head_dim, dtype=torch.float32)).to(torch.get_default_dtype())
    ...
    def _attn(
        ...
        tt_attn_weights = tt_lib.tensor.bmm(query, tt_key_transposed)

        tt_scale_attn_recip = tt_lib.tensor.recip(self.scale_attn)

        tt_attn_weights = tt_lib.tensor.bmm(tt_attn_weights, tt_scale_attn_recip)
        ...
        # Mask heads if we want to
        if head_mask is not None:
            tt_attn_weights = tt_lib.tensor.bmm(attn_weights, head_mask)

        tt_attn_output = tt_lib.tensor.bmm(tt_attn_weights, value)
