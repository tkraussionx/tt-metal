...
class TtCodeGenMLP(torch.nn.Module):
    def __init__(self, base_address, config: CodeGenConfig(), intermediate_size, state_dict, device):
        super().__init__()
        # Get the weights

        embed_dim = config.n_embd

        self.tt_weight_fc_in = state_dict[f"{base_address}.fc_in.weight"]
        self.tt_weight_fc_out = state_dict[f"{base_address}.fc_out.weight"]
        ...
        self.tt_weight_fc_in = torch2tt_tensor(
            self.tt_weight_fc_out, device, tt_layout=tt_lib.tensor.Layout.ROW_MAJOR
        )
        self.tt_weight_fc_out = torch2tt_tensor(
            self.tt_weight_fc_out, device, tt_layout=tt_lib.tensor.Layout.ROW_MAJOR
        )

        #self.tt_weight_fc_in = tt_lib.tensor.transpose(self.tt_weight_fc_in)
        #self.tt_weight_fc_out= tt_lib.tensor.transpose(self.tt_weight_fc_out)
        ...
        self.fc_in = Linear(embed_dim, intermediate_size, self.tt_weight_fc_in, self.tt_bias_fc_in)
        self.fc_out = Linear(intermediate_size, embed_dim, self.tt_weight_fc_out, self.tt_bias_fc_out)


    def forward(self, hidden_states):
        hidden_states = self.fc_in(hidden_states)
        hidden_states = codegen_gelu.new_gelu(hidden_states, self.device)
