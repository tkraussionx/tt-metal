...
def format_tensor(x, target_layout, device, pad_value=0.0):
    ...
    if x.layout() == tt_lib.tensor.Layout.ROW_MAJOR and target_layout == tt_lib.tensor.Layout.TILE:
        x_padded_shape = tt_lib.tensor.AutoFormat.pad_to_tile_shape(x.shape_object(), False, False, True, True)
        return tt_lib.tensor.AutoFormat.format_input_tensor(x, device, x_padded_shape, pad_value, target_layout)
    elif x.layout() == tt_lib.tensor.Layout.TILE and target_layout == tt_lib.tensor.Layout.ROW_MAJOR:
        return tt_lib.tensor.AutoFormat.format_output_tensor(x, x.shape_object().without_padding(), device, target_layout)
    else:
        assert False
...
class ResNet(nn.Module):
    def __init__(
        ...
        #unpadded_avgpool_input_shape = [1, 2048, 7, 7]
        #self.avgpool = fallback_ops.AdaptiveAvgPool2d((1, 1), channels_last=True, unpadded_shape=unpadded_avgpool_input_shape)
