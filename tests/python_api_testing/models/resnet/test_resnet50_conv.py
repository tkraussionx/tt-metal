...
@pytest.mark.parametrize(
    ...
    (
        ...
        (256, 256, 28, 28, 3, 3, 2, 2, 1, 1), # low pcc in resnet50 model with fused bias - 0.96
        #K= 256 C= 256 H= 14 W= 14 R= 3 S= 3 U= 1 V= 1 P_H= 1 P_W= 1
        #(256, 256, 14, 14, 3, 3, 1, 1, 1, 1), # low pcc in resnet50 model with fused bias - 0.87
        #K= 512 C= 512 H= 14 W= 14 R= 3 S= 3 U= 2 V= 2 P_H= 1 P_W= 1
        #(512, 512, 14, 14, 3, 3, 2, 2, 1, 1), # low pcc in resnet50 model with fused bias - 0.94
        #K= 512 C= 512 H= 7 W= 7 R= 3 S= 3 U= 1 V= 1 P_H= 1 P_W= 1
        #(512, 512, 7, 7, 3, 3, 1, 1, 1, 1), # low pcc in resnet50 model with fused bias - 0.88
        #K= 512 C= 512 H= 7 W= 7 R= 3 S= 3 U= 1 V= 1 P_H= 1 P_W= 1
        #(512, 512, 7, 7, 3, 3, 1, 1, 1, 1), # low pcc in resnet50 model with fused bias - 0.97
...
def test_resnet50_conv(use_program_cache, device, K,C,H,W,R,S,stride_h,stride_w,pad_h,pad_w, use_new_matmul):
    for i in range(1):
        ...
        if (is_1x1_conv):
            ...
            conv = TtResnetConv(conv_weight_pyt.reshape(-1).tolist(), conv_params, device, [1, 1], [1, 1], [1, 1], conv_bias_pyt.reshape(-1).tolist())
        else:
        ...
        output_on_device = conv(conv_input_on_device, True)
