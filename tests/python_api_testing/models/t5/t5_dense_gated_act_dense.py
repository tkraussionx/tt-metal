...
class TtT5DenseGatedActDense(nn.Module):
    def __init__(self, config, state_dict, base_address, device):
        ...
        self.wi_0_weights = torch2tt_tensor(state_dict[f"{base_address}.wi_0.weight"], tt_lib.device.GetHost())
        self.wi_1_weights = torch2tt_tensor(state_dict[f"{base_address}.wi_1.weight"], tt_lib.device.GetHost())
        self.wo_weights = torch2tt_tensor(state_dict[f"{base_address}.wo.weight"], tt_lib.device.GetHost())
