...
def test_trocr_vit_attention_inference(pcc, reset_seeds):
    ...
    with torch.no_grad():
        ...
        base_address = f"encoder.encoder.layer.0.attention"

        torch_model = model.encoder.encoder.layer[0].attention

        tt_model = TtViTAttention(
            config=config,
        ...
        if passing:
            logger.info("VitAttention Passed!")
        else:
            logger.warning("VitAttention Failed!")
