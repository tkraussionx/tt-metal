...
def test_trocr_vit_intermediate_inference(pcc, reset_seeds):
    ...
    with torch.no_grad():
        ...
        base_address = f"encoder.encoder.layer.10.intermediate"

        torch_model = model.encoder.encoder.layer[10].intermediate

        tt_model = TtViTIntermediate(
            config=config,
        ...
        if passing:
            logger.info("VitIntermediate Passed!")
        else:
            logger.warning("VitIntermediate Failed!")
