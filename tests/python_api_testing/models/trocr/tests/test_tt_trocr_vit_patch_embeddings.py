...
def test_trocr_vit_patch_embeddings_inference(pcc, reset_seeds):
    ...
    with torch.no_grad():
        ...
        base_address = f"encoder.embeddings.patch_embeddings"

        torch_model = model.encoder.embeddings.patch_embeddings

        tt_model = TtViTPatchEmbeddings(
            config=config,
        ...
        if passing:
            logger.info("VitPatchEmbeddings Passed!")
        else:
            logger.warning("VitPatchEmbeddings Failed!")
