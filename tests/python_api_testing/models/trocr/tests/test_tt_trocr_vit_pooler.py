...
def test_trocr_vit_pooler_inference(pcc, reset_seeds):
    ...
    with torch.no_grad():
        ...
        base_address = f"encoder.pooler"

        torch_model = model.encoder.pooler

        tt_model = TtViTPooler(
            config=config,
        ...
        if passing:
            logger.info("VitPooler Passed!")
        else:
            logger.warning("VitPooler Failed!")
