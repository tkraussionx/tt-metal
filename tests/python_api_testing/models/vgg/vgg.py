...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False, state_dict=None, base_address="features", device=None, host=None) -> nn.Sequential:
    layers: List = []
    in_channels = 3
    ...
    for v in cfg:
        if v == "M":
            ...
            if not batch_norm:
                ...
                if is_conv_supported_on_device(conv2d_params):
                    assert device is not None
                    conv2d_weight = state_dict[f"{base_address}.{ind}.weight"]
                    conv2d_bias = state_dict[f"{base_address}.{ind}.bias"].tolist()
                    conv2d = run_conv_on_device_wrapper(TtConv(conv2d_weight.reshape(-1).tolist(), conv2d_params, device, conv2d_bias), conv2d_params, device, host)
                else:
...
def vgg16(device, host, state_dict, base_address="") -> TtVGG:
    ...
    model = TtVGG(make_layers(cfgs["D"], batch_norm=False, state_dict=state_dict, device=device, host=host), init_weights=False, device=device, host=host, state_dict=state_dict)
    return model
