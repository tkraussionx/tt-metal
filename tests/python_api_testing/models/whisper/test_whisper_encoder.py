...
def run_whisper_encoder(device, for_audio_classification=False, encoder_layers=1):
    ...
    if use_real_inputs:
        ...
        print(f"Input of size {input_features.size()}")
    ...
    with torch.no_grad():
        ...
        print(ttm_output.last_hidden_state.shape())
        ...
        print("Encoder output to torch")
        print(ttm_output_pt.size())
        # else:
        ...
        print(comp_allclose(pytorch_output.last_hidden_state, ttm_output_pt))
        print(pcc_message)
