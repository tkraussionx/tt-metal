...
run_t3000_resnet_tests() {
  ...
  # TODO: Issue #11029 prevents us from running 1/2cqs tests in the same process, so temporarily filter to run them separately
  # Will remove filter once issue is resolved
  WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yaml pytest -n auto models/demos/ttnn_resnet/tests/multi_device/test_ttnn_resnet50_performant.py -k "not 2cqs" ; fail+=$?
  WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yaml pytest -n auto models/demos/ttnn_resnet/tests/multi_device/test_ttnn_resnet50_performant.py -k "2cqs" ; fail+=$?
