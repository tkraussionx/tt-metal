...
Tensor host_function(const Tensor& input_tensor_a, const Tensor& input_tensor_b) {
    auto input_a_buffer = tt::tt_metal::owned_buffer::get_as<uint32_t>(input_tensor_a);
    auto input_b_buffer = tt::tt_metal::owned_buffer::get_as<uint32_t>(input_tensor_b);

    auto output_buffer = tt::tt_metal::owned_buffer::create<uint32_t>(input_tensor_a.volume());

    for (auto index = 0; index < output_buffer.size(); index++) {
        auto value = BinaryFunction{}(input_a_buffer[index], input_b_buffer[index]);
        output_buffer[index] = uint32_t(value);
    }
...
bool run_test(const Shape& shape, const DeviceFunction& device_function, Device* device, Args... args) {
    auto input_tensor_a = tt::numpy::arange<uint32_t>(0, 32 * 32, 1).reshape(1, 1, 32, 32);
    auto input_tensor_b = tt::numpy::arange<uint32_t>(0, 32 * 32, 1).reshape(1, 1, 32, 32);
    ...
    host_output.print();
    device_output.print();
    return tt::numpy::allclose<uint32_t>(host_output, device_output, args...);
}
