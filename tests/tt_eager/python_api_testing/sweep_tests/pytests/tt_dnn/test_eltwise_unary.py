...
@pytest.mark.parametrize("output_mem_config", input_mem_cfgs)
class TestEltwiseUnary:
    ...
    @pytest.mark.parametrize("fn_kind", ["signbit"])
    @pytest.mark.parametrize("fill_val", [1.0])
    def test_run_eltwise_sign_ops(
        ...
        # ]
        datagen_func = [
            generation_funcs.gen_func_with_cast(partial(generation_funcs.gen_rand, low=1, high=10), torch.bfloat16)
        ]
        test_args = generation_funcs.gen_default_dtype_layout_device(input_shapes)[0]
        test_args.update(
            {
