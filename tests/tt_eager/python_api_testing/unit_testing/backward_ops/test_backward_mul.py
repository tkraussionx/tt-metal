...
def test_bw_mul(input_shapes, device):
    in_data_a, input_tensor_a = data_gen_with_range(input_shapes, -101, 100, device, True)
    in_data_b, input_tensor_b = data_gen_with_range(input_shapes, -51, 50, device, True)
...
def test_bw_mul_opt_output(input_shapes, device, are_required_outputs, pass_queue_id):
    ...
    if pass_queue_id:
        tt_output_tensor_on_device = ttnn.mul_bw(
            ...
            optional_input_a_grad=input_a_grad,
            optional_input_b_grad=input_b_grad,
            queue_id=cq_id,
        )
    # else:
    #     tt_output_tensor_on_device = ttnn.mul_bw(
