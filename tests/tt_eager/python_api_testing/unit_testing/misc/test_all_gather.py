...
def is_known_failing_case(input_shape, dim, mem_config, num_devices, num_links, input_dtype, layout):
    if (
        tuple(input_shape) == (4, 1, 33, 256)
        and dim == 0
...
def test_all_gather_on_t3000(
    ...
    if num_links > 1:
        pytest.skip("Skipping multi-link for now")
    ...
    # Specific failing tests skipped for now:'
    if is_known_failing_case(input_shape, dim, mem_config, num_devices, num_links, input_dtype, layout):
        pytest.skip("Skipping known failing cases")
