...
#     [
#         (False, True, True, 128, 8192, 4096, None, (8, 4)),
#     ],
# )
...
def test_llama2_matmul(
    ...
    interleaved_mem_config_in0 = ttl.tensor.MemoryConfig(
        memory_layout=ttl.tensor.TensorMemoryLayout.INTERLEAVED,
        buffer_type=ttl.tensor.BufferType.DRAM,
        num_tiles_per_page=1,
    )
    ...
    interleaved_mem_config_in1 = ttl.tensor.MemoryConfig(
        memory_layout=ttl.tensor.TensorMemoryLayout.INTERLEAVED,
        buffer_type=ttl.tensor.BufferType.DRAM,
        num_tiles_per_page=2,
    )
    interleaved_mem_config_out = ttl.tensor.MemoryConfig(
        memory_layout=ttl.tensor.TensorMemoryLayout.INTERLEAVED,
        buffer_type=ttl.tensor.BufferType.DRAM,
        num_tiles_per_page=1,
    )
    ...
    in0_t = torch2tt_tensor(
        in0, device, tt_memory_config=interleaved_mem_config_in0, tt_dtype=ttl.tensor.DataType.BFLOAT16
    )
    in1_t = torch2tt_tensor(
    ...
    if out_sharded:
        output_t = ttl.tensor.sharded_to_interleaved(output_t, interleaved_mem_config_out)
    pt_out = in0 @ in1 + bias
