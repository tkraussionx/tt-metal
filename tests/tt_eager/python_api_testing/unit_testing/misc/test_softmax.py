...
def test_softmax(device, inplace, dtype, enable_async, num_loops):
...
def test_softmax_with_program_cache(device, use_program_cache, inplace, enable_async, num_loops):
    torch.manual_seed(0)
    device.enable_async(enable_async)
    sm_op = ttl.operations.primary.softmax_in_place if inplace else ttl.tensor.softmax
...
def test_softmax_mix_precision(device, inplace, in_dtype, cb_dtype, enable_async, num_loops):
    torch.manual_seed(0)
    device.enable_async(enable_async)
    sm_op = ttl.operations.primary.softmax_in_place if inplace else ttl.tensor.softmax
...
def test_scale_mask_softmax_inplace(device, in_dtype, in0_mem_config, casual_mask, seq_len, enable_async):
...
def test_scale_mask_softmax(device, in_dtype, in0_mem_config, enable_async):
    torch.manual_seed(0)
    device.enable_async(enable_async)
    fuse_head = 2
