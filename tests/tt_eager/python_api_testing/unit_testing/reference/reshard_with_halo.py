...
class Pooling:
    ...
    def insert_pad(self, in_data: torch.Tensor, pad_val=0x0):
        ...
        data_hw_padded = torch.cat((pad_h, w_padded), 1)
        ## one last pad along h also needed
        pad_h = torch.reshape(pad_h[0], [(w + 2), c])
        data_hw_padded = torch.cat((hw_padded, pad_h))
        return data_hw_padded

    def insert_halo(self, in_data: torch.Tensor):
        ## ...
        return in_data
...
def reshard_with_halo(
    ...
    out_data = pool.insert_halo(in_data_with_pad)
