...
bool test_sharded_loopback(Device *device, Layout layout, const Shape & shape, const ShardSpec & shard_spec, const MemoryConfig & mem_config) {
    bool pass = true;
    //Tensor host_a = tt::numpy::arange<uint32_t>((const int64_t)0, (const int64_t)1, (const Shape) config.get_shape(),
    //                                    (const Layout) Layout::TILE,  (Device *)nullptr, (
    //                                    const MemoryConfig) MemoryConfig{.memory_layout=tt::tt_metal::TensorMemoryLayout::INTERLEAVED});
    Tensor host_a = tt::numpy::random::random(shape);

    if(layout == Layout::TILE)
        host_a  = host_a.to(Layout::TILE);
    std::cout << "Host tensor shape (" <<  shape[0] <<  ", " << shape[1] << ", " << shape[2] << ", " << shape[3] << ")" << std::endl;
