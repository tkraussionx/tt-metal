...
std::tuple<tt_metal::Program, tt_metal::KernelHandle, uint32_t> create_program(
    ...
    uint32_t num_blocks,
    uint32_t num_banks,
    std::vector<CoreRange>all_cores_list,
        ...
        for (int i=0; i < adj_core_phy_new.size(); ++i) {
            ...
            for (int j=0; j < all_worker_cores_log.size(); ++j) {
                ...
                if (core_phy == core_phy_) {
                    all_cores_list_all.push_back(CoreRange(all_worker_cores_log[j]));
                }
            }
        }
        // std::vector<CoreRange> all_cores_list_all = {
        //     CoreRange(CoreCoord(1,3)),
        //     CoreRange(CoreCoord(0,0)),
        //     CoreRange(CoreCoord(0,4)),
        //     CoreRange(CoreCoord(0,5)),
        //     CoreRange(CoreCoord(4,0)),
        //     CoreRange(CoreCoord(5,3)),
        //     CoreRange(CoreCoord(4,1)),
        //     CoreRange(CoreCoord(4,7)),
        //     CoreRange(CoreCoord(4,6)),
        //     CoreRange(CoreCoord(4,2)),
        //     CoreRange(CoreCoord(4,4)),
        //     CoreRange(CoreCoord(4,5))
        // };
        std::vector<CoreRange> all_cores_list;
        std::set<CoreRange> all_cores_set;

        for (int i=bank_start_id; i<bank_start_id+num_banks; i++) {
            all_cores_list.push_back(all_cores_list_all[i]);
            all_cores_set.insert(all_cores_list_all[i]);
        }
