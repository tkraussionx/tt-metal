...
void add_prefetcher_paged_read_cmd(vector<uint32_t>& cmds,
    ...
    log_info(tt::LogTest, "Generating CQ_PREFETCH_CMD_RELAY_PAGED w/ is_dram: {} start_page: {} base_addr: {} page_size: {} pages: {}",
...
// Model a paged write to DRAM by updating dram data with interleaved/paged data, for validation later.
void add_paged_write_data_to_dram_data(Device *device,
    ...
    // FIXME - Missing fix for start_page != 0 like abve function, I think.
    if (start_page != 0) {
        log_warning("{} start_page: {} probably not handled correctly", __FUNCTION__, start_page);
    }
    ...
    for (uint32_t page_idx = 0; page_idx < pages; page_idx++) {
        ...
        log_info(tt::LogTest, "Starting page_idx: {} of pages: {} dram_channel: {} bank_core: {}", page_idx, pages, dram_channel, bank_core.str());

        for (uint32_t j = 0; j  < page_size_words; j++) {

            // log_info(tt::LogTest, "{} - Start setting dram_bank_id: {} bank_offset: {} j: {} of page_size_words: {}", __FUNCTION__, dram_bank_id, bank_offset, j, page_size_words);
            // log_info(tt::LogTest, "worker data: {}", worker_data.at(bank_core).at(dram_bank_id).data);

            // KCM FIXME - Is this worker data lookup correct?
            dram_data_map.at(dram_bank_id)[bank_offset + j] = worker_data.at(bank_core).at(dram_bank_id).data[bank_offset + j];

            log_info(tt::LogTest, "{} - Done setting dram_bank_id: {} bank_offset: {} j: {} data: 0x{:x}",
                __FUNCTION__, dram_bank_id, bank_offset, j, dram_data_map.at(dram_bank_id)[bank_offset + j]);
...
void gen_dram_read_cmd(Device *device,
    ...
    // Generates the below cmds
    // CQ_PREFETCH_CMD_RELAY_INLINE_NOFLUSH
    // CQ_DISPATCH_CMD_WRITE (w/ a size in bytes) without any payload.
    // CQ_PREFETCH_CMD_RELAY_PAGED - read paged DRAM and relay the data to dispatcher kernel.
    // Then, update worker data struct for appropriate core based on DRAM modelled data, for later checking.


    // Note - No data is being written to DRAM here. Only reading from DRAM and writing to worker core.
    // KCM TODO - Add Paged Write Cmd Here with some data (how)
    // KCM TODO - Checking (new code or somehow leverage validate_results checking)
    ...
    log_info(tt::LogTest, "Starting {} with worker_core: {} dst_addr: 0x{:x} start_page: {} base_addr: {} page_size: {} pages: {}. worker_data_size: {}",
...
void gen_dram_write_cmd(Device *device,
    ...
    vector<uint32_t> dispatch_cmds;
    const bool is_dram = true;
    log_info(tt::LogTest, "Starting {} with start_page: {} base_addr: {} page_size: {} pages: {}", __FUNCTION__, start_page, base_addr, page_size, pages);
...
void gen_paged_read_dram_test(Device *device,
    ...
    // Waits until enough data read back into worker. Arbitrarily picks the first worker.
...
// End-To-End Paged/Interleaved Write + Read Test w/ comparison.
void gen_paged_write_read_dram_test(Device *device,
    ...
    bool do_single_loop = tt::parse_env("DO_SINGLE_LOOP", true);

    // FIXME - Try it with different starting page.

    // FIXME - This isn't a good measure to quit, I think.
    while (worker_data_size(worker_data) * sizeof(uint32_t) < WORKER_DATA_SIZE) {
        uint32_t start_page = 0;
        uint32_t base_addr = 0;

        log_info(tt::LogTest, "{} - XXXX Starting loop with worker_data_size: {} less than WORKER_DATA_SIZE: {}", __FUNCTION__, worker_data_size(worker_data) * sizeof(uint32_t), WORKER_DATA_SIZE);

        // Goal here - Write and Read are for the same size and target.
        // FIXME - What is the address being read from? Base address to paged read cmd is what controls it.

        // Generate a paged DRAM Write command (test data written to interleaved DRAM)
        gen_dram_write_cmd(device, prefetch_cmds, cmd_sizes, dram_data_map, worker_data,
                          start_page, base_addr, dram_page_size_g, dram_pages_to_read_g);

        // Generate a paged DRAM Read command and write to device L1.
        gen_dram_read_cmd(device, prefetch_cmds, cmd_sizes, dram_data_map, worker_data, worker_core, dst_addr,
                          start_page, base_addr, dram_page_size_g, dram_pages_to_read_g);

        bytes_of_data_g += dram_page_size_g * dram_pages_to_read_g;

        log_info(tt::LogTest, "{} - XXXX Generated dram read command. worker_data_size: {} start_page: {} base_addr: {} WORKER_DATA_SIZE: {} dram_pages_to_read_g: {} dram_page_size_g: {} => bytes_of_data_g: {}",
            __FUNCTION__, worker_data_size(worker_data) * sizeof(uint32_t), start_page, base_addr, WORKER_DATA_SIZE, dram_pages_to_read_g, dram_page_size_g, bytes_of_data_g);

        if (do_single_loop) {
            log_info(tt::LogTest, "Exiting {} after single loop.", __FUNCTION__);
            break;
        }
...
void initialize_dram_banks(Device *device)
{
    ...
    auto bank_size = DRAM_DATA_SIZE_WORDS * sizeof(uint32_t); // device->bank_size(BufferType::DRAM); // Or can hardcode to subset like 16MB.
