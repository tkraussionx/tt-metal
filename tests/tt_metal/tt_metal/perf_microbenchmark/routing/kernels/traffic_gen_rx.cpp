...
void kernel_main() {
    ...
    input_queue->wait_for_src_ready(timeout_cycles);
    ...
    while (num_words_checked < total_data_words) {
        ...
        if (!disable_data_check) {
            ...
            if (!check_packet_data(reinterpret_cast<tt_l1_ptr uint32_t*>(input_queue->get_queue_rptr_cleared_addr_bytes()),
                ...
                check_failed = true;
                debug_log_index(5, 0xee000003);
                debug_log_index(6, words_before_wrap);
                debug_log_index(7, words_after_wrap);
                break;
            }
