...
static_assert((max_packet_size_words & (max_packet_size_words - 1)) == 0, "max_packet_size_words must be a power of 2");
...
void kernel_main() {
    ...
    while (true) {
        ...
        uint32_t words_in_input_queue = input_queue_ptr->get_queue_data_num_words_available_to_send();
        if (words_in_input_queue > 0) {
    ...
    debug_log_index(7, total_data_words>>32);
    debug_log_index(8, total_data_words);
    debug_log_index(9, cycles_elapsed>>32);
    debug_log_index(10, cycles_elapsed);
