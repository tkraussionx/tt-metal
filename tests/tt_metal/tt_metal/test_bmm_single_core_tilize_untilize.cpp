...
Tensor test_bmm_single_core_tilize_untilize(const Tensor &in0,
                                            const Tensor &in1,
    ...
    if (untilize_out) {
        ...
        // auto dram_dst_noc_xy = out_dram_noc_xy;
        // writer_rt_args = {
        //     out_dram_addr,
        //     (std::uint32_t)dram_dst_noc_xy.x,
        //     (std::uint32_t)dram_dst_noc_xy.y,
        //     (std::uint32_t)out_subblock_height_ntiles, // num tiles per sub block m
        //     (std::uint32_t)out_subblock_width_ntiles, // num tiles per sub block n
        //     (std::uint32_t)in0_height_ntiles / out_subblock_height_ntiles, // num sub blocks m
        //     (std::uint32_t)in1_width_ntiles / out_subblock_width_ntiles, // num sub blocks n
        //     (std::uint32_t)out_subblock_width_ntiles * tile_size_bytes * (in1_width_ntiles / out_subblock_width_ntiles), // bytes offset to next row within sub-block
        //     (std::uint32_t)out_subblock_height_ntiles * out_subblock_width_ntiles * tile_size_bytes * (in1_width_ntiles / out_subblock_width_ntiles), // bytes offset to next row of sub-blocks
        //     (std::uint32_t)out_subblock_width_ntiles * tile_size_bytes
...
bool test_bmm_single_core(const tt::ARCH& arch, bool activations_rm, bool output_rm) {
    // setup device
    constexpr uint32_t pcie_slot = 0;
    tt_metal::Device *device = tt_metal::CreateDevice(arch, pcie_slot);
    tt_metal::InitializeDevice(device);

    // construct inputs on device
    constexpr uint32_t nbatch = 1;
    constexpr uint32_t nchannel = 1;
    constexpr uint32_t in0_ntiles_w = 1;    //4;
    constexpr uint32_t in0_ntiles_h = 1;    //8;
    constexpr uint32_t in1_ntiles_w = 1;    //4;
    constexpr uint32_t in1_ntiles_h = in0_ntiles_w;

    const Shape in0_shape {nbatch, nchannel, in0_ntiles_h * constants::TILE_HEIGHT, in0_ntiles_w * constants::TILE_WIDTH};
    const Shape in1_shape {nbatch, nchannel, in0_ntiles_w * constants::TILE_HEIGHT, in1_ntiles_w * constants::TILE_WIDTH};
    const Shape out_shape {nbatch, nchannel, in0_ntiles_h * constants::TILE_HEIGHT, in1_ntiles_w * constants::TILE_WIDTH};

    // /////////
    // Tensor test = Tensor(in0_shape, Initialize::RANDOM, DataType::BFLOAT16, Layout::ROW_MAJOR);
    // Tensor test_tm = test.to(Layout::TILE);
    // // std::vector<bfloat16> *test_data = reinterpret_cast<std::vector<bfloat16>*>(test.data_ptr());
    // // std::vector<bfloat16> *test_data_tm = reinterpret_cast<std::vector<bfloat16>*>(test_tm.data_ptr());
    // std::cout << "ROW MAJOR " << (uint) test.layout() << std::endl;
    // tensor_impl::print<bfloat16>(test, test.layout(), false);
    // test.pretty_print();
    // std::cout << "TILE " << (uint) test_tm.layout() << std::endl;
    // tensor_impl::print<bfloat16>(test_tm, test_tm.layout(), false);
    // test_tm.pretty_print();
    // return true;
    // /////////

    Tensor in0_rm = Tensor(in0_shape, Initialize::RANDOM, DataType::BFLOAT16, Layout::ROW_MAJOR);
    tensor_impl::print<bfloat16>(in0_rm, in0_rm.layout(), false);

    Tensor in0 = in0_rm.to(Layout::TILE).to(device);
    // Tensor in0 = Tensor(in0_shape, Initialize::RANDOM, DataType::BFLOAT16, activations_rm ? Layout::ROW_MAJOR : Layout::TILE, device);
    // Tensor in0 = tt_metal::Tensor(in0_shape, Initialize::ZEROS, DataType::BFLOAT16, activations_rm ? Layout::ROW_MAJOR : Layout::TILE, device);

    // Tensor in1 = Tensor(in1_shape, Initialize::RANDOM, DataType::BFLOAT16, Layout::TILE, device);
    // Tensor in1 = tt_metal::Tensor(in1_shape, Initialize::ZEROS, DataType::BFLOAT16, Layout::TILE, device);
    auto identity = create_identity_matrix(in1_shape[2], in1_shape[3], std::min(in1_shape[2], in1_shape[3]));
    Tensor in1 = Tensor(identity, in1_shape, DataType::BFLOAT16, Layout::ROW_MAJOR).to(Layout::TILE).to(device);
    tensor_impl::print<bfloat16>(in1, in1.layout(), false);

    // Tensor decomposition (blocking) parameters
    // uint32_t out_subblock_ntiles_h = 4, out_subblock_ntiles_w = 2;
    uint32_t out_subblock_ntiles_h = 1, out_subblock_ntiles_w = 1;
    uint32_t in0_block_ntiles_h = in0_ntiles_h;
    uint32_t in0_block_ntiles_w = in0_ntiles_w;
    uint32_t in1_block_ntiles_w = in1_ntiles_w;
    uint32_t in0_nblocks_h = in0_ntiles_h / in0_block_ntiles_h;
    uint32_t in0_nblocks_w = in0_ntiles_w / in0_block_ntiles_w;
    uint32_t in1_nblocks_w = in1_ntiles_w / in1_block_ntiles_w;

    // call the test function
    Tensor out = test_bmm_single_core_tilize_untilize(in0,
                                                      in1,
                                                      in0_nblocks_h,
                                                      in0_nblocks_w,
                                                      in1_nblocks_w,
                                                      ...
                                                      activations_rm,
                                                      output_rm);
    Tensor out_host = out.to(GetHost());    //.to(Layout::ROW_MAJOR);
    Tensor out_host_rm = out_host.to(Layout::ROW_MAJOR);
    Tensor in0_host = in0.to(GetHost());    //.to(Layout::ROW_MAJOR);
    Tensor in1_host = in1.to(GetHost());    //.to(Layout::ROW_MAJOR);

    // return true;

    // std::vector<bfloat16> *in0_data = reinterpret_cast<std::vector<bfloat16>*>(in0_host.data_ptr());
    // std::vector<bfloat16> *in1_data = reinterpret_cast<std::vector<bfloat16>*>(in1_host.data_ptr());
    // std::vector<bfloat16> *out_data = reinterpret_cast<std::vector<bfloat16>*>(out_host.data_ptr());

    std::cout << std::endl << "IN0:----------------------------------- " << (uint) in0_host.layout() << std::endl << std::endl;
    // tensor_impl::print_row_major_data(*in0_data, in0_shape, DataType::BFLOAT16);
    tensor_impl::print<bfloat16>(in0_host, in0_host.layout(), false);

    // std::cout << std::endl << "IN1:-----------------------------------" << std::endl << std::endl;;
    // tensor_impl::print_row_major_data(*in1_data, in1_shape, DataType::BFLOAT16);

    std::cout << std::endl << "OUT:----------------------------------- " << (uint) out_host.layout() << std::endl << std::endl;
    // tensor_impl::print_row_major_data(*out_data, out_shape, DataType::BFLOAT16);
    tensor_impl::print<bfloat16>(out_host, out_host.layout(), false);
    std::cout << std::endl << "OUT:----------------------------------- " << (uint) out_host_rm.layout() << std::endl << std::endl;
    tensor_impl::print<bfloat16>(out_host_rm, out_host_rm.layout(), false);

    bool is_equal = tensor_impl::compare<bfloat16>(out_host, in0_host);
...
int main(int argc, char **argv) {
    ...
    bool pass = true;
    pass &= test_bmm_single_core(arch, false, false);
    // pass &= test_bmm_single_core(arch, true, false);
    // pass &= test_bmm_single_core(arch, true, true);
    // pass &= test_bmm_single_core(arch, false, true);
