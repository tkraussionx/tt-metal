...
void kernel_main() {
    ...
    DPRINT << "RR: num_buffers_per_channel: " << (uint32_t)num_buffers_per_channel <<
                "\n\tnum_pages_per_read_chunk: " << (uint32_t)num_pages_per_read_chunk <<
                "\n\ttotal_pages_to_read: " << (uint32_t)total_pages_to_read <<
                "\n\treceiver_read_sem_addr: " << (uint32_t)receiver_read_sem_addr <<
    ...
    DPRINT << "RECEIVER WAIT (FIRST)\n";
    // reader.wait_for_payload_available();
    // noc_semaphore_wait(receiver_read_semaphore_addr_ptr
