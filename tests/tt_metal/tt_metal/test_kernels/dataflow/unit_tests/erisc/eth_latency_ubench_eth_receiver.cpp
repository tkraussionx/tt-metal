...
FORCE_INLINE void roundtrip_ping(
    ...
    if (is_ring_start) {
        ...
        for (uint32_t i = 0; i < max_concurrent_samples; i++) {
            ...
            noc_async_write(buffer_addr, send_buffer_noc_addr, page_size);
            noc_semaphore_inc(send_sem_noc_addr, 1);
            // eth_receiver_channel_ack(eth_channel_syncs[i], eth_channel_sync_ack_addr);
        }
...
void kernel_main() {
    ...
    const uint32_t eth_noc_x = get_arg_val<uint32_t>(arg_idx++);g
    ...
    *start_semaphore = 0;
    ...
    DPRINT << "Notified by worker... Ready to go\n";
    ...
    DPRINT << "Flushed first roundtrip ping\n";
    {
