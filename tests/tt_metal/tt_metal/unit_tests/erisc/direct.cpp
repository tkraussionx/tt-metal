...
bool send_over_eth(
  ...
  std::cout << " running test, sender chip " << sender_device->id() << " core " << sender_core.x << " " << sender_core.y << " receiver chip " << receiver_device->id() << " core " << receiver_core.x << " " << receiver_core.y << std::endl;
  ...
  ll_api::memory binary_mem_send = llrt::get_risc_binary("erisc_app_direct_transfer.hex", sender_device->id(), true);
  ll_api::memory binary_mem_receive = llrt::get_risc_binary("erisc_app_direct_transfer.hex", receiver_device->id(), true);
