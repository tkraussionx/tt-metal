...
TEST_F(CommandQueueSingleCardFixture, TestPageSizeTooLarge) {
    ...
    for (Device *device : devices_) {
        EXPECT_ANY_THROW(local_test_functions::test_EnqueueWriteBuffer_and_EnqueueReadBuffer(device, device->command_queue(), config));
    }
}
...
TEST_F(CommandQueueSingleCardFixture, StressWrapTest) {
    ...
    for (Device *device : devices_) {
        EXPECT_TRUE(
            local_test_functions::stress_test_EnqueueWriteBuffer_and_EnqueueReadBuffer_wrap(device, device->command_queue(), config));
    }
}
