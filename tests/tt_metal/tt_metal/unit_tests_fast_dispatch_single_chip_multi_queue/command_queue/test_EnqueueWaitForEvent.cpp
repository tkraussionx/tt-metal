...
bool test_events_write_program_read(EventsTestConfig &config, Device* device) {
    ...
    if (config.always_insert_events || config.cq_idx_write != config.cq_idx_prog) {
    ...
    if (config.always_insert_events || config.cq_idx_read != config.cq_idx_prog) {
    ...
    log_info(tt::LogTest, "KCM Checking local_pass: {} input_vec: {} result_vec: {}", input_vec == result_vec, input_vec, result_vec);
...
//  Always | FATAL    | Event ID is expected to increase. Wrapping not supported for sync. Completed event 4 but last recorded completed event is 6
TEST_F(MultiCommandQueueSingleDeviceFixture, TestEventsWriteProgram_NB_CQ0_Passthrough_Events) {
    EventsTestConfig config = {.blocking = false, .cq_mode = CommandQueue::CommandQueueMode::PASSTHROUGH, .cq_idx_write = 0, .cq_idx_read = 0, .cq_idx_prog = 0, .num_tiles_or_pages = 1, .always_insert_events = true};
    EXPECT_TRUE(test_events_write_program_read(config, this->device_));
}
