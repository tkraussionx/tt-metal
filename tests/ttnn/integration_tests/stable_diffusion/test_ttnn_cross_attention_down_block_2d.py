...
def test_cross_attn_down_block_2d_256x256(device, model_name, N, C, H, W, index):
    torch.manual_seed(0)
    ...
    ttnn_output = tt_to_torch_tensor(ttnn_output)
...
def test_cross_attn_down_block_2d_512x512(device, model_name, N, C, H, W, index):
    torch.manual_seed(0)
