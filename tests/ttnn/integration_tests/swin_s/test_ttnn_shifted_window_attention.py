...
def test_shifted_window_attention(device, reset_seeds):
    ...
    ds_state_dict = {k: v for k, v in state_dict.items() if (k.startswith("features.1.0.attn."))}

    torch_model = ShiftedWindowAttention(96, [7, 7], [0, 0], 3)
    ...
    torch_input_tensor = torch.randn(1, 8, 8, 96)
    ...
    ttnn_model = TtShiftedWindowAttention(parameters, device, 96, [7, 7], [0, 0], 3)
    ...
    assert_with_pcc(torch_output, tt_output, 0.99)  # pcc = 0.6160427932082001
