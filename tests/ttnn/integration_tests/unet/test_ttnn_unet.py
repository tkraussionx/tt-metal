...
def create_custom_preprocessor(device):
    def custom_preprocessor(model, name, ttnn_module_args):
        ...
        if isinstance(model, UNet):
            ...
            # conv1_weight=model.encoder3[0].weight
            # conv1_bias=model.encoder3[0].bias
...
def test_unet(device, reset_seeds):
    ...
    ttnn_model = TtUnet(device, parameters, new_state_dict, torch_model)
    ...
    output_tensor = ttnn_model(device, torch_input_tensor)
    ...
    print("comp", comp_allclose_and_pcc(torch_output_tensor, output_tensor, pcc=0.99))
    assert_with_pcc(torch_output_tensor, output_tensor, pcc=0.99)  # pcc = 0.5216060229849134
