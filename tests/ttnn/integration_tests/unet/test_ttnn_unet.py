...
def test_unet(device, reset_seeds):
    ...
    torch_decoder1_output, torch_conv_output, torch_output_tensor = torch_model(torch_input_tensor)
    ...
    decoder1_output, conv_output, output_tensor = ttnn_model(device, input_tensor)

    #
    # Tensor Postprocessing
    #

    decoder1_output = ttnn.to_torch(decoder1_output)
    decoder1_output = decoder1_output.reshape(1, 480, 640, 32)
    decoder1_output = torch.permute(decoder1_output, (0, 3, 1, 2))
    decoder1_output = decoder1_output.to(torch_input_tensor.dtype)
    passing, pcc_msg = check_with_pcc_without_tensor_printout(
        torch_decoder1_output, decoder1_output, pcc=0.99
    )  # pcc = 0.9808506765509414
    logger.info(pcc_msg)

    conv_output = ttnn.to_torch(conv_output)
    conv_output = conv_output.reshape(1, 480, 640, 1)
    conv_output = torch.permute(conv_output, (0, 3, 1, 2))
    conv_output = conv_output.to(torch_input_tensor.dtype)
    passing, pcc_msg = check_with_pcc_without_tensor_printout(
        torch_conv_output, conv_output, pcc=0.99
    )  # pcc = 0.8468204189183342
    logger.info(pcc_msg)
    ...
    assert_with_pcc(torch_output_tensor, output_tensor, pcc=0.99)  # pcc = 0.8066952002315315
