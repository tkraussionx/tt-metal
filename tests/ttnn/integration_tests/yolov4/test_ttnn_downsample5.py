...
def test_downsample5(reset_seeds):
    ...
    torch_model.eval()
    torch_input_tensor = torch.randn(1, 512, 20, 20)  # Batch size of 1, 128 input channels, 160x160 height and width
    ...
    ttnn_model = TtDownSample5(parameters)
    ...
    ttnn.tracer.visualize(output_tensor, file_name="downsample5.svg")
    # output_tensor = ttnn_model(device, input_tensor)
