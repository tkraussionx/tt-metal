...
def run_conv(
    ...
    if not is_grayskull():
        compute_kernel_config = ttnn.WormholeComputeKernelConfig(
            math_fidelity=math_fidelity, math_approx_mode=True, fp32_dest_acc_en=fp32_accum, packer_l1_acc=packer_l1_acc
        )
...
@pytest.mark.parametrize("batch_size", [8, 16, 20])
...
@pytest.mark.parametrize("weights_dtype", [ttnn.float32, ttnn.bfloat16])
@pytest.mark.parametrize("activations_dtype", [ttnn.float32, ttnn.bfloat16])
...
@pytest.mark.parametrize("weights_dtype", [ttnn.bfloat8_b])
@pytest.mark.parametrize("activations_dtype", [ttnn.bfloat8_b, ttnn.bfloat16])
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
...
@pytest.mark.parametrize(
    "batch_size, output_channels, input_channels, input_height, input_width, filter_height, filter_width, stride_h, stride_w, pad_h, pad_w, config_override, xfail",
    (
        (1, 32, 3, 512, 512, 7, 7, 4, 4, 3, 3, None, False),
        (1, 32, 32, 128, 128, 8, 8, 8, 8, 0, 0, None, False),
        # (1,64,32,128,128,3,3,2,2,1,1,None,False), Passing
        (1, 64, 64, 64, 64, 4, 4, 4, 4, 0, 0, None, False),
        # (1,160,64,64,64,3,3,2,2,1,1,None,False),  Passing
        # (1,160,160,32,32,2,2,2,2,0,0,None,False), Passing
        # (1,256,160,32,32,3,3,2,2,1,1,None,False), Passing
    ),
)
@pytest.mark.parametrize("use_1d_systolic_array", [True])
def test_conv_segformer(
    device,
    use_program_cache,
    use_1d_systolic_array,
    batch_size,
...
@pytest.mark.parametrize("device_params", [{"l1_small_size": 24576}], indirect=True)
@pytest.mark.parametrize(
    "batch_size, output_channels, input_channels, input_height, input_width, filter_height, filter_width, stride_h, stride_w, pad_h, pad_w, use_1d_systolic_array,groups",
    (
        (1, 128, 128, 128, 128, 3, 3, 1, 1, 1, 1, True, 128),
        (1, 256, 256, 64, 64, 3, 3, 1, 1, 1, 1, True, 256),
        (1, 640, 640, 32, 32, 3, 3, 1, 1, 1, 1, True, 640),
        (1, 1024, 1024, 16, 16, 3, 3, 1, 1, 1, 1, True, 1024),
    ),
)
@pytest.mark.parametrize("weights_dtype", [ttnn.bfloat16, ttnn.bfloat8_b])
@pytest.mark.parametrize("activations_dtype", [ttnn.bfloat16, ttnn.bfloat8_b])
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_segformer_conv_gs_groups(
    device,
    use_program_cache,
    math_fidelity,
    activations_dtype,
