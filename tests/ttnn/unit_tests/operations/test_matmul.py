...
# @pytest.mark.parametrize("core_grid", [None, ttnn.CoreGrid(8, 8)])
def test_sd_matmul(device, batch_size, channel_a, channel_b, m_size, k_size, n_size, has_bias):  # , core_grid):
    torch.manual_seed(0)
    ...
    torch_output_tensor = torch_input_tensor_a @ torch_input_tensor_b
    if has_bias:
        torch_input_tensor_c = torch.randn((1, 1, m_size, n_size), dtype=torch.bfloat16)
        torch_output_tensor = torch_output_tensor + torch_input_tensor_c
        if max(channel_a, channel_b) > 1:
            torch_input_tensor_c = torch.repeat_interleave(torch_input_tensor_c, max(channel_a, channel_b), dim=1)

    input_tensor_a = ttnn.from_torch(torch_input_tensor_a, layout=ttnn.TILE_LAYOUT, device=device)
    input_tensor_b = ttnn.from_torch(torch_input_tensor_b, layout=ttnn.TILE_LAYOUT, device=device)
    input_tensor_c = ttnn.from_torch(torch_input_tensor_c, layout=ttnn.TILE_LAYOUT, device=device) if has_bias else None

    output_tensor = ttnn.ttnn.operations.matmul._matmul(
        "ttnn.linear",
        input_tensor_a,
        input_tensor_b,
        bias=input_tensor_c,
