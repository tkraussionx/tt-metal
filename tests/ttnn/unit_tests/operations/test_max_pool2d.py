...
@pytest.mark.parametrize(
    "act_shape, kernel_size, padding, stride, dilation, nblocks, dtype",
    (([1, 256, 60, 80], (2, 2), (0, 0), (2, 2), (1, 1), 1, [ttnn.bfloat16]),),
)
def test_run_max_pool_unet(
    act_shape,
    kernel_size,
    padding,
    stride,
    ...
    act = torch.load("/home/user/harini/hrnet18/tt-metal/input_pool4.pt")
    ...
    if dtype == ttnn.bfloat8_b:
        ...
        ttact = ttnn.from_torch(act_reshaped, device=device, dtype=dtype)
    ttact_d = max_pool.copy_input_to_device(ttact)
    ...
    out_pytorch = out_pytorch.reshape(golden_shape[0], golden_shape[2], golden_shape[3], golden_shape[1])
    out_pytorch = torch.permute(out_pytorch, (0, 3, 1, 2))  ## N, C, H, W
    assert_with_pcc(out_pytorch, golden_pytorch, pcc=1)
    ...
    # allclose = torch.allclose(out_pytorch.float(), golden_pytorch.float(), atol=atol)
    # isclose = torch.all(torch.isclose(out_pytorch.float(), golden_pytorch.float(), atol=atol))
    # isequal = torch.equal(out_pytorch, golden_pytorch)
