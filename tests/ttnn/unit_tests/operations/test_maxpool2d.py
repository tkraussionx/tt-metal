...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            [1, 64, 1018, 122],  # Fails with with valid_page_size error
            [1, 64, 506, 58],  # Fails with with valid_page_size error
        )
    ),
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            [1, 64, 1019, 123],  # Fails with with valid_page_size error
            [1, 64, 506, 58],  # Fails with with valid_page_size error
            [1, 64, 250, 26],  # Fails with with valid_page_size error
        )
    ),
)
@pytest.mark.parametrize(
