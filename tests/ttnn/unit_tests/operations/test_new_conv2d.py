...
@pytest.mark.parametrize(
    "batch_size, output_channels, input_channels, input_height, input_width, filter_height, filter_width, stride_h, stride_w, pad_h, pad_w, use_1d_systolic_array, config_override, use_shallow_conv_variant, groups",
    (
        (1, 255, 256, 40, 40, 1, 1, 1, 1, 0, 0, True, None, False, 1),
        (1, 255, 512, 20, 20, 1, 1, 1, 1, 0, 0, True, None, False, 1),
        (1, 255, 1024, 10, 10, 1, 1, 1, 1, 0, 0, True, None, False, 1),
    ),
)
@pytest.mark.parametrize(
    "weights_dtype",
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
@pytest.mark.parametrize("output_layout", [ttnn.TILE_LAYOUT])
def test_yolov4_out_channels_255(
    device,
    use_program_cache,
    math_fidelity,
    activations_dtype,
