...
@pytest.mark.parametrize(
    ...
    (
        ...
        (1, 64, 64, 1021, 125, 4, 4, 1, 1, 0, 0, 1, 1, 1, "relu"),  # Fails with OOM issue
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_model_net_encoder_4094x510_maxpooling_True(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_model_net_encoder_2047x255_maxpooling_True(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
...
@pytest.mark.parametrize(
    ...
    (
        (1, 64, 10, 4094, 510, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", {"act_block_h": 32}),  # Fails with OOM issue
        (1, 64, 64, 2046, 254, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", {"act_block_h": 32}),  # Fails with OOM issue
        # (1, 64, 64, 1022, 126, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", {"act_block_h": 32}),  # Passed
        (1, 64, 64, 510, 62, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", None),  # Fails with OOM issue
    ),
)
@pytest.mark.parametrize(
    "use_1d_systolic_array",
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_model_net_encoder_4094x510_maxpooling_False(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
...
@pytest.mark.parametrize(
    "batch_size, output_channels, input_channels, input_height, input_width, filter_height, filter_width, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w, groups, activation, config_override",
    (
        (1, 64, 10, 2047, 255, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", {"act_block_h": 32}),  # Fails with OOM issue
        # (1, 64, 64, 1022, 126, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", {"act_block_h": 32}),  # Passed
        (1, 64, 64, 510, 62, 4, 4, 2, 2, 0, 0, 1, 1, 1, "relu", None),  # Fails with OOM issue
...
@pytest.mark.parametrize("math_fidelity", [ttnn.MathFidelity.LoFi])
def test_model_net_encoder_2047x255_maxpooling_False(
    device,
    math_fidelity,
    activations_dtype,
    weights_dtype,
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (1, 1, 2, 4034, 450, 1, 1, 1, 1, 0, 0, 1, 1, 1, "None"),  # Fails with OOM issue
        # Input resolution: 2047x255
        (1, 1, 2, 1986, 194, 1, 1, 1, 1, 0, 0, 1, 1, 1, "None"),  # Fails with OOM issue
    ),
)
@pytest.mark.parametrize(
    "config_override",
