...
@pytest.mark.parametrize(
    "input_shape, shard_strategy",
    [
        ([1, 32, 640, 640], ttnn.ShardStrategy.HEIGHT),  # Failed with OOM Error
        ([1, 32, 640, 640], ttnn.ShardStrategy.BLOCK),  # Failed with OOM Error
        ([1, 64, 320, 320], ttnn.ShardStrategy.BLOCK),  # Failed with OOM error
    ],
)
def test_failing_silu_yolov7(device, input_shape, shard_strategy):
    ## input shape is N C H W
    batch_size, num_channels, height, width = input_shape
    torch.manual_seed(0)
    input = torch.rand(input_shape, dtype=torch.bfloat16)
