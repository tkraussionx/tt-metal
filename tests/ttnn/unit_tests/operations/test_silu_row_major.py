...
@pytest.mark.parametrize(
    "input_shape",
    [
        [1, 32, 640, 640],  # Failed with OOM Error
        # [1, 64, 320, 320], # Passed for Height Sharded and Failed for Block Sharded: OOM error
        # [1, 128, 160, 160], # Passed
        # [1, 64, 160, 160], # Passed
        # [1, 256, 160, 160], # Passed
        # [1, 128, 80, 80], # Passed
        # [1, 512, 80, 80], # Passed
        # [1, 256, 40, 40], # Passed
        # [1, 256, 80, 80], # Passed
        ...
        # [1, 512, 20, 20], # Passed
        # [1, 512, 40, 40], # Passed
        # [1, 256, 20, 20], # Passed for Height Sharded and Skipped for Block Sharded: Shard sizes that are not multiples of 1024 are not supported.
        # [1, 1024, 20, 20], # Passed
        # [1, 128, 40, 40], # Passed
        # [1, 64, 80, 80], # Passed
    ],
...
@pytest.mark.parametrize("shard_strategy", [ttnn.ShardStrategy.HEIGHT, ttnn.ShardStrategy.BLOCK])
def test_silu_yolov7(device, input_shape, shard_strategy):
    ## input shape is N C H W
    batch_size, num_channels, height, width = input_shape
    torch.manual_seed(0)
    input = torch.rand(input_shape, dtype=torch.bfloat16)
