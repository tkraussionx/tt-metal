...
def test_multi_device_multi_op_trace(pcie_device_mesh, shape, use_all_gather, enable_async):
    if use_all_gather:
    ...
    for i in range(50):
        ...
        if blocking:
            for device_id in pcie_device_mesh.get_device_ids():
                ttnn.synchronize_device(pcie_device_mesh.get_device(device_id))
        logger.info("Execute Trace 0")
        # Execute trace
        ttnn.execute_multi_device_trace(pcie_device_mesh, tid, 0, blocking)
        logger.info("Execute Trace 1")
        ttnn.execute_multi_device_trace(pcie_device_mesh, tid_1, 0, blocking)
        if use_all_gather:
