...
NUM_TRACE_LOOPS = int(os.getenv("NUM_TRACE_LOOPS", 100))
...
def test_multi_device_single_trace(t3k_device_mesh, shape, use_all_gather, enable_async):
    ...
    for i in range(NUM_TRACE_LOOPS):
        event = create_event(t3k_device_mesh)
        # Create torch inputs
        ...
        record_event(t3k_device_mesh, 1, event)
        logger.info("Execute Trace")
        # Execute trace
        wait_for_event(t3k_device_mesh, 0, event)
...
def test_multi_device_multi_trace(t3k_device_mesh, shape, use_all_gather, enable_async):
    ...
    for i in range(num_trace_loops):
        ...
        ttnn.copy_host_to_device_tensor(ttnn_input_tensor_0, input_0_dev, cq_id=1)
        ttnn.copy_host_to_device_tensor(ttnn_input_tensor_1, input_1_dev, cq_id=1)
        ttnn.copy_host_to_device_tensor(ttnn_weight, weight_dev, cq_id=1)
        record_event(t3k_device_mesh, 1, event)
        wait_for_event(t3k_device_mesh, 0, event)
        # Execute trace
