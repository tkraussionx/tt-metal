...
void dump_multi_device_host_storage(ofstream& output_stream, const MultiDeviceHostStorage& storage) {
    ...
    output_stream.write(reinterpret_cast<const char*>(&storage.strategy), sizeof(DistributionStrategy)); // change this so same for single-device
...
MultiDeviceHostStorage load_multi_device_host_storage(ifstream& input_stream, DeviceMesh* device_mesh) {
    ...
    input_stream.read(reinterpret_cast<char*>(&strategy), sizeof(DistributionStrategy));
