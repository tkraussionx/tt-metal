...
Tensor create_multi_device_tensor(const std::vector<Tensor>& tensors, StorageType storage_type, const DistributionStrategy& strategy) {
    if (tensors.empty()) {
...
Tensor transform(const Tensor& tensor, std::function<Tensor(const Tensor&)> transform_func) {
    ...
    return create_multi_device_tensor(output_tensors, tensor.storage_type(), get_distribution_strategy_from_tensor(tensor));
}
