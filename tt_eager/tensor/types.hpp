...
struct MultiDeviceHostStorage {
    ...
    void insert_buffer_for_device(Device* device, const OwnedBuffer buffer) {
    ...
    void insert_tensor_shape_for_device(Device* device, const Shape shape) {
        std::lock_guard<std::mutex> lock(mtx);
...
struct MultiDeviceStorage {
    ...
    void insert_buffer_for_device(Device* device, const DeviceBuffer buffer) {
        std::lock_guard<std::mutex> lock(mtx);
