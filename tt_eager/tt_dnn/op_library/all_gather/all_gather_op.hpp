...
enum AllGatherMode { RING_INTERLEAVED, FULL_WORKER_GRID_SHARDED, SINGLE_TILE_HIGH_WIDTH_SHARDED };
   ...
   public:
    AllGatherConfig(
        ...
        this->num_eth_buffers =
            (this->enable_bidirectional ? 8
                                        : (this->is_sharded && topology != all_gather_op::Topology::Linear ? 8 : 4));
        if (this->is_sharded) {
        ...
        this->eth_buffer_size = round_down(
            (total_l1_buffer_space - this->semaphore_offset) / (this->num_eth_buffers * num_duplicate_directions),
            page_size);
        ...
        TT_FATAL(
            this->eth_buffer_size * (this->num_eth_buffers * num_duplicate_directions) + this->semaphore_offset <=
            total_l1_buffer_space);

        // FIXME: dynamically select the number and size of each buffer based on tensor attributes, link count, ring
        // size, etc. Erisc is able to keep up with workers up to around 17-20 GBps bidirectional (numbers still being
        // locked down) depending on payload size. In general, the smaller each eth buffer, the more overhead per send,
        // and the larger each buffer, the less overhead. However, larger buffers are less desirable for smaller tensors
        // as the first send is delayed until a larger percentage of the overall tensor has landed in the erisc buffer
        // (which can impact latency negatively) and for smaller tensors, latency can be more dominant than throughput
        // with respect to end-to-end runtime. Additionally, tensor layout and location can affect worker throughput.
        // Based on loose empirical testing, if workers are in RowMajor or DRAM tile layout (maybe L1 too - need to
        // measure), it's preffered add more workers (8) with smaller buffers so each worker can keep up with erisc.
    }
    ...
    bool is_buffer_in_clockwise_ring(const uint32_t buffer_index) const {
        ...
        return this->enable_bidirectional ? buffer_index < get_num_edm_channels_in_clockwise_direction() : true;
    }
    uint32_t get_num_edm_channels_in_counter_clockwise_direction() const {
        // return all_gather_buffer_params::enable_bidirectional ? all_gather_buffer_params::num_buffers -
        // all_gather_buffer_params::num_buffers / 2 : 0; Force all through counter-clockwise direction
...
struct FullWorkerGridShardedAllGatherVariantConfig : public AllGatherConfig {
    std::string const& reader_kernel =
        "tt_eager/tt_dnn/op_library/all_gather/kernels/dataflow/"
        "worker_sharded_all_shard_workers_ring_gather_reader.cpp";
    std::string const& writer_kernel =
        "tt_eager/tt_dnn/op_library/all_gather/kernels/dataflow/"
        "worker_sharded_all_shard_workers_ring_gather_writer.cpp";
};
...
struct AllGather {
    ...
    static constexpr auto attribute_names = std::forward_as_tuple(
        "dim", "num_links", "ring_size", "ring_index", "receiver_device_id", "sender_device_id", "output_mem_config");
...
struct ShardedAllGatherConfig {
    ShardedAllGatherConfig(Tensor const& input_tensor, Tensor const& output_tensor, uint32_t dim) {
        ...
        switch (input_tensor.memory_config().memory_layout) {
            case TensorMemoryLayout::WIDTH_SHARDED: this->shard_type = ccl::ShardType::Width; break;
            case TensorMemoryLayout::BLOCK_SHARDED: this->shard_type = ccl::ShardType::Block; break;
            case TensorMemoryLayout::HEIGHT_SHARDED: this->shard_type = ccl::ShardType::Height; break;
            case TensorMemoryLayout::INTERLEAVED:
            case TensorMemoryLayout::SINGLE_BANK:
            default: TT_ASSERT(false, "ShardedAllGatherConfig only supports sharded memory layouts"); break;
        };

        Shape const& output_shape = output_tensor.get_legacy_shape();
        bool multiple_dims_are_multi_tile =
            std::count_if(output_shape.begin(), output_shape.end(), [](uint32_t s) { return s > 1; }) > 1;
...
struct ShardAddrGenArgGenerator {
    ...
    std::vector<uint32_t> generate() const {
        ...
        TT_ASSERT(
            args.size() == args_struct.get_expected_num_args(), "Generated args size doesn't match expected size");
    ...
    void dump_to_log() const {
        ...
        log_trace(
            tt::LogOp, "\tcontiguous_chunks_before_stride: {}", this->args_struct.contiguous_chunks_before_stride);
        ...
        for (auto n = 0; n < this->args_struct.num_dest_cores; ++n) {
            log_trace(
                ...
                "\t\tdest_core[{}]: x={},y={}",
                n,
                this->args_struct.dest_cores.at(n).x,
                this->args_struct.dest_cores.at(n).y);
        }
...
struct InputTensorShardAddrGenArgGenerator final : public ShardAddrGenArgGenerator {
    static std::vector<CoreCoord> ctor_generate_dest_cores(
        CoreRangeSet const& all_shard_cores, uint32_t worker_index, uint32_t num_workers) {
        bool row_wise = true;
        ...
        uint32_t worker_cores_start =
            worker_index * shards_per_worker + std::min(num_shard_cores % num_workers, worker_index);
    ...
    InputTensorShardAddrGenArgGenerator(
        ...
        std::vector<CoreCoord> const& dest_core_coords =
            ctor_generate_dest_cores(input_tensor_config->get_shard_spec().grid, worker_index, num_workers);
        this->args_struct.dest_cores.reserve(dest_core_coords.size());
        std::transform(
            dest_core_coords.begin(),
            dest_core_coords.end(),
            std::back_inserter(this->args_struct.dest_cores),
...
struct OutputTensorShardAddrGenArgGenerator final : ShardAddrGenArgGenerator {
    static std::vector<CoreCoord> compute_worker_coord_worker_dest_cores(
        ...
        TT_ASSERT(
            input_shards_per_dest_core > input_shards_per_input_worker
        ...
        uint32_t const contiguous_dest_cores_before_stride =
            input_shards_per_input_worker < input_shards_per_dest_core
                ? 1
                : input_shards_per_input_worker / input_shards_per_dest_core;
        ...
        while (current_dest_core_offset + core_offset_into_contiguous_chunk < global_shard_dest_cores.size()) {
            CoreCoord const& dest_core_coord =
                global_shard_dest_cores.at(current_dest_core_offset + core_offset_into_contiguous_chunk);
    ...
    static std::vector<ccl::WorkerXY> compute_worker_dest_cores(
        ...
        std::transform(
            worker_coord_worker_dest_cores.begin(),
            worker_coord_worker_dest_cores.end(),
            std::back_inserter(dest_cores_of_worker),
            [&device](CoreCoord const& core) {
                return ccl::WorkerXY(
                    static_cast<uint16_t>(device.worker_core_from_logical_core(core).x),
                    static_cast<uint16_t>(device.worker_core_from_logical_core(core).y));
    ...
    static std::pair<uint32_t, uint32_t> get_first_output_shard_starting_location(
        ...
        TT_ASSERT(
            num_input_tensor_shards >= global_num_buffers_per_chip, "Not enough input shards to support all gather");
        uint32_t worker_shard_offset = serving_worker_index * (num_input_tensor_shards / global_num_buffers_per_chip);
        TT_ASSERT(
            num_input_tensor_shards % global_num_buffers_per_chip == 0, "Don't support this non-divisibility yet");
        if (num_input_tensor_shards % global_num_buffers_per_chip != 0) {
            worker_shard_offset +=
                std::min(serving_worker_index, num_input_tensor_shards % global_num_buffers_per_chip);
        }
    ...
    static uint16_t get_intra_core_stride_in_shards(
        uint32_t input_shard_grid_size, uint32_t num_workers, uint32_t ring_size) {
    ...
    OutputTensorShardAddrGenArgGenerator(
        ...
        uint32_t starting_chunk_into_shard,
        bool is_worker_in_clockwise_ring) {
        bool is_shard_orientation_row_major = true;  // hardcoded until the switch is flipped for col_major. Just needs
                                                     // test cycles and transpose when iterating dest cores

        auto const& tensor_shard_grid = input_tensor_config->get_shard_spec().grid;
        uint32_t sharded_tensor_num_cores = tensor_shard_grid.num_cores();
        TT_ASSERT(
            sharded_tensor_num_cores == output_tensor_config->get_shard_spec().grid.num_cores(),
            "Input and output tensor must have the same number of cores");
        this->args_struct.is_clockwise = is_worker_in_clockwise_ring;
        this->args_struct.shard_size_in_bytes = input_tensor_config->get_shard_size_in_bytes();
        ...
        this->args_struct.intra_core_stride_in_shards =
            get_intra_core_stride_in_shards(sharded_tensor_num_cores, num_workers, ring_size);
        this->args_struct.contiguous_chunks_before_stride =
            get_contiguous_chunks_before_stride(sharded_tensor_num_cores, num_workers, ring_size);
        ...
        TT_ASSERT(this->args_struct.dest_cores.size() > 0);
        std::vector<CoreCoord> const& global_shard_dest_cores =
            corerange_to_cores(tensor_shard_grid, std::nullopt, is_shard_orientation_row_major);
        ...
        auto it = std::find(
            this->args_struct.dest_cores.begin(), this->args_struct.dest_cores.end(), noc0_starting_dest_core_xy);
        TT_ASSERT(
            it != this->args_struct.dest_cores.end(),
            "Didn't find starting dest core in dest cores. Internal logic error");
...
struct FullWorkerGridShardAddrGenArgGenerator {
    std::vector<uint32_t> generate() const {
        ...
        TT_ASSERT(
            args_struct.tile_size_in_bytes != ccl::UNINITIALIZED_VALUE_U32, "tile_size_in_bytes was uninitialized");
        TT_ASSERT(
            args_struct.shards_start_address != ccl::UNINITIALIZED_VALUE_U32, "shards_start_address was uninitialized");
        ...
        TT_ASSERT(
            args_struct.input_shard_num_tiles_x != ccl::UNINITIALIZED_VALUE_U16,
            "input_shard_num_tiles_x was uninitialized");
        TT_ASSERT(
            args_struct.input_shard_num_tiles_y != ccl::UNINITIALIZED_VALUE_U16,
            "input_shard_num_tiles_y was uninitialized");
