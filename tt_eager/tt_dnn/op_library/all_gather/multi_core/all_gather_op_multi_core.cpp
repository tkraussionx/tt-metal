...
// TODO(snijjar): Change naming to clockwise, counter-clockwise for direction, and
//                sender,receiver for each of these directions. The local chip's
//                clockwise sender is on the same core as the counter-clockwise
//                receiver.
...
uint32_t get_num_buffers_in_clockwise_direction() {
    ...
    // return 0;
    // return all_gather_buffer_params::num_buffers - 1;
}
bool is_buffer_in_clockwise_ring(const uint32_t buffer_index) {
    ...
    // Force all through counter-clockwise direction
    // return false;
    // return buffer_index < get_num_buffers_in_clockwise_direction();
}
...
operation::ProgramWithCallbacks all_gather_multi_core_with_workers(const Tensor& input_tensor, Tensor& output_tensor, const uint32_t dim, const uint32_t num_links, const uint32_t ring_size, const uint32_t ring_index, const chip_id_t receiver_device_id, const chip_id_t sender_device_id) {
    ...
    for (uint32_t i = 0; i < num_links; ++i) {
        ...
        for (uint32_t b = 0; b < all_gather_buffer_params::num_buffers; ++b) {
            ...
            //     std::cout << "\tlast_output_page_offset=" << last_output_page_offset << std::endl;
            //     std::cout << "\tlast_output_addr_offset=" << last_output_addr_offset << std::endl;
            //     std::cout << "\treceiver_ring_index=" << receiver_ring_index << std::endl;
            //     std::cout << "\treceiver_output_start_addr_offset=" << receiver_output_start_addr_offset << std::endl;
            //     std::cout << "\treceiver_output_start_page_idx=" << receiver_output_start_page_idx << std::endl;
            // }
            ...
            // std::cout << "HOST rwr " << b << " producer eth core: (x=" << worker_eth_receiver_core.x << ", y=" << worker_eth_receiver_core.y << ")" << std::endl;
            // Receiver Worker Kernels
            // std::cout << "host rwr " << b << " num_transfers=" << num_transfers << std::endl;
        ...
        std::vector<uint32_t> eth_sender_ct_args = {
            static_cast<uint32_t>(get_num_buffers_in_clockwise_direction() ? 1 : 0), // enable_sender_side
            static_cast<uint32_t>(get_num_buffers_in_counter_clockwise_direction() ? 1 : 0) // enable_receiver_side
        };
        ...
        {
            std::stringstream ss;
            ss << "edm_clockwise_kernel_rt_args:" << std::endl;
            for (auto arg : edm_clockwise_kernel_rt_args) {
                ss << "\t" << arg << std::endl;
            }
            // std::cout << ss.str();
        }
        ...
        {
            std::stringstream ss;
            ss << "edm_counter_clockwise_kernel_rt_args:" << std::endl;
            for (auto arg : edm_counter_clockwise_kernel_rt_args) {
                ss << "\t" << arg << std::endl;
            }
            // std::cout << ss.str();
        }
