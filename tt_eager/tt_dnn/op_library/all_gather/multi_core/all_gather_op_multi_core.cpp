            ...
            switch (topology) {
                ...
                case all_gather_op::Topology::Ring:
                    switch (all_gather_config.get_bidirectional_mode()) {
                        case tt::tt_metal::AllGatherBidirectionalMode::SPLIT_TENSOR:
...
operation::ProgramWithCallbacks all_gather_multi_core_with_workers(const Tensor& input_tensor, Tensor& output_tensor, const uint32_t dim, const uint32_t num_links, const uint32_t ring_size, const uint32_t ring_index, const std::optional<chip_id_t> receiver_device_id, const std::optional<chip_id_t> sender_device_id, all_gather_op::Topology topology) {
    ...
    bool full_send_both_directions =
        (topology == all_gather_op::Topology::Linear ||
         (topology == all_gather_op::Topology::Ring &&
          all_gather_config.get_bidirectional_mode() == tt::tt_metal::AllGatherBidirectionalMode::FULL_TENSOR));
    ...
    for (uint32_t direction = 0; direction < num_full_send_directions; direction++) {
        ...
        CoreRangeSet receiver_workers{{}}, sender_workers{{}};
        std::unordered_map<std::size_t, std::vector<CoreCoord>> receiver_worker_cores, sender_worker_cores;
        for (std::size_t i = 0; i < num_links; i++) {
            auto const& [receivers, senders] = select_worker_cores(all_gather_config, num_links, i, direction);
            receiver_workers = receiver_workers.merge(receivers);
            sender_workers = sender_workers.merge(senders);
            sender_worker_cores[i] = corerange_to_cores(sender_workers, std::nullopt, true);
            receiver_worker_cores[i] = corerange_to_cores(receiver_workers, std::nullopt, true);
            all_worker_sender_cores.insert(all_worker_sender_cores.end(), sender_worker_cores.at(i).begin(), sender_worker_cores.at(i).end());
            all_worker_receiver_cores.insert(all_worker_receiver_cores.end(), receiver_worker_cores.at(i).begin(), receiver_worker_cores.at(i).end());
        }
        ...
        auto build_worker_send_reader_ct_args = [&]() {
            if (is_sharded) {
                ...
                log_trace(tt::LogOp, "Worker SR args");
        ...
        KernelHandle worker_reader_sender_kernel_id = tt_metal::CreateKernel(
            ...
            sender_workers,
            tt_metal::ReaderDataMovementConfig(build_worker_send_reader_ct_args(), worker_defines));
        ...
        KernelHandle worker_sender_writer_kernel_id = tt_metal::CreateKernel(
            ...
            sender_workers,
            tt_metal::WriterDataMovementConfig(build_worker_sender_writer_ct_args(), worker_defines));
        ...
        KernelHandle worker_receiver_reader_kernel_id = tt_metal::CreateKernel(
            ...
            receiver_workers,
            tt_metal::ReaderDataMovementConfig(build_worker_receiver_reader_ct_args(), worker_defines));
        ...
        KernelHandle worker_receive_writer_kernel_id = tt_metal::CreateKernel(
            ...
            receiver_workers,
            tt_metal::WriterDataMovementConfig(build_worker_receive_writer_ct_args(), worker_defines));
        ...
        for (uint32_t i = 0; i < num_links; ++i) {
            ...
            {   // Compute num_full_chunks_per_worker and rem_pages_per_worker -> This should be pulled into a separte component
                ...
                //     std::size_t num_buffers_in_direction = all_gather_config.get_num_eth_buffers_per_edm();
                //     std::size_t num_clockwise_buffers = is_buffer_in_clockwise_direction(0) ? num_buffers_in_direction : 0;
                //     std::size_t num_counter_clockwise_buffers = is_buffer_in_clockwise_direction(0) ? 0 : num_buffers_in_direction;
                ...
                //     for (std::size_t b = 0; b < num_buffers_in_direction; b++) {
                ...
                //     bool rem_pages_added = false;
                //     for (std::size_t b = 0; b < all_gather_config.get_num_eth_buffers_per_edm(); b++) {
                //         if (b < num_full_chunks % num_buffers_in_direction) {
                ...
                //             if (!rem_pages_added) {
                //                 std::size_t idx = b % all_gather_config.get_num_eth_buffers_per_edm();
                //                 rem_pages_per_worker.at(b) = rem_pages;
                //                 rem_pages_added = true;
            ...
            for (uint32_t b = 0; b < all_gather_config.get_num_eth_buffers_per_edm(); ++b) {
                ...
                for (uint32_t w = b * num_workers_per_eth_buffer; w < (b + 1) * num_workers_per_eth_buffer; ++w) {
                    sender_worker_coords.push_back(
                        ccl::WorkerXY(
                            device->worker_core_from_logical_core(sender_worker_cores.at(i).at(w)).x,
                            device->worker_core_from_logical_core(sender_worker_cores.at(i).at(w)).y));
                    receiver_worker_coords.push_back(
                        ccl::WorkerXY(
                            device->worker_core_from_logical_core(receiver_worker_cores.at(i).at(w)).x,
                            device->worker_core_from_logical_core(receiver_worker_cores.at(i).at(w)).y));
                }
            ...
            for (uint32_t b = 0; b < all_gather_config.get_num_eth_buffers_per_edm(); ++b) {
                ...
                if (!is_linear || !is_first_chip_in_chain) {
                    ...
                    tt_metal::SetRuntimeArgs(
                        ...
                        receiver_worker_cores.at(i).at(b),
                    ...
                    auto build_worker_receive_writer_rt_args = [&]() {
                        auto worker_sender_reader = device->worker_core_from_logical_core(sender_worker_cores.at(i).at(b));
                        if (is_sharded) {
                            // # Receiver Writer (RT)
