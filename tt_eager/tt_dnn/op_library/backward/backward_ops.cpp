...
std::vector<Tensor> _exp_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    result = where(logical_and(gte_unary(abs(exp_result, output_mem_config), 1e+38, output_mem_config),ltz(grad, output_mem_config), std::nullopt, output_mem_config), -t_inf, result, output_mem_config);
    grad_tensor.emplace_back(result);
    return grad_tensor;
}
...
std::vector<Tensor> _rsqrt_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    result = where(logical_and(eqz(input, output_mem_config), eqz(grad, output_mem_config), std::nullopt, output_mem_config), t_nan, result, output_mem_config);
    grad_tensor.emplace_back(result);
    return grad_tensor;
}
...
std::vector<Tensor> _i0_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    result = where(eqz(input, output_mem_config), 0, result, output_mem_config);
    grad_tensor.emplace_back(result);
    return grad_tensor;
}
...
std::vector<Tensor> _acosh_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    grad_a = where(logical_and(eq_unary(abs(input), 1, output_mem_config), ltz(grad, output_mem_config), std::nullopt, output_mem_config), -t_inf, grad_a, output_mem_config);
    grad_a = where(logical_and(eq_unary(abs(input), 1, output_mem_config), gtz(grad, output_mem_config), std::nullopt, output_mem_config), t_inf, grad_a, output_mem_config);
    grad_tensor.emplace_back(grad_a);
    return grad_tensor;
}
...
std::vector<Tensor> _logit_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    grad_result = where(logical_and(eq_unary(grad, 0.0f, output_mem_config), eq_unary(input, 0.0f, output_mem_config), std::nullopt, output_mem_config), std::nanf(""), grad_result, output_mem_config);
    grad_tensor.emplace_back(grad_result);
    return grad_tensor;
}
...
std::vector<Tensor> _complex_recip_bw(const Tensor& grad, const Tensor& input, const MemoryConfig& output_mem_config) {
    ...
    Tensor condition_nan = logical_and(eqz(input_r,output_mem_config), eqz(input_i,output_mem_config), std::nullopt, output_mem_config);
    input_r.deallocate();
    input_i.deallocate();
