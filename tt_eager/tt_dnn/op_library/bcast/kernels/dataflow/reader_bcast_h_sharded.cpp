...
void kernel_main() {
    ...
    //DPRINT << "HELLO THERE I am IN KERNEL " << ENDL();
    ...
    uint32_t i = 0;
    uint32_t i1 = get_arg_val<uint32_t>(15);
    ...
    for (uint32_t ht = 0; ht < Ht; ht++) {
        for (uint32_t wt = 0; wt < Wt; wt++) {
                ...
                // noc_async_read(src_noc_addr, l1_write_addr_in0, tile_bytes);
