...
void Matmul::validate(
    ...
    std::visit(
        [input_tensor_a](const auto& program_config) {
            ...
            if constexpr (
                (not std::is_same_v<ProgramConfigType, MatmulDefaultProgramConfig>) and (not std::is_same_v<ProgramConfigType, MatmulMultiCoreReuseMultiCastDRAMShardedProgramConfig>)
            ) {
    ...
    std::visit(
        [
            ...
            if constexpr (
                ...
                std::is_same_v<ProgramConfigType, MatmulMultiCoreReuseMultiCastDRAMShardedProgramConfig>
            ) {
                TT_FATAL(input_tensor_a.is_sharded());
...
std::vector<Shape> Matmul::compute_output_shapes_dram_sharded(const std::vector<Tensor>& input_tensors, uint32_t N_unpadded) const {
    const auto input_shape_a = input_tensors.at(0).get_legacy_shape();
    const auto input_shape_b = input_tensors.at(1).get_legacy_shape();
        ...
        return std::visit(
            [&](const auto& program_config) -> std::vector<Tensor> {
                ...
                if constexpr (
                    ...
                    uint32_t N_unpaddded = input_tensor_b.get_legacy_shape()[-1] - input_tensor_b_shape.padding()[input_tensor_b_shape.rank() - 1].back;
                    ...
                    auto mem_config = this->output_mem_config;
                    mem_config.shard_spec = shard_spec;
                    return {create_sharded_device_tensor(this->compute_output_shapes_dram_sharded(input_tensors, N_unpaddded).at(0), this->output_dtype, output_layout, input_tensor_a.device(), mem_config)};
                } else if constexpr (
                    std::is_same_v<ProgramConfigType, MatmulMultiCoreReuseMultiCastProgramConfig>
                ) {
                    uint32_t M = input_tensor_a.volume() / input_tensor_a.get_legacy_shape()[-1] / TILE_HEIGHT;
    ...
    return std::visit(
        [&](const auto& program_config) -> MatmulParallelizationStrategy {
            ...
            if constexpr (std::is_same_v<ProgramConfigType, MatmulDefaultProgramConfig>) {
                ...
                return MatmulParallelizationStrategy::MULTI_CORE_REUSE_MCAST_DRAM_SHARDED_OPTIMIZED;
            }
            else {
                TT_THROW("Unrecognized Config");
            }
