...
void MAIN {
    ...
    constexpr uint32_t num_transaction_groups_per_block = get_compile_time_arg_val(14);
    constexpr uint32_t in1_block_num_tiles_per_group = get_compile_time_arg_val(15);
    #else
    constexpr uint32_t num_transaction_groups_per_block = 1;
    #endif
    ...
    #ifdef MULTIPLE_GROUPS_PER_BLOCK
    constexpr uint32_t in1_cb_id_inplace[8] = {1, 4, 5, 6, 7, 8, 9, 10};
    #endif
    ...
    for (uint32_t b = 0; b < batch; b++){
        ...
        {
            #ifndef MULTIPLE_GROUPS_PER_BLOCK
            ...
            uint32_t in0_index_group_offset = 0;
            uint32_t in1_index_group_offset = 0;
            for(uint32_t i = 0; i < num_transaction_groups_per_block; ++i) {
                #ifdef MULTIPLE_GROUPS_PER_BLOCK
                bool last_out = block == (num_blocks-1) and i == (num_transaction_groups_per_block - 1);
                // Configure packer once for pack out without Bias
                #if not defined FUSE_BIAS and defined PACK_RELU
                if (last_out) {
                    // if last block we pack the final result with relu enabled
                ...
                #ifdef MULTIPLE_GROUPS_PER_BLOCK
                cb_wait_front(in1_cb_id_inplace[i], in1_block_num_tiles);
                #else
                ...
                for (uint32_t in0_subblock = 0; in0_subblock < in0_num_subblocks; in0_subblock++) {
                    ...
                    for (uint32_t in1_subblock = 0; in1_subblock < in1_num_subblocks; in1_subblock++) {
                        ...
                        #ifdef MULTIPLE_GROUPS_PER_BLOCK
                        uint32_t in0_index = in0_index_group_offset + in0_index_subblock_offset; // offset into in0 block
                        uint32_t in1_index = in1_index_group_offset + in1_index_subblock_offset; // offset into in1 block
                        #else
                        ...
                        if (last_out) {
                            ...
                            #ifdef PACKER_L1_ACC
                                #ifdef MULTIPLE_GROUPS_PER_BLOCK
                                if (block == 0 and i == 0) { // no accumulation for first iteration
                                    PACK((  llk_pack_reconfig_l1_acc(0) ));
                                } else if (block == 0 and i == 1) {
                                    PACK((  llk_pack_reconfig_l1_acc(1) ));
                                }
                                #else
                                if (block == 0 ) { // no accumulation for first iteration
                    ...
                    in0_index_subblock_offset += in0_subblock_num_tiles;
                    ...
                    #ifdef FUSE_BIAS
                        #ifdef MULTIPLE_GROUPS_PER_BLOCK
                        if ((block < num_blocks - 1) or (block == num_blocks - 1 and i < num_transaction_groups_per_block - 1)) {
                        ...
                        if ((block < num_blocks - 1) or (block == num_blocks - 1 and i < num_transaction_groups_per_block - 2)) {
                            cb_wait_front(mm_partials_cb_id, out_block_num_tiles);
                            cb_pop_front(mm_partials_cb_id, out_block_num_tiles);
                        }
                        if (block == num_blocks - 1 and i == num_transaction_groups_per_block - 2) { enable_reload = true; } // reload when last iteration
                        #else
                        //Last iteration does spill and reload to output buffer
                ...
                #ifdef MULTIPLE_GROUPS_PER_BLOCK
                in0_index_group_offset += in0_block_w;
                in1_index_group_offset += in1_block_num_tiles_per_group;
                #endif
            ...
            #ifdef MULTIPLE_GROUPS_PER_BLOCK
            for(uint32_t i = 0; i < num_transaction_groups_per_block; ++i) {
                cb_pop_front(in1_cb_id_inplace[i], in1_block_num_tiles);
            }
            #else
