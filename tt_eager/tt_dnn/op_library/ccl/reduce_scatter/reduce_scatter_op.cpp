...
#include "tt_eager/tt_dnn/op_library/ccl/reduce_scatter/host/reduce_scatter_full_worker_grid.cpp"
...
operation::ProgramWithCallbacks ReduceScatter::create_program(const std::vector<Tensor> & input_tensors, std::vector<Tensor> &output_tensors) const {
    return reduce_scatter_with_workers(
        input_tensors,
        output_tensors,
        this->binary_op_type,
        this->scatter_dim,
...
std::vector<Tensor> reduce_scatter_impl(
    ...
    for (uint32_t i = 0; i < input_tensors.size(); ++i) {
        ...
        ops.emplace_back(ReduceScatter{
            ...
            output_mem_config,
            topology});
        output_tensors.push_back(operation::run(ops[i], input_tensors).at(0));
    }
...
static BinaryOpType convert_reduce_type_to_eltwise_type(ReduceOpMath reduce_op) {
    switch (reduce_op) {
        ...
        default:
            TT_FATAL("Reduce scatter only support reduce_op_type SUM");
            return BinaryOpType::ADD;
    }
}
