...
inline void tilize_in(
    ...
    tilize_init_short(in_cb_id, in_block_w);
     DPRINT << "TILIZE_IN_1 " << in_block_w << " " << in_subblock_h << "    " << in_num_subblocks << ENDL();
    for (uint32_t in_subblock = 0; in_subblock < in_num_subblocks; ++in_subblock) {
        for (uint32_t h = 0; h < in_subblock_h; ++h) {
            //DPRINT << "TILIZE_IN: in_subblock=" << in_subblock << " h=" << h << ENDL();
            ...
            //DPRINT << "TILIZE_IN_2 "<< ENDL();
        }
    }
    tilize_uninit(in_cb_id);
    //DPRINT << "TILIZE_IN: complete" << ENDL();
} // tilize_in()
...
void MAIN {
    ...
    #ifdef FUSE_BIAS
    DPRINT << "FUSE BIAS entering" << ENDL();
    ...
    for(uint32_t in1_block_w_i = 0; in1_block_w_i < 1; ++in1_block_w_i) {

        for(uint32_t in0_block_h_i = 0; in0_block_h_i < 1; ++in0_block_h_i) {
            #ifdef PRE_TILIZE
            DPRINT << "PRE_TILIZE entering" << ENDL();
            ...
            // #ifdef PACK_RELU
            // DPRINT << "PACK_RELU entering" << ENDL();
            ...
            // DPRINT << "COMPUTE_167" << ENDL();
            for(uint32_t in0_block_w_i = 0; in0_block_w_i < 1; ++in0_block_w_i) {
            ...
            // DPRINT << "COMPUTE_329" << ENDL();
            // cb_wait_front(bias_cb_id, bias_ntiles_w);
            // cb_wait_front(matmul_partials_cb, out_block_num_tiles);
            // DPRINT << "COMPUTE_332" << ENDL();
            // for (uint32_t in0_subblock_i = 0; in0_subblock_i < in0_num_subblocks; ++in0_subblock_i) {
            //     uint32_t in1_index_subblock_offset = 0;
            //     for (uint32_t in1_subblock_i = 0; in1_subblock_i < in1_num_subblocks; ++in1_subblock_i) {
            //         tile_regs_acquire();
