...
void MAIN {
    ...
    #endif
    PACK ( DPRINT << "in compute kernel" << ENDL(); )
    ...
    for(uint32_t in1_block_w_i = 0; in1_block_w_i < in1_num_blocks_w; ++in1_block_w_i) {
        ...
        for(uint32_t in0_block_h_i = 0; in0_block_h_i < in0_num_blocks_h; ++in0_block_h_i) {
            ...
            #ifdef FUSE_BIAS
            PACK( DPRINT << "fuse bias enabled" << ENDL());
            #ifdef PACK_RELU
            PACK( DPRINT << "pack relu enabled" << ENDL());
            // if last block we pack the final result with relu enabled
            PACK(( llk_pack_relu_config(ReluType::ZERO_RELU) ));
            #endif
            #ifdef PACKER_L1_ACC
            ...
            PACK( DPRINT << "waiting for bias tiles" << ENDL());
            cb_wait_front(bias_cb_id, bias_ntiles_w);
            PACK( DPRINT << "done waiting for bias" << ENDL());
            cb_wait_front(matmul_partials_cb, out_block_num_tiles);
            PACK( DPRINT << "Done waiting for mm partials cb" << ENDL());
            for (uint32_t in0_subblock_i = 0; in0_subblock_i < in0_num_subblocks; ++in0_subblock_i) {
                uint32_t in1_index_subblock_offset = 0;
                for (uint32_t in1_subblock_i = 0; in1_subblock_i < in1_num_subblocks; ++in1_subblock_i) {
                    tile_regs_acquire();
