...
void kernel_main() {
    ...
    constexpr uint32_t act_block_w_bytes = get_compile_time_arg_val(13); // must divide conv_act_read_bytes evenly
    constexpr uint32_t num_partial_channel_reads = get_compile_time_arg_val(14); // must equal to conv_act_c_read_bytes / act_block_w_bytes;
    ...
    */
    ...
    static_assert(act_block_w_bytes <= NOC_MAX_BURST_SIZE);
    // set_state uses just x/y from the get_noc_addr, addr is ignored
    noc_async_read_one_packet_set_state(get_noc_addr(act_l1_read_addr), act_block_w_bytes);
    uint32_t start_reader_idx = 0;
    for (uint32_t nbh = 0; nbh < act_num_blocks_h; nbh++) {
        reader_offset_idx = 0;
        for (uint32_t fh  = 0; fh < window_outer; fh++) {
            for (uint32_t fw = 0; fw < window_inner; fw++) {
                uint32_t reader_offset = act_l1_read_addr + (reader_offsets_ptr[reader_offset_idx] * conv_act_c_read_bytes);
                uint32_t partial_channel_read_offset = 0;
                for (uint32_t ncr = 0; ncr < num_partial_channel_reads; ncr++) {
                    reader_idx = start_reader_idx;
                    cb_reserve_back(cb_id_act, act_block_num_tiles_read);
                    uint32_t l1_write_addr_act = get_write_ptr(cb_id_act);
                    for (uint32_t bhd = 0; bhd < act_block_h_datums_read; bhd++) {
                        // Local read. Not optimal. Re-reading the same indices for (num_partial_channel_reads * window_outer * window_inner) times
                        uint32_t two_reader_indices = packed_reader_indices_ptr[reader_idx];
                        uint32_t reader_idx_1 = two_reader_indices & 0xffff;
                        uint32_t reader_idx_2 = two_reader_indices >> 16;

                        act_l1_offset = reader_offset + partial_channel_read_offset + (reader_idx_1 * conv_act_c_read_bytes);
                        noc_async_read_one_packet_with_state<true>(act_l1_offset, l1_write_addr_act);
                        l1_write_addr_act += act_block_w_bytes;

                        act_l1_offset = reader_offset + partial_channel_read_offset + (reader_idx_2 * conv_act_c_read_bytes);
                        noc_async_read_one_packet_with_state<true>(act_l1_offset, l1_write_addr_act);
                        l1_write_addr_act += act_block_w_bytes;
                    ...
                    partial_channel_read_offset += act_block_w_bytes;
                }
                reader_offset_idx += 1;
            }
        }
