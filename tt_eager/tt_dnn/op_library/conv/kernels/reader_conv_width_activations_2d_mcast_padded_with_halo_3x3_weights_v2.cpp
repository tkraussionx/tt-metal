...
void read_channels(uint32_t& l1_write_addr_act, const uint32_t act_l1_read_addr, const uint32_t reader_channel_idx,
    ...
    //print_pages(act_l1_read_addr_plus_offset, 1, 1);
...
void kernel_main() {
    ...
    uint32_t act_block_num_tiles = get_compile_time_arg_val(15);
    ...
    //     DPRINT << "ACTIVATION" << ENDL();
    //     return;
    // }
    // DPRINT << "TESTING_READER "<<(uint) my_x[0] << "," << (uint) my_y[0] << ENDL();
    // DPRINT << " stride_h " << stride_h << " stride_w " << stride_w << " conv_act_size_w " << conv_act_size_w << " conv_output_w_last_index " << conv_output_w_last_index << " conv_act_c_read_bytes " << conv_act_c_read_bytes << " window_outer " << window_outer << " window_inner " << window_inner << " act_block_h_datums " << act_block_h_datums << " act_num_blocks_h " << act_num_blocks_h << ENDL();
    // DPRINT << " act_block_num_tiles " << act_block_num_tiles << " act_w_num_outer " << act_w_num_outer << " act_mcast_num_dests " << act_mcast_num_dests << " act_mcast_num_cores " << act_mcast_num_cores << " act_mcast_sender_semaphore_addr " << act_mcast_sender_semaphore_addr << " act_mcast_receiver_semaphore_addr "  << act_mcast_receiver_semaphore_addr << " act_mcast_sender_size_bytes " << act_mcast_sender_size_bytes << ENDL();
    ...
    DPRINT << act_mcast_dest_noc_start_x << " " << act_mcast_dest_noc_start_y << " " << act_mcast_dest_noc_end_x << " " << act_mcast_dest_noc_end_y << ENDL();
    ...
    //constexpr uint32_t conv_act_c_read_bytes = 16;
    ...
    noc_async_read_one_packet_set_state(get_noc_addr(act_l1_read_addr), coalesced_read_bytes); //why writen like this

    // Reset reader_idx to finish act_block_h_datums
    uint32_t reader_idx = 0;
    for (uint32_t nbh = 0; nbh < 1 /*act_num_blocks_h*/; nbh++) {
        ...
        DPRINT << "reader half completed" << ENDL();
        //noc_async_read_inc_num_issued(num_issued_reads_per_block); // "false" on read
        DPRINT << "reader half_1 completed" << ENDL();
        noc_async_read_barrier();
        DPRINT << "reader completed" << ENDL();
        cb_push_back(cb_id_act_row_major_bfloat16, act_block_num_tiles);
        uint32_t tilized_act_start_address = get_read_ptr(cb_id_act_row_major_bfloat16);
        print_pages(tilized_act_start_address, 288, 1);
        DPRINT << "COMING_HERE " << act_w_num_outer << "    " << act_mcast_num_dests << "   " << act_mcast_sender_id << " act_block_num_tiles " << act_block_num_tiles << ENDL();
        constexpr uint32_t act_mcast_num_dests_test = 35;
