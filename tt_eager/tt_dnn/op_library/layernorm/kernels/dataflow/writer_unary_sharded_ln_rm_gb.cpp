...
void kernel_main() {
    ...
    if constexpr(fuse_gamma) {
        ...
        for (uint32_t w = 0; w < block_w; w++) {
            ...
            noc_async_read<true>(gamma_noc_addr, l1_write_addr_gamma, mask_read_tile_face_bytes);
            gamma_noc_addr += mask_read_tile_face_bytes;
            noc_async_read<true>(gamma_noc_addr, l1_write_addr_gamma + mask_read_tile_offset_bytes, mask_read_tile_face_bytes);
    ...
    if constexpr(fuse_beta) {
        ...
        for (uint32_t w = 0; w < block_w; w++) {
            ...
            noc_async_read<true>(beta_noc_addr, l1_write_addr_beta, mask_read_tile_face_bytes);
            beta_noc_addr += mask_read_tile_face_bytes;
            noc_async_read<true>(beta_noc_addr, l1_write_addr_beta + mask_read_tile_offset_bytes, mask_read_tile_face_bytes);
