...
std::vector<std::optional<Tensor>> moreh_matmul_backward_(
    ...
    if (input_requires_grad) {
        ...
        if (is_same_batch_dim(output_grad, input_grad_tensor)) {
            ...
            moreh_sum(temp_input_grad, reduce_dims, input_grad_tensor, output_mem_config, compute_kernel_config);
        }
    ...
    if (other_requires_grad) {
        ...
        if (is_same_batch_dim(output_grad, other_grad_tensor)) {
            ...
            moreh_sum(temp_other_grad, reduce_dims, other_grad_tensor, output_mem_config, compute_kernel_config);
        }
