...
struct Move {
    ...
    static constexpr auto attribute_names =
        std::forward_as_tuple("output_mem_config", "move_op_parallelization_strategy");
    const auto attribute_values() const {
        return std::forward_as_tuple(this->output_mem_config, this->move_op_parallelization_strategy);
    }
};
...
inline Tensor move(const Tensor &input_tensor, const std::optional<MemoryConfig> &mem_config) {
    ...
    uint32_t size_per_bank = tt_metal::detail::SizeBytesPerBank(
        output_tensor.buffer()->size(), output_tensor.buffer()->page_size(), num_banks);
    ...
    uint32_t size_per_l1_bank = tt_metal::detail::SizeBytesPerBank(
        output_tensor.buffer()->size(), output_tensor.buffer()->page_size(), num_l1_banks);
    ...
    bool fits_in_cb =
        (L1_UNRESERVED_BASE + size_per_l1_bank) <= (output_mem_config.buffer_type == tt_metal::BufferType::L1
    ...
    auto output =
        operation::run(Move{output_mem_config, move_op_parallelization_strategy}, {input_tensor, output_tensor}).at(0);
    return output;
}
    ...
    operation::launch_op(
        [mem_config](
            ...
            return operation::run(
                Move{output_mem_config, move_op_parallelization_strategy}, {input_tensor, output_tensor});
        },
        {input_tensor},
        output_tensors);
