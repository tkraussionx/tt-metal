...
void kernel_main() {
    ...
    uint32_t in1_tensor_tile_id                  = get_arg_val<uint32_t>(2);
    ...
    constexpr uint32_t cb_id_q = 1; // cb for Q

    constexpr uint32_t onetile = 1;
    const uint32_t single_tile_size_bytes = get_tile_size(cb_id_q);
    const DataFormat data_format = get_dataformat(cb_id_q);
    ...
    for (uint32_t block = 0; block < num_blocks; block++) {
        ...
        for (uint32_t i = 0; i < q_num_tiles; i++) {
            cb_reserve_back(cb_id_q, onetile);
            uint32_t l1_write_addr = get_write_ptr(cb_id_q);
            noc_async_read_tile(in0_tensor_tile_id, s0, l1_write_addr);
            noc_async_read_barrier();
            cb_push_back(cb_id_q, onetile);
            in0_tensor_tile_id++;
        }
    }
}
