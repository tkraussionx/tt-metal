...
operation::ProgramWithCallbacks multi_core_tutorial_nlp_concat_heads(const Tensor &input_tensor_a, Tensor &output, CoreCoord compute_with_storage_grid_size);
...
inline Tensor tutorial_nlp_concat_heads(const Tensor &input_tensor_a, const MemoryConfig& mem_config) {
    return operation::run(TutorialNlpConcatHeads{mem_config}, {input_tensor_a}).at(0);
}
}  // namespace tt_metal
