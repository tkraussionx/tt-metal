...
inline void reduce_h_fused(
    ...
    cb_wait_front(curr_in_cb_id, 8*nblocks);
    for (uint32_t n = 0; n < 8; ++ n) {
        tile_regs_acquire();
        for (uint32_t out_elem_i = 0; out_elem_i < nblocks; ++ out_elem_i) {
            unpack_tilizeA_B_block(curr_in_cb_id, in_scalar_cb_id, in_ntiles_hwc, n*in_ntiles_hwc, 0 /*tile idx for Src b is 0 because only 1 tile of constants is loaded*/, num_faces_in_tile /* unpack 1 or 2 faces ) */);
    ...
    cb_pop_front(curr_in_cb_id, 8*nblocks);
}
...
void MAIN {
    ...
    cb_reserve_back(out_cb_id, nsticks_per_core_by_nblocks);

    cb_wait_front(in_scalar_cb_id, 1);
    for (uint32_t i = 0; i < nsticks_per_core_by_nblocks/8; ++ i) {
