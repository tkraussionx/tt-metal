...
std::vector<Tensor> MaxPool::create_output_tensors(const std::vector<Tensor> &inputs) const {
    ...
    if (this->out_mem_config_.is_sharded()) {
        ...
        uint32_t out_nhw_padded = ceil_multiple_of(out_nhw, constants::TILE_HEIGHT);
        uint32_t ncores = max_pool_helpers::get_num_cores(input.device()->compute_with_storage_grid_size(), out_nhw_padded);
