...
void MAIN {
    ...
    for (uint32_t block_index = 0; block_index < per_core_block_cnt; block_index = block_index + 2 ) {
        cb_reserve_back(tt::CB::c_out0, per_core_block_dim);
        // for(uint32_t tile_index = 0; tile_index < per_core_block_dim; ++tile_index) {
            acquire_dst(tt::DstMode::Half);

            // Pop tile after tile, copy to DST and pack
            cb_wait_front(tt::CB::c_in0, block_index);
            cb_wait_front(tt::CB::c_intermed0, block_index + 1);
