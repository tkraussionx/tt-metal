...
auto decorate_run_operation(Function function) {
    return [function]<typename Operation, typename InputTensors, typename... Args>(
               Operation operation, InputTensors input_tensors, Args&&... args) {
        ...
        auto profile_scope =
            op_profiler::OpProfileScope(operation.get_type_name(), get_profiler_operation_type<Operation>());
        auto do_profile = op_profiler::get_profiler_flag();
        if (do_profile) {
            detail::setup_profiler(operation, input_tensors);
        }

        auto output_tensors = function(operation, input_tensors, args...);
...
std::vector<Tensor> run(const HostOperation& operation, const std::vector<Tensor>& input_tensors) {
    return detail::decorate_run_operation(detail::run_host_operation)(operation, input_tensors);
}
...
std::vector<Tensor> run(
    ...
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) {
    return detail::decorate_run_operation(detail::run_device_operation)(
        operation, input_tensors, optional_input_tensors);
}
