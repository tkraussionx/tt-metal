...
void kernel_main() {
    ...
    const uint32_t sender_core_x = get_arg_val<uint32_t>(16);
    const uint32_t sender_core_y = get_arg_val<uint32_t>(17);
    ...
    const uint64_t k_mcast_sender_semaphore_noc_addr = get_noc_addr(sender_core_x, sender_core_y, k_mcast_sender_semaphor);
    ...
    const uint64_t v_mcast_sender_semaphore_noc_addr = get_noc_addr(sender_core_x, sender_core_y, v_mcast_sender_semaphor);
        ...
        for (uint32_t nq = local_nh_start; nq < local_nh_end; ++nq) {
            for (uint32_t q_iter = 0; q_iter < q_chunks_per_core; ++q_iter) {
                ...
                for (uint32_t k_chunk = 0; (k_chunk * Sk_chunk_t) < q_high_idx; ++k_chunk) {
                    ...
                    noc_semaphore_set(k_mcast_receiver_semaphore_addr_ptr, INVALID);

                    // Atomic increment source core counter
                    noc_semaphore_inc(k_mcast_sender_semaphore_noc_addr, 1);

                    // wait on in1 semaphore value to become VALID (set by mcast sender after it multicasts data)
                    noc_semaphore_wait(k_mcast_receiver_semaphore_addr_ptr, VALID);
                    ...
                    // Set in1 semaphore value to INVALID
                    noc_semaphore_set(v_mcast_receiver_semaphore_addr_ptr, INVALID);

                    // Atomic increment source core counter
                    noc_semaphore_inc(v_mcast_sender_semaphore_noc_addr, 1);

                    // wait on in1 semaphore value to become VALID (set by mcast sender after it multicasts data)
                    noc_semaphore_wait(v_mcast_receiver_semaphore_addr_ptr, VALID);
