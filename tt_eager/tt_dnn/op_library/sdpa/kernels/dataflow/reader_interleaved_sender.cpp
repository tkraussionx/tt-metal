...
void kernel_main() {
    ...
    const uint32_t num_dests = get_arg_val<uint32_t>(11);
    const uint32_t k_mcast_sender_semaphor = get_arg_val<uint32_t>(12);
    const uint32_t k_mcast_receiver_semaphor = get_arg_val<uint32_t>(13);
    const uint32_t v_mcast_sender_semaphor = get_arg_val<uint32_t>(14);
    const uint32_t v_mcast_receiver_semaphor = get_arg_val<uint32_t>(15);
    const uint32_t top_core_x = get_arg_val<uint32_t>(16);
    const uint32_t top_core_y = get_arg_val<uint32_t>(17);
    const uint32_t bottom_core_x = get_arg_val<uint32_t>(18);
    const uint32_t bottom_core_y = get_arg_val<uint32_t>(19);
    ...
    const uint32_t k_block_size_bytes = k_tile_bytes * Sk_chunk_t * DHt;
    const uint32_t v_block_size_bytes = v_tile_bytes * Sk_chunk_t * DHt;

    constexpr uint32_t barrier_threshold = get_barrier_read_threshold<q_tile_bytes, num_cores>();

    // Set ur local VALID value, to be mcasted to destinations flag address after the data has been mcasted
    volatile tt_l1_ptr uint32_t* k_mcast_receiver_semaphore_addr_ptr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(k_mcast_receiver_semaphor);
    *(k_mcast_receiver_semaphore_addr_ptr) = VALID;
    // local address that will be atomically incremented by mcast receivers, to know when all receivers are ready
    // to receive the mcast
    volatile tt_l1_ptr uint32_t* k_mcast_sender_semaphore_addr_ptr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(k_mcast_sender_semaphor);

    const uint64_t k_mcast_receiver_semaphore_noc_addr = get_noc_multicast_addr(
        top_core_x,
        top_core_y,
        bottom_core_x,
        bottom_core_y,
        k_mcast_receiver_semaphor);

    const uint64_t k_multicast_data_noc = get_noc_multicast_addr(
        top_core_x,
        top_core_y,
        bottom_core_x,
        bottom_core_y,
    ...
    // Set ur local VALID value, to be mcasted to destinations flag address after the data has been mcasted
    volatile tt_l1_ptr uint32_t* v_mcast_receiver_semaphore_addr_ptr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(v_mcast_receiver_semaphor);
    *(v_mcast_receiver_semaphore_addr_ptr) = VALID;
    // local address that will be atomically incremented by mcast receivers, to know when all receivers are ready
    // to receive the mcast
    volatile tt_l1_ptr uint32_t* v_mcast_sender_semaphore_addr_ptr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(v_mcast_sender_semaphor);

    const uint64_t v_mcast_receiver_semaphore_noc_addr = get_noc_multicast_addr(
        top_core_x,
        top_core_y,
        bottom_core_x,
        bottom_core_y,
        v_mcast_receiver_semaphor);

    const uint64_t v_multicast_data_noc = get_noc_multicast_addr(
        top_core_x,
        top_core_y,
        bottom_core_x,
        bottom_core_y,
        ...
        for (uint32_t nq = local_nh_start; nq < local_nh_end; ++nq) {
            for (uint32_t q_iter = 0; q_iter < q_chunks_per_core; ++q_iter) {
                ...
                for (uint32_t k_chunk = 0; (k_chunk * Sk_chunk_t) < q_high_idx; ++k_chunk) {
                    ...
                    uint32_t k_start_address = k_write_ptr;
                    barrier_count = 0;
                    ...
                    noc_semaphore_wait(k_mcast_sender_semaphore_addr_ptr, num_dests);
                    noc_semaphore_set(k_mcast_sender_semaphore_addr_ptr, 0);

                    // Now we have the block in the CB address, we can mcast to dests!
                    uint64_t k_multicast_data_addr = k_multicast_data_noc | k_start_address;

                    // num_dests must not include source, since we are NOT really doing a local copy!
                    noc_async_write_multicast(k_start_address, k_multicast_data_addr, k_block_size_bytes, num_dests, false, false);
                    ...
                    // num_dests must not include source, since we are NOT really doing a local copy!
                    noc_semaphore_set_multicast(k_mcast_receiver_semaphor, k_mcast_receiver_semaphore_noc_addr, num_dests, false, false);
                    ...
                    uint32_t v_start_address = v_write_ptr;
                    barrier_count = 0;
                    ...
                    // wait until all in1 mcast destinations have atomically incremented the in1 semaphore_addr (i.e. its value should be in0_mcast_num_dests), then reset
                    // the semaphore_addr value back to zero for the next block
                    noc_semaphore_wait(v_mcast_sender_semaphore_addr_ptr, num_dests);
                    noc_semaphore_set(v_mcast_sender_semaphore_addr_ptr, 0);

                    // Now we have the block in the CB address, we can mcast to dests!
                    uint64_t v_multicast_data_addr = v_multicast_data_noc | v_start_address;

                    // num_dests must not include source, since we are NOT really doing a local copy!
                    noc_async_write_multicast(v_start_address, v_multicast_data_addr, v_block_size_bytes, num_dests, false, false);
                    ...
                    // We should also multicast the flag to destinations
                    // num_dests must not include source, since we are NOT really doing a local copy!
                    noc_semaphore_set_multicast(v_mcast_receiver_semaphor, v_mcast_receiver_semaphore_noc_addr, num_dests, false, false);
