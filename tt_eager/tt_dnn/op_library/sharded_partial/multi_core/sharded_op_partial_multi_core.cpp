...
operation::ProgramWithCallbacks interleaved_to_sharded_partial_multi_core(const Tensor& input, Tensor& output, ShardedOpSplitDim split_dim, int num_slices, int slice_index) {
    ...
    if (input.get_layout() == Layout::TILE) {
        ...
        unary_reader_kernel_id = tt_metal::CreateKernel(
            program,
            "tt_eager/tt_dnn/op_library/sharded_partial/kernels/dataflow/reader_unary_sharded_blocks_interleaved_start_id.cpp",
            all_cores,
        ...
        unary_reader_kernel_id = tt_metal::CreateKernel(
            program,
            "tt_eager/tt_dnn/op_library/sharded_partial/kernels/dataflow/"
    ...
    tt_metal::KernelHandle unary_writer_kernel_id = tt_metal::CreateKernel(
        program,
        "tt_eager/tt_dnn/op_library/sharded_partial/kernels/dataflow/writer_unary_sharded.cpp",
        all_cores,
    ...
    if (convert_df) {
        compute_kernel_id = tt_metal::CreateKernel(
            program,
            "tt_eager/tt_dnn/op_library/sharded_partial/kernels/compute/eltwise_copy.cpp",
            all_cores,
            tt_metal::ComputeConfig{});
    }

    uint32_t curr_idx_h = 0, curr_idx_w = 0;
    if (split_dim == ShardedOpSplitDim::DimRow) {
...
operation::ProgramWithCallbacks sharded_to_interleaved_partial_multi_core(const Tensor& input, const Tensor& output, ShardedOpSplitDim split_dim, int num_slices, int slice_index) {
    tt_metal::Program program{};
    ...
    tt_metal::KernelHandle unary_reader_kernel_id = tt_metal::CreateKernel(
        program,
        "tt_eager/tt_dnn/op_library/sharded_partial/kernels/dataflow/reader_unary_sharded.cpp",
        all_cores,
        tt_metal::ReaderDataMovementConfig(reader_compile_time_args));
    ...
    if (input.get_layout() == Layout::TILE) {
        ...
        unary_writer_kernel_id = tt_metal::CreateKernel(
            program,
            "tt_eager/tt_dnn/op_library/sharded_partial/kernels/dataflow/writer_unary_sharded_blocks_interleaved_start_id.cpp",
            all_cores,
            tt_metal::WriterDataMovementConfig(writer_compile_time_args));
    } else {
