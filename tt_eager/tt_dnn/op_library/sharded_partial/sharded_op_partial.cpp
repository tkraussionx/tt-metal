...
void ShardedPartial::validate(const std::vector<Tensor>& input_tensors) const {
    ...
    TT_FATAL(split_dim == ShardedOpSplitDim::DimRow, "Currently, split dim::ROW is only supported");
...
operation::ProgramWithCallbacks ShardedPartial::create_program(
    ...
    if (this->sharded_op_type == ShardedOpPartialType::InterleavedToShardedPartial) {
        ...
        return interleaved_to_sharded_partial_multi_core(input_tensor, output_tensor, this->split_dim, this->num_slices, this->slice_index);
    } else {
        // For S->I partial, we store output tensor as input_tensors[1]
        auto& output_tensor = input_tensors[1];
        return sharded_to_interleaved_partial_multi_core(input_tensor, output_tensor, this->split_dim, this->num_slices, this->slice_index);
    }
}
