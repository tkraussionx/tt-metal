...
enum class ShardedOpSplitDim {
    DimRow      = 0,
    DimCol      = 1,
    DimRowCol   = 2
};

operation::ProgramWithCallbacks interleaved_to_sharded_partial_multi_core(const Tensor &a, Tensor &output, ShardedOpSplitDim split_dim, int num_slices, int slice_index);
operation::ProgramWithCallbacks sharded_to_interleaved_partial_multi_core(const Tensor &a, const Tensor &output, ShardedOpSplitDim split_dim, int num_slices, int slice_index);

struct ShardedPartial {
    const CoreCoord grid_size;
    const ShardSpec shard_spec;
    const ShardedOpSplitDim split_dim;
...
inline Tensor interleaved_to_sharded_partial(
    ...
    const ShardedOpSplitDim shard_split_dim,
    const uint32_t num_slices,
    const uint32_t slice_index,
    ...
    std::visit(
        [&](const auto &grid) {
            ...
            if constexpr (std::is_same_v<GridType, CoreCoord>) {
                ...
                if (shard_split_dim == ShardedOpSplitDim::DimRow) {
    ...
    log_info(
        "Produced shard_spec is: Core_Ranges {}, Shard_Shape: [{}, {}] Shard_Orientation: {}",
        grid_set.str(),
        shard_shape[0],
        shard_shape[1],
        shard_orientation == ShardOrientation::COL_MAJOR ? "ColMajor" : "RowMajor");
    MemoryConfig sharded_mem_config = MemoryConfig{.memory_layout = shard_scheme, .buffer_type = BufferType::L1};
    return operation::run(
               ShardedPartial{
                   .grid_size = grid_size,
                   .shard_spec = shard_spec,
                   .split_dim = shard_split_dim,
