...
void kernel_main() {
    ...
    for (uint32_t w = 0; w<block_wt; w++) {
        ...
        noc_async_read<true>(mask_noc_addr, l1_write_addr, mask_read_tile_face_bytes);
        mask_noc_addr += mask_read_tile_face_bytes;
        noc_async_read<true>(mask_noc_addr, l1_write_addr + mask_read_tile_offset_bytes, mask_read_tile_face_bytes);
