                ...
                [&](const auto& program_config) {
                    ...
                    if constexpr (std::is_same_v<
                        ...
                        TT_FATAL(!this->is_scale_causal_mask_hw_dims_softmax);
                                             ...
                                             ProgramConfigType,
                                             tt::operations::primary::transformers::
...
operation::ProgramWithCallbacks Softmax::create_program(
    ...
    return std::visit(
        [&](const auto& program_config) -> operation::ProgramWithCallbacks {
            ...
            if constexpr (std::is_same_v<
                              ProgramConfigType,
                              tt::operations::primary::transformers::SoftmaxShardedMultiCoreProgramConfig>) {
...
Tensor softmax_in_place(
    Tensor& input_tensor,
    const transformers::SoftmaxProgramConfig& program_config,
    std::optional<const DeviceComputeKernelConfig> compute_kernel_config) {
    return transformers::scale_mask_softmax_in_place(
        input_tensor, std::nullopt, std::nullopt, program_config, false, compute_kernel_config);
}
...
Tensor scale_mask_softmax_in_place(
    ...
    operation::launch_op(
        [scale, mask, program_config, is_causal_mask, compute_kernel_config](
            const std::vector<Tensor>& input_tensors,
            const std::vector<std::optional<const Tensor>>& optional_input_tensors,
...
Tensor scale_causal_mask_hw_dims_softmax_in_place(
    ...
    std::vector<Tensor> dummy_output_tensors = {Tensor(operation::get_workers_for_op_output({input_tensor}))};
    operation::launch_op(
        [scale, mask, program_config, compute_kernel_config](
            const std::vector<Tensor>& input_tensors,
            const std::vector<std::optional<const Tensor>>& optional_input_tensors,
            const std::vector<std::optional<Tensor>>& optional_output_tensors) mutable -> std::vector<Tensor> {
            auto& input_tensor = input_tensors.at(0);
...
Tensor softmax(
    ...
    std::optional<const DeviceComputeKernelConfig> compute_kernel_config) {
    return transformers::scale_mask_softmax(
        input_tensor, std::nullopt, std::nullopt, output_mem_config, false, compute_kernel_config);
}
...
Tensor scale_mask_softmax(
    ...
    operation::launch_with_autoformat(
        [scale, mask, output_mem_config, is_causal_mask, compute_kernel_config](
            const std::vector<Tensor>& input_tensors,
            const std::vector<std::optional<const Tensor>>& optional_input_tensors,
            const std::vector<std::optional<Tensor>>& optional_output_tensors) mutable -> std::vector<Tensor> {
            auto& input_tensor = input_tensors.at(0);
