...
SliceRange srcColumn = SliceRange{.h0 = 0, .h1 = 32, .hs = 1, .w0 = 0, .w1 = 1, .ws = 1};
SliceRange srcRow = SliceRange{.h0 = 0, .h1 = 1, .hs = 1, .w0 = 0, .w1 = 32, .ws = 1};

template <typename T>
inline void dprint_tile(uint32_t cb_id, uint32_t wt) {
    #if defined(TRISC_PACK)
    uint32_t write_addr = cb_interface[cb_id].fifo_wr_ptr << 4;
    DPRINT << "PACKED VALUES: " << ENDL();
    #else
    uint32_t write_addr = cb_interface[cb_id].fifo_rd_ptr << 4;
    DPRINT << " UNPACKED VALUES: " << ENDL();
    #endif
    volatile tt_l1_ptr T* ptr = reinterpret_cast<volatile tt_l1_ptr T*>(write_addr);
    ptr += wt * 1024;
    for (uint16_t k = 0; k < 1024; k++) {
        if (k % 32 == 0) {
            DPRINT << ENDL();
        }
        DPRINT << SETW(6) << (T) (ptr[k]) << " ";
    }
    DPRINT << ENDL() << ENDL();
}
...
void MAIN {
    ...
    for(uint32_t ht = 0; ht < Ht; ++ht) {
        ...
        copy_tile_to_dst_init_short_with_dt(index_cb_index, input_transposed_cb_index);
