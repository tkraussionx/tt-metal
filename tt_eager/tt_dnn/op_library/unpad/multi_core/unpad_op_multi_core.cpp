...
inline __attribute__((always_inline)) void set_unpad_runtime_args_tile(
    ...
    if constexpr (initialize_args) {
        std::vector<uint32_t> reader_common_args(2 + num_dims * 2);
        uint32_t* num_unpadded_tiles_per_dim = reader_common_args.data() + 2;
    ...
    for (uint32_t i = 0, num_tiles_written = 0; i < num_cores_total; ++i) {
        ...
        if constexpr (initialize_args) {
            ...
            set_reader_rt_args(reader_kernel_args, num_unpadded_tiles_per_dim, num_padded_tiles_per_dim, num_tiles_per_core, start_offset, num_tiles_written);
