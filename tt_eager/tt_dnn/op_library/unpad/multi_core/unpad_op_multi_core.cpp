...
operation::ProgramWithCallbacks unpad_tile_multi_core_sharded(const Tensor &a, Tensor& output, const Shape &output_tensor_start, const Shape &output_tensor_end) {
...
operation::ProgramWithCallbacks unpad_multi_core(const Tensor &a, Tensor& output, const Shape &output_tensor_start, const Shape &output_tensor_end) {
    switch (a.get_layout()) {
        case Layout::ROW_MAJOR:
            // log_info("[xuncai] Unpad multi core RM");
            return unpad_rm_multi_core(a, output, output_tensor_start, output_tensor_end);
        case Layout::TILE:
            if (output.memory_config().is_sharded()){
                // log_info("[xuncai] Unpad multi core tile sharded");
                // log_info("[xuncai] a: {}", a.get_legacy_shape());
                // log_info("[xuncai] output: {}", output.get_legacy_shape());
                // log_info("[xuncai] output_tensor_start: {}", output_tensor_start);
                // log_info("[xuncai] output_tensor_end: {}", output_tensor_end);
                return unpad_tile_multi_core_sharded(a, output, output_tensor_start, output_tensor_end);
            } else{
                // log_info("[xuncai] Unpad multi core tile");
