...
const operation::Hash Unpad::compute_program_hash(const std::vector<Tensor> &input_tensors) const {
    ...
    auto input_mem_config = std::get<DeviceStorage>(input_tensor.tensor_attributes->storage).memory_config();
    auto output_mem_config = this->output_mem_config;
    auto dtype = input_tensor.tensor_attributes->dtype;
    auto num_dims = input_tensor.tensor_attributes->shape.rank();

    std::string rm_width = "TILE";
    if (input_tensor.get_layout() == Layout::ROW_MAJOR) {
        rm_width = fmt::format("{}", input_tensor.tensor_attributes->shape.value()[3]);
    }
