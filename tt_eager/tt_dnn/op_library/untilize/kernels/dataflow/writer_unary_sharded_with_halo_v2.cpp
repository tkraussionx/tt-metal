...
void kernel_main() {
    ...
    uint32_t local_pad_nsegments            = get_arg_val<uint32_t>(13);
    uint32_t local_data_src_start_offset    = get_arg_val<uint32_t>(14);
    uint32_t local_data_nsegments           = get_arg_val<uint32_t>(15);
    uint32_t ll_data_src_start_offset       = get_arg_val<uint32_t>(16);
    uint32_t ll_data_nsegments              = get_arg_val<uint32_t>(17);
    uint32_t l_data_src_start_offset        = get_arg_val<uint32_t>(18);
    uint32_t l_data_nsegments               = get_arg_val<uint32_t>(19);
    uint32_t r_data_src_start_offset        = get_arg_val<uint32_t>(20);
    uint32_t r_data_nsegments               = get_arg_val<uint32_t>(21);
    uint32_t rr_data_src_start_offset       = get_arg_val<uint32_t>(22);
    uint32_t rr_data_nsegments              = get_arg_val<uint32_t>(23);
    ...
    if (local_pad_nsegments > 0) {
        ...
        for (uint32_t i = 0; i < 2 * local_pad_nsegments; i += 2) {
    ...
    if (local_data_nsegments > 0) {
        ...
        for (uint32_t i = 0; i < 2 * local_data_nsegments; i += 2) {
    ...
    if (has_ll && ll_data_nsegments > 0) {
        ...
        push_to_neighbor_async(ll_noc_x, ll_noc_y, ll_data_ss_cb_id, ll_data_nsegments, in_base_l1_addr + ll_data_src_start_offset, out_base_l1_addr, stick_nbytes);
    }
    if (has_l && l_data_nsegments > 0) {
        cb_wait_front(l_data_ss_cb_id, 1);
        push_to_neighbor_async(l_noc_x, l_noc_y, l_data_ss_cb_id, l_data_nsegments, in_base_l1_addr + l_data_src_start_offset, out_base_l1_addr, stick_nbytes);
    }
    if (has_r && r_data_nsegments > 0) {
        cb_wait_front(r_data_ss_cb_id, 1);
        push_to_neighbor_async(r_noc_x, r_noc_y, r_data_ss_cb_id, r_data_nsegments, in_base_l1_addr + r_data_src_start_offset, out_base_l1_addr, stick_nbytes);
    }
    if (has_rr && rr_data_nsegments > 0) {
        cb_wait_front(rr_data_ss_cb_id, 1);
        push_to_neighbor_async(rr_noc_x, rr_noc_y, rr_data_ss_cb_id, rr_data_nsegments, in_base_l1_addr + rr_data_src_start_offset, out_base_l1_addr, stick_nbytes);
    }
