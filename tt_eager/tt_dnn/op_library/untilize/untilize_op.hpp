...
// NOTE: UntilizeWithHaloConcat is only for sharded input/output
struct UntilizeWithHaloConcat {
    const MemoryConfig output_mem_config;
...
Tensor untilize_with_halo_concat(const Tensor &a, const MemoryConfig& mem_config = operation::DEFAULT_OUTPUT_MEMORY_CONFIG);
