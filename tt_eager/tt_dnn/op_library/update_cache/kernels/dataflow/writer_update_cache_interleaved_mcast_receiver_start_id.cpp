...
void kernel_main() {
    ...
    // READER
    // mcast args
    const uint32_t mcast_sender_noc_x             = get_arg_val<uint32_t>(12);
    const uint32_t mcast_sender_noc_y             = get_arg_val<uint32_t>(13);
    ...
    constexpr uint32_t u_count = get_compile_time_arg_val(6); // user count

    // mcast args
    constexpr uint32_t mcast_sender_semaphore_addr    = get_compile_time_arg_val(7);
    constexpr uint32_t mcast_receiver_semaphore_addr  = get_compile_time_arg_val(8);
    ...
    const uint64_t mcast_sender_semaphore_noc_addr = get_noc_addr(mcast_sender_noc_x, mcast_sender_noc_y, mcast_sender_semaphore_addr);
    ...
    for (uint32_t h = 0; h < num_batched_heads; ++h) {
        ...
        cb_reserve_back(untilized_input_cb_id, Wt);
        ...
        cb_push_back(untilized_input_cb_id, Wt);
