...
operation::ProgramWithCallbacks update_cache_multi_core(const Tensor& cache_tensor, const Tensor &input_tensor, const uint32_t update_idx, const uint32_t batch_offset, DeviceComputeKernelConfig compute_kernel_config) {
    ...
    bool row_major;
    uint32_t num_cores, num_users_per_core_group_1, num_users_per_core_group_2;
    ...
    if (shard_spec.has_value()) {
        ...
        num_users_per_core_group_1 = shard_spec.value().shape[0] / TILE_HEIGHT;
        num_users_per_core_group_2 = 0;
        ...
        row_major = true;
        std::tie(num_cores, all_cores, core_group_1, core_group_2, num_users_per_core_group_1, num_users_per_core_group_2) = split_work_to_cores(compute_with_storage_grid_size, num_batched_heads, row_major);
    ...
    if (!core_group_2.ranges().empty()) {
        compute_kernel_args[6] = num_users_per_core_group_2;
    ...
    for (uint32_t i = 0, num_tiles_read = 0; i < num_cores; ++i) {
        ...
        if (i < g1_numcores) {
            num_batched_heads_per_core = num_users_per_core_group_1;
        } else {
            num_batched_heads_per_core = num_users_per_core_group_2;
        }
...
operation::ProgramWithCallbacks update_cache_multi_core_mcast(const Tensor& cache_tensor, const Tensor &input_tensor, const uint32_t update_idx, const uint32_t batch_offset, DeviceComputeKernelConfig compute_kernel_config) {
    log_info("Update cache mcast!!!!");
    ...
    log_info("cache_cb_data_format: {}", cache_cb_data_format);
    log_info("input_cb_data_format: {}", input_cb_data_format);
    log_info("interm_cb_data_format: {}", interm_cb_data_format);
    log_info("Wbytes: {}", Wbytes);
    log_info("Wt: {}", Wt);
    ...
    uint32_t num_users = input_tensor.get_legacy_shape()[-2];
    ...
    if (shard_spec.has_value()) {
        ...
        row_major = true;
        std::tie(num_cores, all_cores, core_group_1, core_group_2, num_users_per_core_group_1, num_users_per_core_group_2) = split_work_to_cores(compute_with_storage_grid_size, num_users, row_major);
        ...
        log_info("num_users_per_core_group_1:{}", num_users_per_core_group_1);
        log_info("num_users_per_core_group_2:{}", num_users_per_core_group_2);
        num_input_tiles = 2 * Wt; // double buffered
    }
    uint32_t src0_cb_index = CB::c_in0;
    uint32_t num_cache_tiles = 2 * granularity * Wt; // double buffered
    ...
    uint32_t input_start_id = 0;
    uint32_t batch_start_id = 0;
    uint32_t user_start_id = 0;
    uint32_t total_batched_heads = 0;
    std::vector<uint32_t> cache_start_ids;
    cache_start_ids.reserve(num_cores);
    for (uint32_t i = 0, num_tiles_read = 0; i < num_cores; ++i) {
        const CoreCoord &core = cores.at(i);
        input_start_id = user_start_id * Wt;
        batch_start_id = (user_start_id * TILE_HEIGHT) % B;

        // Batch Offset + Head Offset + Index Offset
        cache_start_id = batch_start_id * cache_batch_num_tiles + ((user_start_id * TILE_HEIGHT) / B) * cache_head_num_tiles;
        cache_start_ids.push_back(cache_start_id);
        cache_start_id += cache_tile_idx;
    ...
    for (uint32_t i = 0, num_tiles_read = 0; i < num_cores; ++i) {
        ...
        cache_start_ids.push_back(cache_start_id);
        cache_start_id += cache_tile_idx;
        log_info("total_batched_heads: {}", total_batched_heads);
        log_info("input_start_id: {}", input_start_id);
        log_info("batch_start_id: {}", batch_start_id);
        log_info("cache_start_id: {}", cache_start_id);
        SetRuntimeArgs(
            program,
            unary_reader_kernel_id,
            core,
