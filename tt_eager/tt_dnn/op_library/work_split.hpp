...
inline std::set<CoreRange> num_cores_to_corerange_set(const CoreCoord start_core, const uint32_t target_num_cores, const CoreCoord grid_size, const bool row_wise = false) {
	...
	TT_FATAL(target_num_cores <= total_available_cores, "Target number of cores is greater than total number of available cores target: {} total: {}", target_num_cores, total_available_cores);
