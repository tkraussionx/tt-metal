...
void py_module(py::module& m_transformers) {
    ...
    m_transformers.def(
        "attn_matmul_from_cache",
        &attn_matmul_from_cache,
        py::arg().noconvert(),
        py::arg().noconvert(),
        py::arg("num_tokens").noconvert(),
        py::arg("transpose_hw").noconvert(),
        py::arg("compute_with_storage_grid_size").noconvert(),
        py::arg("output_mem_config").noconvert() = operation::DEFAULT_OUTPUT_MEMORY_CONFIG,
        py::arg("output_dtype").noconvert() = std::nullopt,
        py::arg("compute_kernel_config").noconvert() = std::nullopt,
    ...
    py::class_<SoftmaxDefaultProgramConfig>(m_transformers, "SoftmaxDefaultProgramConfig").def(py::init<>());
    ...
    py::class_<SDPADefaultProgramConfig>(m_transformers, "SDPADefaultProgramConfig").def(py::init<>());
    ...
    m_transformers.def(
        ...
        "Accepts a `SDPAMultiCoreProgramConfig` which specifies the grid size and chunk tiles in the Q and K sequence "
        "lengths. The op parallelizes over `b`, `nqh`, and Q's `s` dimension.");
