...
void bind_op_with_mem_config_and_dtype_and_opt_output(py::module_ &module, std::string op_name, Func &&f, std::string docstring, Extra&&... extra) {
    if constexpr (mem_config_arg && dtype_arg && opt_output_arg) {
        ...
        module.def(op_name.c_str(), f,
            std::forward<Extra>(extra)..., py::arg(mem_config_name.c_str()).noconvert() = operation::DEFAULT_OUTPUT_MEMORY_CONFIG, py::arg(dtype_name.c_str()).noconvert() = std::nullopt, py::arg(opt_output_tensor_name.c_str()).noconvert() = default_opt_output_tensor, docstring.c_str()
        );
        ...
        module.def(op_name.c_str(), f,
            std::forward<Extra>(extra)..., py::arg(opt_output_tensor_name.c_str()).noconvert() = default_opt_output_tensor, docstring.c_str()
        );
    } else {
        module.def(op_name.c_str(), f,
