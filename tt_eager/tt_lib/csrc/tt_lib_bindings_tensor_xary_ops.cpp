...
namespace tt::tt_metal::detail {
    ...
    detail::bind_unary_op_with_param(
        ...
        m_tensor.def("bitwise_not",bitwise_not,
            py::arg("input").noconvert(),py::arg("value"),py::arg("output_mem_config").noconvert() = operation::DEFAULT_OUTPUT_MEMORY_CONFIG,R"doc(
            Computes bitwise_not of input tensor ``input``. Input tensor needs to be in the range [-32767, 2147483583]. Support provided only for Wormhole_B0.

            Input tensor must have INT32 data type.

            Output tensor will have INT32 data type for positive input range and INT16 data type for negative input range.
