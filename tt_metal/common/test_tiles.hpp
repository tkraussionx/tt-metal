...
enum TensorLayout {
    ...
    CHANNELS_LAST = 3,
    TILED32_SWIZZLED_CL = 4, // row major order of tiles 32x32, tiled across channel-width dimension, each tile is row-major-swizzled
    TILED32_4FACES_CL = 5, // rowm major order of tiles 32x32, tiled across channel-width dimension, each tile is 4 faces, each face is row-major, faces are swizzled
};
...
// Converts a 32-swizzled tilized channel last tensor to a linear 32-zero-padded channel last tensor
template<typename T, template<typename> typename BufferType>
inline std::vector<T> untilize_channels_last(const BufferType<T>& in, const std::vector<std::uint32_t>& shape) {

    TT_ASSERT(shape.size() == 4);
    TT_ASSERT(shape[3] % 32 == 0 && shape[1] % 32 == 0);

    std::vector<T> result;
    // Untilize into channels last
    // Since shape is channels last, in the following code,
    // C represents height
    // H represents width
    // W represents channel depth
    int N = shape[0], C = shape[2], H = shape[3], W = shape[1];

    result.resize(N*C*H*W);
    uint32_t linear = 0;
    for (int n = 0; n < N; n++)
    for (int c = 0; c < C; c++)
    for (int hs32 = 0; hs32 < H; hs32 += 32) // iterate over width dim with stride 32
    for (int ws32 = 0; ws32 < W; ws32 += 32) // iterate over channel dim with stride 32
    for (int h32 = 0; h32 < 32; h32++) // hs32 + h32 = h
    for (int w32 = 0; w32 < 32; w32++) { // ws32 + w32 = w
        T val = in[linear];
        auto w = w32 + ws32;
...
// Converts a linear non-zero-padded channels last tensor to zero-padded-32 32-swizzled tilized channels last tensor
template<typename T, template<typename> typename BufferType>
inline std::vector<T> tilize_channels_last(const BufferType<T>& in_rowmajor, const std::vector<std::uint32_t>& shape) {
    // Since the buffer is in channels last layout, in the follwing code,
    // C represents height
    // H represents width
    // W represents channel depth
    int N = shape[0], C = shape[2], H = shape[3], W = shape[1];
...
inline vector<T> convert_layout(const BufferType<T>& inp, const vector<uint32_t>& shape, TensorLayout inL, TensorLayout outL) {
    switch (inL) {
        ...
        case CHANNELS_LAST:
            ...
            } else if (outL == TILED32_SWIZZLED_CL) {
                return tilize_channels_last<T>(inp, shape);
            } else if (outL == TILED32_4FACES_CL) {
                auto swiz32 = convert_layout<T>(inp, shape, inL, TILED32_SWIZZLED_CL);
                return convert_layout<T>(swiz32, shape, TILED32_SWIZZLED_CL, outL);
            } else {
                TT_ASSERT(false && "Unsupported conversion");
            }
            break;
        case TILED32_SWIZZLED_CL:
            if (outL == TILED32_4FACES_CL) {
                return convert_to_tile_layout<T>(inp);
            } else if (outL == CHANNELS_LAST) {
                return untilize_channels_last<T>(inp, shape);
            } else {
                TT_ASSERT(false && "Unsupported conversion.");
            }
            break;
        case TILED32_4FACES_CL:
            if (outL == TILED32_SWIZZLED_CL) {
                return convert_to_flat_layout<T>(inp);
            } else if (outL == CHANNELS_LAST) {
                auto swiz32 = convert_layout<T>(inp, shape, inL, TILED32_SWIZZLED_CL);
                return untilize_channels_last<T>(swiz32, shape);
            } else {
                TT_ASSERT(false && "Unsupported conversion");
            }
            break;
