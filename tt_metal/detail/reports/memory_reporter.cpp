...
void write_headers(std::ofstream &memory_usage_summary_report, std::ofstream &l1_usage_summary_report, bool add_program_id) {
    ...
    l1_usage_summary_report << ", Largest Contiguous Free Block (KB), Total Free L1 Space (KB)\n";
    memory_usage_summary_report << ", Total Allocatable Size (KB), Total Allocated (KB), Total Free (KB), Largest Free Block (KB)\n";
}
...
void write_memory_usage(
    ...
    if (buffer_type == BufferType::L1) {
        l1_usage_summary_report << ","
                        << stats.largest_free_block_bytes / 1024 << ","
                        << (stats.largest_free_block_bytes * num_banks) / 1024 << "\n";
    }
}
