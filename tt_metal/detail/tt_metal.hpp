                ...
                for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(device->id())) {
                    ...
                    for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                        ...
                        std::vector<uint32_t> producer_compile_args = {
                            host_issue_queue_read_ptr_addr, issue_queue_start_addr, issue_queue_size, command0_l1_addr, data_section_l1_addr, get_producer_data_buffer_size(false), consumer_cmd_base_addr, consumer_data_buff_size};
