...
template <std::uint32_t block_ct_dim = 8, std::uint32_t full_ct_dim = block_ct_dim>
inline void llk_pack_untilize(const std::uint32_t block_rt_dim, const std::uint32_t output, const std::uint32_t face_r_dim = FACE_R_DIM, const std::uint32_t num_faces = 4, const std::uint32_t block_c_index = 0, uint32_t out_index = 0) {
    ...
    // offset to the given page index
    pack_tile_addr += cb_interface[output_id].fifo_page_size * out_index;
