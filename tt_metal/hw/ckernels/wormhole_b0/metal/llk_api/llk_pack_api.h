...
void __attribute__ ((noinline)) llk_pack_mop_config(const uint32_t output) {
...
void __attribute__ ((noinline))  llk_pack_hw_configure(const llk_pack_params_t *pack_params) {
...
void __attribute__ ((noinline)) llk_pack_hw_configure_disaggregated(std::uint32_t pack_output) {
...
void __attribute__ ((noinline)) llk_pack_reduce_hw_configure(const llk_pack_params_t *pack_params) {
    const std::uint32_t output_id = get_output_id(pack_params->pack_output);
    const std::uint32_t face_r_dim = get_output_face_r_dim(output_id);
    const std::uint32_t num_faces = get_output_num_faces(output_id);
    const bool partial_face = get_output_partial_face(output_id);
...
void __attribute__ ((noinline)) llk_pack_reduce_hw_configure_disaggregated(std::uint32_t pack_output) {
    llk_pack_params_t llk_pack_params = {
        .pack_output = pack_output, .relu_config = {.f = {.ApplyRelu = (std::uint32_t)relu_type, .Threshold = relu_threshold}}};
...
template <
    ...
    bool tilize = false/*unused*/>
void __attribute__ ((noinline)) llk_pack_init(const std::uint32_t pack_output = 16) {
...
void __attribute__ ((noinline)) llk_pack(std::uint32_t tile_index, std::uint32_t output, std::uint32_t output_tile_index = 0) {
...
void __attribute__ ((noinline)) llk_pack_untilize_init(std::uint32_t output, const std::uint32_t face_r_dim = FACE_R_DIM, const std::uint32_t num_faces = 4) {
    const std::uint32_t output_id = get_output_id(output);
...
template <std::uint32_t block_ct_dim = 8, std::uint32_t full_ct_dim = block_ct_dim, bool diagonal = false, bool narrow_row = false, std::uint32_t row_num_datums = TILE_C_DIM>
void __attribute__ ((noinline)) llk_pack_untilize(std::uint32_t block_rt_dim, std::uint32_t output, const std::uint32_t face_r_dim = FACE_R_DIM, const std::uint32_t num_faces = 4, const std::uint32_t block_c_index = 0) {
...
template <bool out_of_order_output = false, bool untilize = false, bool is_fp32_dest_acc_en = false>
void __attribute__ ((noinline)) llk_matmul_pack(std::uint32_t start_tile_index, std::uint32_t output, uint32_t ntiles, std::uint32_t output_tile_index = 0) {
    std::uint8_t output_id = get_output_id(output);
...
void __attribute__ ((noinline)) llk_packer_wait_for_math_done() {
    _llk_packer_wait_for_math_done_();
}

template <uint WaitRes = p_stall::NONE>
void __attribute__ ((noinline)) llk_packer_set_math_semaphore() {
    _llk_packer_set_math_semaphore_<WaitRes>();
}

template <bool is_fp32_dest_acc_en = false>
void __attribute__ ((noinline)) llk_pack_dest_section_done() {
    _llk_pack_dest_section_done_<DstSync::SyncHalf, is_fp32_dest_acc_en>();
}

template <bool untilize = false, bool diagonal = false>
void __attribute__ ((noinline)) llk_init_packer_dest_offset_registers(const std::uint32_t pack_output = 16) {
    const std::uint32_t output_id = get_output_id(pack_output);
    const std::uint32_t face_r_dim = get_output_face_r_dim(output_id);
    const bool narrow_tile = get_output_narrow_tile(output_id);
...
template <bool untilize = false, bool is_fp32_dest_acc_en = false>
void __attribute__ ((noinline)) llk_pack_dest_init(const std::uint32_t pack_output = 16) {
...
void __attribute__ ((noinline)) llk_pack_get_tile(std::uint32_t output, std::uint32_t tile_index, std::uint32_t *p_tile) {
    _llk_pack_get_tile_<mail2math, mail2pack>(tile_index, p_tile);
}

template <bool mail2math=true, bool mail2pack=true>
void __attribute__ ((noinline)) llk_pack_release_tile(std::uint32_t output) {
    _llk_pack_release_tile_<mail2math, mail2pack>();
}

void __attribute__ ((noinline)) llk_pack_debug_dump(std::uint8_t *data, std::uint32_t byte_size) {
    _llk_pack_debug_dump_(data, byte_size);
}

void __attribute__ ((noinline)) llk_pack_debug_dump_seek(std::uint8_t offset) {
    _llk_pack_debug_dump_seek_(offset);
}

template <bool is_fp32_dest_acc_en = false, bool is_tile_dim_reconfig_en = false>
void __attribute__ ((noinline)) llk_pack_reconfig_data_format(const std::uint32_t new_output) {
...
template <bool is_fp32_dest_acc_en = false, bool is_tile_dim_reconfig_en = false>
void __attribute__ ((noinline)) llk_pack_reconfig_data_format(const std::uint32_t old_output, const std::uint32_t new_output) {
...
void __attribute__ ((noinline)) llk_pack_reconfig_l1_acc(const std::uint32_t enable) {
    _llk_pack_reconfig_l1_acc_(enable);
}

template <bool untilize = false, ReduceDim dim>
void __attribute__ ((noinline)) llk_pack_reduce_mask_config() {
    _llk_pack_reduce_mask_config_<untilize, dim>();
}

void __attribute__ ((noinline)) llk_pack_reduce_mask_clear() {
    _llk_pack_reduce_mask_clear_();
}

// FIXME-WH-UPLIFT
template <ReduceDim dim, bool at_kernel_start = false, bool revert=false, bool is_fp32_dest_acc_en = false>
void __attribute__ ((noinline)) llk_pack_reduce_config_v2(uint32_t icb_out) {
