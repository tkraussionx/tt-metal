...
inline void llk_setup_operands() {
    ...
    for (uint32_t cb_id = 0; cb_id < NUM_CIRCULAR_BUFFERS; cb_id++) {
        ...
        *(dbg_dump+1) = 0;
        // *(dbg_dump+5) = (uint32_t)(&cb_interface[0]);
...
inline void llk_wait_tiles(int operand, std::int32_t num_tiles) {
    ...
    *(dbg_dump+5) = (uint32_t)cb_interface[input].fifo_rd_ptr;
    *(dbg_dump+6) = (uint32_t)cb_interface[input].fifo_size;
    *(dbg_dump+7) = (uint32_t)cb_interface[input].fifo_limit;
    *(dbg_dump+8) = (uint32_t)cb_interface[input].tiles_acked;
    *(dbg_dump+9) = (uint32_t)cb_interface[input].fifo_page_size;
