...
DeviceAddr get_unreserved_base_address(const HalMemType &mem_type) {
    switch (mem_type) {
        case HalMemType::DRAM:
            return static_cast<DeviceAddr>(DRAM_BARRIER_BASE + DRAM_BARRIER_SIZE);
        case HalMemType::L1:
            return static_cast<DeviceAddr>(((L1_KERNEL_CONFIG_BASE + L1_KERNEL_CONFIG_SIZE - 1) | (ALLOCATOR_ALIGNMENT - 1)) + 1);
        default: {
...
void init_one_bank_per_channel(Allocator &allocator, const AllocatorConfig &alloc_config) {
    // TODO (#11830): this should be based off of the HAL
    DeviceAddr alloc_start_offset_bytes = get_unreserved_base_address(HalMemType::DRAM);
    // DRAM bank is between unreserved start and trace_region start: UNRESERVED | DRAM BANK | TRACE REGION
    DeviceAddr dram_bank_size = alloc_config.dram_bank_size - alloc_start_offset_bytes - alloc_config.trace_region_size;
    ...
    allocator.dram_manager =
        BankManager(BufferType::DRAM, bank_offsets, dram_bank_size, ALLOCATOR_ALIGNMENT, alloc_start_offset_bytes);
    ...
    allocator.trace_buffer_manager = BankManager(
        ...
        ALLOCATOR_ALIGNMENT,
        dram_bank_size + alloc_start_offset_bytes);
    for (uint32_t bank_id = 0; bank_id < alloc_config.num_dram_channels; bank_id++) {
        CoreCoord logical_core = CoreCoord{bank_id, 0};
        allocator.bank_id_to_dram_channel.insert({bank_id, bank_id});
        allocator.dram_channel_to_bank_ids.insert({bank_id, {bank_id}});
...
void init_one_bank_per_l1(Allocator &allocator, const AllocatorConfig &alloc_config) {
    ...
    DeviceAddr offset_bytes = static_cast<DeviceAddr>(get_unreserved_base_address(HalMemType::L1));
    DeviceAddr l1_bank_size = alloc_config.worker_l1_size - offset_bytes;
