...
std::vector< std::vector<uint32_t> > core_to_host_pages(std::vector<CoreCoord> all_cores, int pages_per_shard, int num_shards,
                                                    TensorMemoryLayout layout, std::array<uint32_t, 2> page_shape,
...
Buffer::Buffer(Device *device, uint64_t size, uint64_t page_size, const BufferType buffer_type,
    ...
    if(is_sharded(buffer_layout)){
        ...
        all_cores_ = corerange_to_cores(shard_parameters.value().shard_grid, row_major);
        core_host_page_indices_ = core_to_host_pages(all_cores_, shard_size(), all_cores_.size(), buffer_layout, page_shape(), shard_shape(), tensor2d_size());
        core_bank_indices_.reserve(all_cores_.size());

        auto total_dev_pages = all_cores_.size() * shard_size();
...
void Buffer::allocate() {
    ...
    if(is_sharded(this->buffer_layout_)){
        this->address_ = allocator::allocate_buffer(*this->device_->allocator_, this->size_, this->page_size_ * this->shard_size(), this->buffer_type_, bottom_up);
    }
    else{
