...
void Device::compile_command_queue_programs() {
    ...
    if (this->is_mmio_capable()) {
        for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(this->id())) {
            ...
            for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                ...
                if (device_id == this->id()) {
                    ...
                    CoreCoord tunneler_physical_core = this->ethernet_core_from_logical_core(tunneler_location);
                    ...
                    tt::tt_metal::CreateSemaphore(*command_queue_program_ptr, prefetch_location, 0); // prefetch_sync_sem
                    tt::tt_metal::CreateSemaphore(*command_queue_program_ptr, prefetch_location, dispatch_constants::PREFETCH_D_BUFFER_PAGES); // prefetch_downstream_cb_sem
                    tt::tt_metal::CreateSemaphore(*command_queue_program_ptr, prefetch_location, 0);

                    tt::tt_metal::CreateSemaphore(*command_queue_program_ptr, mux_location, 0); // unused mux semaphore
                    tt::tt_metal::CreateSemaphore(*command_queue_program_ptr, mux_location, 0); // mux_upstream_cb_sem

                    tt_metal::CreateSemaphore(*command_queue_program_ptr, demux_location, 0); // unused
                    tt_metal::CreateSemaphore(*command_queue_program_ptr, demux_location, 0); // unused
                    // for the unpacketize stage, we use rptr/wptr for flow control, and poll semaphore
                    // value only to update the rptr:
                    tt_metal::CreateSemaphore(*command_queue_program_ptr, demux_location, 0);

                    constexpr uint32_t dispatch_h_cb_sem = 0;
                    tt_metal::CreateSemaphore(*command_queue_program_ptr, dispatch_location, 0);
        ...
        log_debug(LogDevice, "run tunneler at device {} Core {}", std::get<0>(connected_eth_core), r_tunneler_logical_core.str());
    ...
    if (this->is_mmio_capable()) {
        for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(this->id())) {
            ...
            for (uint8_t cq_id = 0; cq_id < curr_num_hw_cqs; cq_id++) {
                ...
                if (device_id != this->id()) {
                    tt_cxy_pair dispatch_d_location = dispatch_core_manager::get(curr_num_hw_cqs).dispatcher_d_core(device_id, curr_channel, cq_id);
        ...
        for (const chip_id_t &device_id : tt::Cluster::instance().get_devices_controlled_by_mmio_device(this->id_)) {
            ...
            for (uint8_t cq_id = 0; cq_id < curr_num_hw_cqs; cq_id++) {
                if (device_id == this->id_) {
                    ...
                    log_info(tt::LogMetal, "MMIO Device dispatch cores: {} : {}", dispatch_location.str(), prefetch_location.str());
                } else {
                    tt_cxy_pair dispatch_location = dispatch_core_manager::get(curr_num_hw_cqs).dispatcher_core(device_id, curr_channel, cq_id);
                    tt_cxy_pair prefetch_location = dispatch_core_manager::get(curr_num_hw_cqs).prefetcher_core(device_id, curr_channel, cq_id);
