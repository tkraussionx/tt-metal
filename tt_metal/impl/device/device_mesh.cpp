    ...
    : device_grid(device_grid)
{
    ...
    DeviceIds sorted_ids = device_ids;
    ...
    std::cout << "cluster_tunnel_depth: " << cluster_tunnel_depth << " cluster_tunnel_count: " << cluster_tunnel_count << " num_mmio_devices: " << num_mmio_devices << std::endl;
    ...
    for (int mmio_device_id = 0; mmio_device_id < num_mmio_devices; mmio_device_id++) {
        ...
        for (uint32_t t = 0; t < tunnels_from_mmio.size(); t++) {
            ...
            int col_idx = 0;
            std::size_t start = (this->is_galaxy_) ? 1 : 0; // this is because we skip the gateway wormhole device.
            for (uint32_t ts = start; ts < tunnels_from_mmio[t].size(); ts++) {
    ...
    tt::log_info("galaxy_device_ids: {}", galaxy_device_ids);
...
void DeviceMesh::enable_async(bool enable) {
    for (auto& [device_id, device] : mesh_devices) {
