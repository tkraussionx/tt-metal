...
class MeshDevice;
using MeshShape = std::pair<std::size_t, std::size_t>;
...
public:
    ...
    [[nodiscard]] device_pointer get_device(std::size_t row, std::size_t col);
    [[nodiscard]] const_device_pointer get_device(std::size_t row, std::size_t col) const;
...
namespace std {
    ...
    struct hash<tt::tt_metal::Coordinate> {
        std::size_t operator()(const tt::tt_metal::Coordinate& coord) const noexcept {
            std::size_t seed = 0;
