...
void EnqueueProgramCommand::assemble_device_commands(
    ...
    if (not is_cached) {
        ...
        cached_program_command_sequence.program_config_buffer_data_size = program_command_sequence.write_offset_bytes();
...
void EnqueueProgramCommand::process() {
    ...
    uint32_t program_config_buffer_data_size = cached_program_command_sequence.program_config_buffer_data_size;

    uint32_t program_rem_fetch_size_bytes = program_fetch_size_bytes - program_config_buffer_data_size;
    ...
    if (total_fetch_size_bytes <= dispatch_constants::get(dispatch_core_type).max_prefetch_command_size()) {
        ...
        if (not stall_first) {
            this->manager.cq_write(program_command_sequence_data, program_config_buffer_data_size, write_ptr);
            program_command_sequence_data += program_config_buffer_data_size;
            write_ptr += program_config_buffer_data_size;
        ...
        if (not stall_first) {
            this->manager.issue_queue_reserve(program_config_buffer_data_size, this->command_queue_id);
            write_ptr = this->manager.get_issue_queue_write_ptr(this->command_queue_id);
            this->manager.cq_write(program_command_sequence_data, program_config_buffer_data_size, write_ptr);
            this->manager.issue_queue_push_back(program_rem_fetch_size_bytes, this->command_queue_id);
            this->manager.fetch_queue_reserve_back(this->command_queue_id);
            this->manager.fetch_queue_write(program_config_buffer_data_size, this->command_queue_id);
            program_command_sequence_data += program_config_buffer_data_size;
