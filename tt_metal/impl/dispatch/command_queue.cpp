...
ProgramCommandSequence EnqueueProgramCommand::generate_program_command_sequence(std::vector<ConfigBufferEntry>& kernel_config_addrs) {
    ...
    return program_command_sequence;
}

void EnqueueProgramCommand::write_program_command_sequence(bool stall_first, const ProgramCommandSequence& program_command_sequence) {
...
void EnqueueProgramCommand::process() {
    ...
    bool use_cache = this->device->program_command_cache.is_enabled();
    bool is_cached = use_cache && this->device->program_command_cache.contains(program.id);
    ...
    if (!is_cached) {
        ...
        if (use_cache) {
            ...
            this->device->program_command_cache.insert(program.id, std::move(program_command_sequence));
        }
    } else {
        ...
        auto& cached_program_command_sequence = this->device->program_command_cache.get(program.id);
        ...
        this->write_program_command_sequence(stall_first, cached_program_command_sequence);
    }
}
