...
void EnqueueProgramCommand::assemble_device_commands(
    ...
    if (not is_cached) {
        ...
        if (unicast_go_signal_sub_cmds.size() > 0) {
            ...
            for (const auto& [num_sub_cmds_in_cmd, unicast_go_signal_payload_sizeB] : unicast_go_signals_payload) {
                ...
                program_command_sequence.add_dispatch_write_packed<CQDispatchWritePackedUnicastSubCmd>(
                    ...
                    curr_sub_cmd_idx);
                curr_sub_cmd_idx += num_sub_cmds_in_cmd;
                cached_program_command_sequence.unicast_launch_msg_write_packed_cmd_ptrs.push_back(&((CQDispatchCmd*) ((uint32_t*)program_command_sequence.data() + (write_offset_bytes + sizeof(CQPrefetchCmd)) / sizeof(uint32_t)))->write_packed);
                uint32_t curr_sub_cmd_data_offset_words =
                    (write_offset_bytes + (sizeof(CQPrefetchCmd) + sizeof(CQDispatchCmd)) +
        ...
        uint32_t write_offset_bytes = program_command_sequence.write_offset_bytes();
        program_command_sequence.add_dispatch_go_signal_mcast(this->expected_num_workers_completed, go_signal_mcast_flag, *reinterpret_cast<uint32_t*>(&run_program_go_signal), DISPATCH_MESSAGE_ADDR, dispatcher_for_go_signal);
...
void HWCommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    auto command = EnqueueProgramCommand(
        ...
        this->device->worker_launch_message_buffer_state.get_multicast_cores_launch_message_wptr(),
        this->device->worker_launch_message_buffer_state.get_unicast_cores_launch_message_wptr());
    // Update wptrs for tensix and eth launch message in the device class
    if (program.runs_on_noc_multicast_only_cores()) {
        this->device->worker_launch_message_buffer_state.increment_multicast_cores_launch_message_wptr(1);
    }
    if (program.runs_on_noc_unicast_only_cores()) {
        this->device->worker_launch_message_buffer_state.increment_unicast_cores_launch_message_wptr(1);
    }
    this->enqueue_command(command, blocking);
...
void HWCommandQueue::enqueue_trace(const uint32_t trace_id, bool blocking) {
    ...
    this->device->worker_launch_message_buffer_state.set_multicast_cores_launch_message_wptr(trace_inst->desc->num_traced_programs_needing_go_signal_multicast);
    this->device->worker_launch_message_buffer_state.set_unicast_cores_launch_message_wptr(trace_inst->desc->num_traced_programs_needing_go_signal_unicast);
...
void HWCommandQueue::record_begin(const uint32_t tid, std::shared_ptr<detail::TraceDescriptor> ctx) {
    ...
    this->multicast_cores_launch_message_wptr_reset = this->device->worker_launch_message_buffer_state.get_multicast_cores_launch_message_wptr();
    this->unicast_cores_launch_message_wptr_reset = this->device->worker_launch_message_buffer_state.get_unicast_cores_launch_message_wptr();
...
void HWCommandQueue::record_end() {
    ...
    this->device->worker_launch_message_buffer_state.set_multicast_cores_launch_message_wptr(this->multicast_cores_launch_message_wptr_reset);
    this->device->worker_launch_message_buffer_state.set_unicast_cores_launch_message_wptr(this->unicast_cores_launch_message_wptr_reset);
