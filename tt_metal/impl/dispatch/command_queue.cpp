...
const DeviceCommand EnqueueReadBufferCommand::assemble_device_command(uint32_t dst_address) {
    ...
    if (tt::Cluster::instance().arch() == tt::ARCH::WORMHOLE_B0) {
        if (route_through_ethernet) {
            ...
            uint32_t producer_consumer_multiple = get_cq_data_buffer_size(dispatch_on_ethernet, true) / router_cb_size;
...
const DeviceCommand EnqueueWriteBufferCommand::assemble_device_command(uint32_t src_address) {
    ...
    if (tt::Cluster::instance().arch() == tt::ARCH::WORMHOLE_B0) {
        ...
        if (route_through_ethernet) {
            ...
            uint32_t producer_consumer_multiple = get_cq_data_buffer_size(dispatch_on_eth, true) / router_cb_size;
            if (producer_consumer_multiple != 1) {
                producer_consumer_multiple = producer_consumer_multiple / 2 * 2; // To ensure that our CB can broken up into 2
            }
