...
void generate_dispatch_write_packed(
    ...
    std::vector<std::array<uint32_t, 4>>& cmd_mapping,
    ...
    while (num_packed_cmds_in_seq != 0) {
        ...
        cmd_mapping.emplace_back(std::array<uint32_t, 4>{
            (uint32_t)runtime_args_command_sequences.size() - 1,
            num_packed_cmds,
            (uint32_t)get_runtime_args_data_offset(num_packed_cmds, max_runtime_args_len, unicast),
            align(max_runtime_args_len * sizeof(uint32_t), L1_ALIGNMENT)});
    ...
    if (this->runtime_args_command_sequences.count(program.id) == 0) {
        ...
        for (size_t kernel_id = 0; kernel_id < program.num_kernels(); kernel_id++) {
            ...
            if (!kernel->cores_with_runtime_args().empty()) {
                program.command_indices.unique_processors.insert(processor_idx);
                ...
                for (const auto& core_coord : kernel->cores_with_runtime_args()) {
                    ...
                    unique_rt_data_and_sizes[processor_idx].emplace_back(runtime_args_data.data(), runtime_args_data.size() * sizeof(uint32_t));
            ...
            if (common_rt_args.size() > 0) {
                program.command_indices.common_kernels.insert(kernel_id);
                ...
                if (kernel->get_kernel_core_type() == CoreType::ETH) {
                    ...
                    for (auto& core_coord : kernel->logical_cores()) {
                        ...
                        unicast_sub_cmd.emplace_back(
                            CQDispatchWritePackedUnicastSubCmd{.noc_xy_addr = unicast_noc_encoding});
                        common_rt_data_and_sizes[kernel_id].emplace_back(common_rt_args.data(), common_rt_args.size() * sizeof(uint32_t));
                    }
                } else {
        ...
        this->runtime_args_command_sequences[program.id].reserve(
            2 * (program.command_indices.unique_processors.size() + program.command_indices.common_kernels.size()));
        std::vector<std::pair<uint32_t, uint32_t>> runtime_args_data_index;
        runtime_args_data_index.reserve(
            2 * (program.command_indices.unique_processors.size() + program.command_indices.common_kernels.size()));
        ...
        // same for all splits
        for (uint32_t processor_idx : program.command_indices.unique_processors) {
            generate_dispatch_write_packed(
                this->runtime_args_command_sequences[program.id],
                program.command_indices.processor_to_cmd_mapping[processor_idx],
        ...
        for (uint32_t kernel_id : program.command_indices.common_kernels) {
            std::visit(
                [&](auto&& sub_cmds) {
                    generate_dispatch_write_packed(
                        this->runtime_args_command_sequences[program.id],
                        program.command_indices.kernel_to_cmd_mapping[kernel_id],
        ...
        // TODO: Currently looping and updating cores with un-updated runtime args
        // we should directly update cmd vector from SetRuntimeArgs
        // array represents current cmd idx, current subcmd idx, curr data offset
        std::vector<std::array<uint32_t, 3>> unicast_data_idx(unique_processor_to_l1_arg_base_addr.size());
        std::vector<std::array<uint32_t, 3>> multicast_data_idx(program.num_kernels());

        for (const auto& processor_idx : program.command_indices.unique_processors) {
            unicast_data_idx[processor_idx] = {
                0, 0, program.command_indices.processor_to_cmd_mapping[processor_idx][0][2]};
        }
        for (const auto& kernel_id : program.command_indices.common_kernels) {
            multicast_data_idx[kernel_id] = {0, 0, program.command_indices.kernel_to_cmd_mapping[kernel_id][0][2]};
        }
        for (size_t kernel_id = 0; kernel_id < program.num_kernels(); kernel_id++) {
            auto kernel = detail::GetKernel(program, kernel_id);
            ...
            if (!kernel->cores_with_runtime_args().empty()) {
                ...
                uint32_t& processor_cmd_idx = unicast_data_idx[processor_idx][0];
                uint32_t& data_start = unicast_data_idx[processor_idx][2];
                for (const auto& core_coord : kernel->cores_with_runtime_args()) {
                    if (unicast_data_idx[processor_idx][1] ==
                        program.command_indices.processor_to_cmd_mapping[processor_idx][processor_cmd_idx][1]) {
                        unicast_data_idx[processor_idx][0]++;
                        processor_cmd_idx = unicast_data_idx[processor_idx][0];
                        unicast_data_idx[processor_idx][1] = 0;
                        unicast_data_idx[processor_idx][2] =
                            program.command_indices.processor_to_cmd_mapping[processor_idx][processor_cmd_idx][2];
                    }
                    uint32_t& data_start = unicast_data_idx[processor_idx][2];
                    const auto& runtime_args_data = kernel->runtime_args(core_coord);
                    this->runtime_args_command_sequences[program.id][program.command_indices.processor_to_cmd_mapping
                                                                         [processor_idx][processor_cmd_idx][0]]
                        .update_cmd_sequence(
                            data_start, runtime_args_data.data(), runtime_args_data.size() * sizeof(uint32_t));
                    data_start += program.command_indices.processor_to_cmd_mapping[processor_idx][processor_cmd_idx][3];
                    unicast_data_idx[processor_idx][1]++;
                }
            }

            const auto& common_rt_args = kernel->common_runtime_args();
            if (common_rt_args.size() > 0) {
                uint32_t& kernel_cmd_idx = multicast_data_idx[kernel_id][0];
                for (uint32_t i = 0; i < kernel->logical_coreranges().size(); ++i) {
                    if (multicast_data_idx[kernel_id][1] ==
                        program.command_indices.kernel_to_cmd_mapping[kernel_id][kernel_cmd_idx][1]) {
                        multicast_data_idx[kernel_id][0]++;
                        kernel_cmd_idx = multicast_data_idx[kernel_id][0];
                        multicast_data_idx[kernel_id][1] = 0;
                        multicast_data_idx[kernel_id][2] =
                            program.command_indices.kernel_to_cmd_mapping[kernel_id][kernel_cmd_idx][2];
                    }
                    uint32_t& data_start = multicast_data_idx[kernel_id][2];
                    this->runtime_args_command_sequences
                        [program.id][program.command_indices.kernel_to_cmd_mapping[kernel_id][kernel_cmd_idx][0]]
                            .update_cmd_sequence(
                                data_start, common_rt_args.data(), common_rt_args.size() * sizeof(uint32_t));
                    data_start += program.command_indices.kernel_to_cmd_mapping[kernel_id][kernel_cmd_idx][3];
                }
            }
        }
    }
...
void * convert_interleaved_to_sharded_on_host(const void* host, const Buffer& buffer, const BufferPageMapping &buffer_page_mapping) {
