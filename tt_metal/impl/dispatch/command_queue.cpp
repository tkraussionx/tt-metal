...
const DeviceCommand EnqueueReadBufferCommand::assemble_device_command(uint32_t dst_address) {
    ...
    uint32_t tensix_consumer_data_buffer_size = get_tensix_consumer_data_buffer_size();
    uint32_t consumer_cb_num_pages;
    // Targeting fast dispatch on remote device means commands have to be tunneled through ethernet
    bool route_through_ethernet = not device->is_mmio_capable();
    if (route_through_ethernet) {
        uint32_t router_data_buffer_size = get_router_data_buffer_size();
...
const DeviceCommand EnqueueProgramCommand::assemble_device_command(uint32_t host_data_src) {
    ...
    const uint32_t producer_cb_num_pages = get_tensix_producer_data_buffer_size() / DeviceCommand::PROGRAM_PAGE_SIZE;
    const uint32_t producer_cb_size = producer_cb_num_pages * DeviceCommand::PROGRAM_PAGE_SIZE;

    // Targeting fast dispatch on remote device means commands have to be tunneled through ethernet
    const uint32_t consumer_cb_num_pages = get_tensix_consumer_data_buffer_size() / DeviceCommand::PROGRAM_PAGE_SIZE;
    ...
    bool route_through_ethernet = not device->is_mmio_capable();
    if (route_through_ethernet) {
        uint32_t router_cb_num_pages = get_router_data_buffer_size() / DeviceCommand::PROGRAM_PAGE_SIZE;
        const uint32_t router_cb_size = router_cb_num_pages * DeviceCommand::PROGRAM_PAGE_SIZE;
        command.set_router_cb_size(router_cb_size);
        command.set_router_cb_num_pages(router_cb_num_pages);
        command.set_router_transfer_num_pages(DeviceCommand::SYNC_NUM_PAGES);
    }
...
void HWCommandQueue::enqueue_write_buffer(const Buffer& buffer, const void* src, bool blocking) {
    ...
    TT_ASSERT(
        buffer.page_size() < MEM_L1_SIZE - get_data_section_l1_address<CoreType::WORKER>(),
