...
void HWCommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    if (program.has_multi_device_dependencies() and not this->device->is_mmio_capable() and tt::Cluster::instance().is_galaxy_cluster()) {
        // Temporary workaround to avoid deadlocks on Galaxy, until Ethernet Routing Fabric supports VCs:
