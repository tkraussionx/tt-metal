...
void CommandQueue::enqueue_command(Command& command, bool blocking) {
    ...
    command.process();
    std::cout << "HOST ENQUEUED CMD" << std::endl;
...
void CommandQueue::enqueue_program(Program& program, std::optional<std::reference_wrapper<Trace>> trace, bool blocking) {
    ...
    std::cout << "issue queue write ptr " << this->manager.get_issue_queue_write_ptr(this->id)
              << " host_data_and_device_command_size " << host_data_and_device_command_size
              << " issue queue size " << this->manager.get_issue_queue_size(this->id) << std::endl;
    ...
    if ((this->manager.get_issue_queue_write_ptr(this->id)) + host_data_and_device_command_size >=
        ...
        std::cout << "enqueuing program wrap issue" << std::endl;
        this->wrap(DeviceCommand::WrapRegion::ISSUE, blocking);
    }
...
void CommandQueue::wait_finish() {
    ...
    // std::cout << "Remote completion q data slot 0: " << std::endl;
    // tt::test_utils::print_vector_fixed_numel_per_row(comp_q_data0, 32);
