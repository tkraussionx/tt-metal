...
ProgramMap ConstructProgramMap(const Device* device, Program& program) {
    ...
    */
    ...
    auto update_program_page_for_kg =
        [&program_page_transfers,
         &num_transfers_in_program_pages,
         &update_program_page_transfers,
         &extract_dst_noc_multicast_info,
         &device,
         &program](uint32_t src, const KernelGroup& kg, PageTransferType page_transfer_type) -> uint32_t {
        vector<pair<uint32_t, uint32_t>> dst_noc_multicast_info =
        ...
        if (kg.riscv0_id)
            kernel_ids.push_back(kg.riscv0_id.value());
        if (kg.riscv1_id)
            kernel_ids.push_back(kg.riscv1_id.value());
        if (kg.compute_id)
            kernel_ids.push_back(kg.compute_id.value());
        if (kg.erisc_id)
            kernel_ids.push_back(kg.erisc_id.value());
                    ...
                    if (page_transfer_type == PageTransferType::UNICAST) {
                        for (const auto& logical_core : kernel->logical_cores()) {
                            uint32_t dst_noc = get_noc_unicast_encoding(
                                device->physical_core_from_logical_core(logical_core, kg.get_core_type()));
    ...
    auto populate_program_binaries_pages =
        [&program_pages, &program_page_idx, &device, &program](const KernelGroup& kg) {
            vector<KernelHandle> kernel_ids;
            if (kg.riscv0_id)
                kernel_ids.push_back(kg.riscv0_id.value());
            if (kg.riscv1_id)
    ...
    std::vector<KernelGroup> kg_multicast;
    std::vector<KernelGroup> kg_unicast;
    for (const KernelGroup& kg : program.get_kernel_groups()) {
        if (kg.get_core_type() == CoreType::WORKER) {
            kg_multicast.emplace_back(kg);
        } else if (kg.get_core_type() == CoreType::ETH) {
            kg_unicast.emplace_back(kg);
        } else {
    ...
    for (const KernelGroup& kg : kg_multicast) {
        src = update_program_page_for_kg(src, kg, PageTransferType::MULTICAST);
    }
    ...
    for (const KernelGroup& kg : kg_unicast) {
        src = update_program_page_for_kg(src, kg, PageTransferType::UNICAST);
    }
    if (num_transfers_within_page) {
    ...
    for (KernelGroup& kg : kg_multicast) {
    ...
    for (const KernelGroup& kg : kg_multicast) {
        populate_program_binaries_pages(kg);
    }
