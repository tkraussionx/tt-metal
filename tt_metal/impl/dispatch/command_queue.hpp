...
enum class EnqueueCommandType { ENQUEUE_READ_BUFFER, ENQUEUE_WRITE_BUFFER, ALLOCATE_BUFFER, DEALLOCATE_BUFFER, GET_BUF_ADDR, ADD_BUFFER_TO_PROGRAM,
                                SET_RUNTIME_ARGS, UPDATE_RUNTIME_ARGS, ENQUEUE_PROGRAM, FINISH, ENQUEUE_WRAP, FLUSH, ENQUEUE_RECORD_EVENT, ENQUEUE_WAIT_FOR_EVENT, INVALID };
...
class HWCommandQueue {
   ...
   private:
    ...
    friend void EnqueueWriteBufferImpl(CommandQueue& cq, std::variant<std::reference_wrapper<Buffer>, std::shared_ptr<Buffer> > buffer, HostBufferMemTypes src, bool blocking);
