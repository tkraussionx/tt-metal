...
class SystemMemoryManager {
   ...
   public:
    ...
    void set_last_completed_event(const uint8_t cq_id, const uint32_t event_id) {
        cq_to_event_locks[cq_id].lock();
        // TT_ASSERT(event_id >= this->cq_to_last_completed_event[cq_id],
        //     "Event ID is expected to increase. Wrapping not supported for sync. (cq_id: {} completed event {} but last recorded completed event is {})",
        //     cq_id, event_id, this->cq_to_last_completed_event[cq_id]);
        log_info(tt::LogMetal, "Setting last_completed_event: {} for cq_id: {}", event_id, cq_id);
        if (event_id < this->cq_to_last_completed_event[cq_id]) {
            log_warning(tt::LogMetal, "Event ID is expected to increase. Wrapping not supported for sync. (cq_id: {} completed event {} but last recorded completed event is {})",
                cq_id, event_id, this->cq_to_last_completed_event[cq_id]);
        }
