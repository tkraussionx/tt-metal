...
void kernel_main() {
    ...
    while (true) {
        ...
        // while(true);
        // DPRINT << "CONSUMER CB SIZE: " << consumer_cb_size << ENDL();
        ...
        if (is_program) {
            ...
            //    // DPRINT << "YOYO" << ENDL();
            //    // while (true);
            //    // DPRINT << "WRITING SHARDED" << ENDL();
            //    write_buffers_sharded(command_ptr, num_cores, num_buffer_transfers, consumer_cb_size, consumer_cb_num_pages, producer_noc_encoding, producer_consumer_transfer_num_pages, db_buf_switch);
            //}
        }
