...
FORCE_INLINE void record_last_completed_event(uint32_t event_id) {
    ...
    DPRINT << "(x=" << (uint32_t) my_x[0] << ",y=" << (uint32_t) my_y[0] << ") - " << "record_last_completed_event()";
    DPRINT << " event_id: " << event_id << " CQ_COMPLETION_LAST_EVENT addr: 0x" << HEX();
    DPRINT << CQ_COMPLETION_LAST_EVENT << " val: " << DEC() << *get_cq_completion_last_event() << ENDL();
}
