...
void kernel_main() {
    ...
    while (!dest_finished && !timeout) {
        ...
        if (input_queues[curr_input].input_queue_curr_packet_num_words_available_to_send() > 0) {
            if (!output_queue.forward_packet_from_input(curr_input, timeout_cycles)) {
                debug_log_index(1, 0xff000007);
                timeout = true;
                break;
