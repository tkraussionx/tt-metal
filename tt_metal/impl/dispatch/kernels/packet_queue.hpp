...
public:
    ...
    inline bool is_remote_ready() const {
        return NOC_STREAM_READ_REG(this->queue_id, STREAM_REMOTE_SRC_REG_INDEX) == PACKET_QUEUE_REMOTE_READY_FLAG;
    }

    inline bool is_remote_finished() const {
        return NOC_STREAM_READ_REG(this->queue_id, STREAM_REMOTE_SRC_REG_INDEX) == PACKET_QUEUE_REMOTE_FINISHED_FLAG;
    }
...
public:
    ...
    inline void forward_packet_from_input(uint32_t input_queue_index) {

        packet_input_queue_state_t* input_queue_ptr = &(this->input_queue_status.input_queue_array[input_queue_index]);
        uint32_t packet_size_words = input_queue_ptr->get_curr_packet_size_words();

        uint32_t words_forwarded = 0;
        while (words_forwarded < packet_size_words) {
            uint32_t num_words_to_forward = this->get_num_words_to_send(input_queue_index, packet_size_words);
            uint32_t src_addr =
 ...
 */
bool wait_all_src_dest_ready(
    packet_input_queue_state_t* input_queue_array, uint32_t num_input_queues,
