...
inline void debug_log_index(uint32_t offset, uint32_t val) {
    uint32_t i = (debug_buf_index + offset);
    if (i >= debug_buf_size) {
        i -= debug_buf_size;
    }
    debug_buf[i] = val;
...
public:
    ...
    inline uint32_t get_queue_local_wptr() const {
        return NOC_STREAM_READ_REG(3*this->queue_id, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_REG_INDEX);
    }

    inline void reset_queue_local_wptr() {
        // Setting STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX resets the credit register
        NOC_STREAM_WRITE_REG(3*this->queue_id, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX, 0);
        volatile uint32_t* ptr = reinterpret_cast<volatile uint32_t*>(0x140000);
        ptr[0] = 0xabcd;
    }

    inline void advance_queue_local_wptr(uint32_t num_words) {
        NOC_STREAM_WRITE_REG(3*this->queue_id, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX, num_words << REMOTE_DEST_BUF_WORDS_FREE_INC);
    }

    inline uint32_t get_queue_local_rptr_sent() const {
        return NOC_STREAM_READ_REG(3*this->queue_id+1, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_REG_INDEX);
    }

    inline uint32_t get_queue_local_rptr_cleared() const {
        return NOC_STREAM_READ_REG(3*this->queue_id+2, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_REG_INDEX);
    }

    inline void reset_queue_local_rptr_sent()  {
        // Setting STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX resets the credit register
        NOC_STREAM_WRITE_REG(3*this->queue_id+1, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX, 0);
    }

    inline void reset_queue_local_rptr_cleared()  {
        // Setting STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX resets the credit register
        NOC_STREAM_WRITE_REG(3*this->queue_id+2, STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX, 0);
    }

    inline void advance_queue_local_rptr_sent(uint32_t num_words)  {
        NOC_STREAM_WRITE_REG(3*this->queue_id+1, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX, num_words << REMOTE_DEST_BUF_WORDS_FREE_INC);
    }

    inline void advance_queue_local_rptr_cleared(uint32_t num_words)  {
        NOC_STREAM_WRITE_REG(3*this->queue_id+2, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX, num_words << REMOTE_DEST_BUF_WORDS_FREE_INC);
    }

    inline uint32_t get_queue_data_num_words_available_to_send() const {
        return this->get_queue_local_wptr() - this->get_queue_local_rptr_sent();
    }

    inline uint32_t get_queue_data_num_words_occupied() const {
        return this->get_queue_local_wptr() - this->get_queue_local_rptr_cleared();
    }
    ...
    inline uint32_t get_num_words_sent_not_cleared() const {
        return this->get_queue_local_rptr_sent() - this->get_queue_local_rptr_cleared();
    }
    ...
    inline uint32_t get_queue_rptr_sent_offset_words() const {
        return this->get_queue_local_rptr_sent() & (this->queue_size_words - 1);
    }

    inline uint32_t get_queue_rptr_cleared_offset_words() const {
        return this->get_queue_local_rptr_cleared() & (this->queue_size_words - 1);
    }
    ...
    inline void remote_reg_update(uint32_t reg_addr, uint32_t val, uint32_t cmd_buf = 0) {
        ...
        if (this->remote_update_network_type == DispatchRemoteNetworkType::NONE) {
            ...
            // debug_log(0xccddccd1);
            // debug_log(remote_x);
            // debug_log(remote_y);
            // debug_log(reg_addr);
            // debug_log(val);
            // debug_log(this->remote_update_network_type);
            // debug_log(0xccddccd2);
    ...
    inline void advance_queue_remote_rptr_sent(uint32_t num_words) {
        uint32_t val = num_words << REMOTE_DEST_BUF_WORDS_FREE_INC;
        uint32_t reg_addr = STREAM_REG_ADDR(3*this->remote_queue_id+1, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX);
        this->remote_reg_update(reg_addr, val);
    }

    inline void advance_queue_remote_rptr_cleared(uint32_t num_words) {
        uint32_t val = num_words << REMOTE_DEST_BUF_WORDS_FREE_INC;
        uint32_t reg_addr = STREAM_REG_ADDR(3*this->remote_queue_id+2, STREAM_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_REG_INDEX);
        this->remote_reg_update(reg_addr, val);
    }
    ...
    inline void send_remote_ready_notification() {
        ...
        this->remote_reg_update(reg_addr, 0xAA);
    }
...
public:
    ...
    inline bool input_queue_full_packet_available_to_send() {
        ...
        uint32_t curr_packet_words_remaining = (this->curr_packet_size_words+1) - this->curr_packet_words_sent;
        return num_words >= curr_packet_words_remaining;
    }
    ...
    struct {
        ...
        uint32_t prev_words_in_flight_flush() {
            ...
            if (this->total_words_in_flight[prev_index] > 0) {
                ...
                if (this->total_words_in_flight[prev_index] != total_words_in_flight) {
                    debug_log(0xff0000ff);
                    debug_log(this->total_words_in_flight[prev_index]);
                    debug_log(total_words_in_flight);
                }
...
public:
    ...
    inline bool output_barrier(uint64_t max_time = 0) {
        ...
        while (this->get_queue_data_num_words_occupied() > 0) {
            ...
            if (max_time > 0 && curr_time > max_time) {
                return false;
            }
        }
