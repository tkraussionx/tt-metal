...
constexpr uint32_t test_results_buf_addr_arg = get_compile_time_arg_val(44);
constexpr uint32_t test_results_buf_size_bytes = get_compile_time_arg_val(45);
    ...
    while (!all_outputs_finished && !timeout) {
        ...
        for (uint32_t i = 0; i < tunnel_lanes; i++) {
            if (input_queues[i].get_curr_packet_valid()) {
                ...
                //}
                //if (full_packet_sent) {
                //    DPRINT << "full packet sent on "<< i << ENDL();
                //}
                //if (words_sent == 0) {
                //    DPRINT << "q is full"<< i << ENDL();
                //}
            }
