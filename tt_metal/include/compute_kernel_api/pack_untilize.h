...
template <uint32_t block_ct_dim = 8, uint32_t full_ct_dim = block_ct_dim>
ALWI void pack_untilize_dst(uint32_t ocb, uint32_t block_rt_dim = 1, uint32_t block_c_index = 0 /* used when full_ct_dim > block_ct_dim*/, uint32_t face_r_dim = 16, uint32_t num_faces = 4, uint32_t out_index = 0) {
    PACK(( llk_pack_untilize<block_ct_dim, full_ct_dim>(block_rt_dim, ocb, face_r_dim, num_faces, block_c_index, out_index) ));
}
