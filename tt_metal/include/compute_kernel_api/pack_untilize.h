...
ALWI void pack_untilize_init(uint32_t icb, uint32_t ocb)
{
    ...
    UNPACK(( llk_unpack_A_init<BroadcastType::NONE, false, EltwiseBinaryReuseDestType::NONE>(false, false, icb) )); // init must be after configure
}
...
ALWI void pack_untilize_block(uint32_t icb, uint32_t block_rt_dim, uint32_t ocb) {
    for (uint32_t r = 0; r < block_rt_dim; ++r) {
        ...
        for (uint32_t c = 0; c < block_ct_dim; ++c) {
            UNPACK(( llk_unpack_A<BroadcastType::NONE, false, EltwiseBinaryReuseDestType::NONE>(icb, c) ));
