...
FORCE_INLINE T get_arg_val(int arg_idx) {
    ...
    return *((volatile T*)(tt::dataflow_internal::get_arg_addr(arg_idx)));
}
...
struct InterleavedAddrGen {
    ...
    std::uint64_t get_noc_addr(const uint32_t id, const uint32_t offset = 0) const {
        ...
        if constexpr (DRAM) {
            ...
            addr += bank_to_dram_offset[bank_id];
            noc_x = tt::dataflow_internal::dram_bank_to_noc_x[bank_id];
            noc_y = tt::dataflow_internal::dram_bank_to_noc_y[bank_id];
#endif
        } else {
            ...
            noc_x = tt::dataflow_internal::l1_bank_to_noc_x[bank_id];
            noc_y = tt::dataflow_internal::l1_bank_to_noc_y[bank_id];
        }

        uint64_t noc_addr = tt::dataflow_internal::get_noc_addr_helper(noc_x, noc_y, addr);
        return noc_addr;
    }
};
...
struct InterleavedAddrGenFast {
    ...
    void noc_async_read_tile(const uint32_t id, uint32_t dest_addr, const uint32_t offset = 0) const {
        ...
        if constexpr (DRAM) {
            ...
            src_addr = MUL_WITH_TILE_SIZE((uint)this->data_format, id >> LOG_BASE_2_OF_NUM_DRAM_BANKS) +
                       this->bank_base_address + offset;
            src_addr += bank_to_dram_offset[bank_id];
            src_noc_xy = tt::dataflow_internal::dram_bank_to_noc_xy[bank_id];
#endif
        } else {
            uint32_t bank_id = id & (NUM_L1_BANKS - 1);
            ...
            src_noc_xy = tt::dataflow_internal::l1_bank_to_noc_xy[bank_id];
        }
    ...
    void noc_async_write_tile(const uint32_t id, uint32_t src_addr) const {
        ...
        if constexpr (DRAM) {
...
#endif
            dest_noc_xy = tt::dataflow_internal::dram_bank_to_noc_xy[bank_id];
        } else {
            uint32_t bank_id = id & (NUM_L1_BANKS - 1);
            dest_addr =
            ...
            dest_noc_xy = tt::dataflow_internal::l1_bank_to_noc_xy[bank_id];
        }
...
void cq_wait_front() {
    ...
    } while (tt::dataflow_internal::cq_read_interface.fifo_rd_ptr == fifo_wr_ptr);
}
...
void cq_pop_front(u32 cmd_size_B) {
    ...
    tt::dataflow_internal::cq_read_interface.fifo_rd_ptr += cmd_size_16B;

    if (tt::dataflow_internal::cq_read_interface.fifo_rd_ptr > tt::dataflow_internal::cq_read_interface.fifo_limit) {
        tt::dataflow_internal::cq_read_interface.fifo_rd_ptr -= tt::dataflow_internal::cq_read_interface.fifo_size;
    }

    uint32_t pcie_noc_x = NOC_X(0);
    uint32_t pcie_noc_y = NOC_Y(4);                              // These are the PCIE core coordinates
    uint64_t pcie_address =
        tt::dataflow_internal::get_noc_addr(pcie_noc_x, pcie_noc_y, HOST_CQ_READ_PTR);  // For now, we are writing to host hugepages at offset
                                                                 // 0 (nothing else currently writing to it)

    u32 rd_ptr = tt::dataflow_internal::cq_read_interface.fifo_rd_ptr;
