...
void MAIN {
    ...
    uint32_t num_blocks_h = get_compile_time_arg_val(8);  // outer inner dim (in inner dim blocks)
    // If I don't make this volatile, causes code size for TRISC2 to be too large if num_blocks > 1
    volatile uint32_t num_blocks_w = get_compile_time_arg_val(9);  // outer inner dim (in inner dim blocks)


    uint32_t out_subblock_h = get_compile_time_arg_val(10); // inner row block size in tiles
    uint32_t out_subblock_w = get_compile_time_arg_val(11); // inner column block size in tiles
