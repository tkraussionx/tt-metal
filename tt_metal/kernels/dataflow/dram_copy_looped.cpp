 ...
 * */
void kernel_main() {
    std::uint32_t l1_buffer_addr             = dataflow::get_arg_val<uint32_t>(0);

    std::uint32_t dram_buffer_src_addr_base  = dataflow::get_arg_val<uint32_t>(1);
    std::uint32_t dram_src_noc_x             = dataflow::get_arg_val<uint32_t>(2);
    std::uint32_t dram_src_noc_y             = dataflow::get_arg_val<uint32_t>(3);

    std::uint32_t dram_buffer_dst_addr_base  = dataflow::get_arg_val<uint32_t>(4);
    std::uint32_t dram_dst_noc_x             = dataflow::get_arg_val<uint32_t>(5);
    std::uint32_t dram_dst_noc_y             = dataflow::get_arg_val<uint32_t>(6);

    std::uint32_t dram_buffer_size           = dataflow::get_arg_val<uint32_t>(7);
    std::uint32_t chunk_size                 = dataflow::get_arg_val<uint32_t>(8);
