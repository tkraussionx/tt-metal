#include <cstdint>

/**
 * NOC APIs are prefixed w/ "ncrisc" (legacy name) but there's nothing NCRISC specific, they can be used on BRISC or other RISCs
 * Any two RISC processors cannot use the same CMD_BUF
 * non_blocking APIs shouldn't be mixed with slow noc.h APIs
 * explicit flushes need to be used since the calls are non-blocking
 * */
void kernel_main() {
    std::uint32_t l1_buffer_addr        = dataflow::get_arg_val<uint32_t>(0);

    std::uint32_t dram_buffer_src_addr  = dataflow::get_arg_val<uint32_t>(1);
    std::uint32_t dram_src_noc_x        = dataflow::get_arg_val<uint32_t>(2);
    std::uint32_t dram_src_noc_y        = dataflow::get_arg_val<uint32_t>(3);

    std::uint32_t num_sticks      = dataflow::get_arg_val<uint32_t>(4);
    std::uint32_t stick_size      =  dataflow::get_arg_val<uint32_t>(5);
    for(uint32_t i = 0; i < 1; i++) {
        // loading_noc variable is defined by either NCRISC or BRISC to be 0 or 1, depending on which RISC the kernel is running
        for(uint32_t stick_id = 0; stick_id < num_sticks; stick_id++) {
            // DRAM NOC src address
            std::uint64_t dram_buffer_src_noc_addr = dataflow::get_noc_addr(dram_src_noc_x, dram_src_noc_y, dram_buffer_src_addr);
            dataflow::noc_async_read(dram_buffer_src_noc_addr, l1_buffer_addr, stick_size);
            dataflow::noc_async_read_barrier();
            l1_buffer_addr += stick_size;
        }
    }
}
