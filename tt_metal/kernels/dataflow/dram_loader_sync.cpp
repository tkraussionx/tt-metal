...
void kernel_main() {
    ...
    std::uint32_t local_buffer_addr                 = dataflow::get_arg_val<uint32_t>(3);
    std::uint32_t consumer_core_noc_x               = dataflow::get_arg_val<uint32_t>(4);
    std::uint32_t consumer_core_noc_y               = dataflow::get_arg_val<uint32_t>(5);
    std::uint32_t stream_register_address           = dataflow::get_arg_val<uint32_t>(6);
    std::uint32_t num_tiles                         = dataflow::get_arg_val<uint32_t>(7);
    std::uint32_t transient_buffer_size_tiles       = dataflow::get_arg_val<uint32_t>(8);
    std::uint32_t transient_buffer_size_bytes       = dataflow::get_arg_val<uint32_t>(9);
    ...
    *(constant_ptr) = VALID_VAL;
    ...
    std::uint64_t remote = dataflow::get_noc_addr(consumer_core_noc_x, consumer_core_noc_y, stream_register_address);
    ...
    while(counter < num_tiles) {
        ...
        dataflow::noc_async_read(dram_buffer_src_noc_addr, local_buffer_addr, transient_buffer_size_bytes);
