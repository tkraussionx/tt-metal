...
void kernel_main() {
    ...
    std::uint32_t l1_buffer_src_addr_base    = dataflow::get_arg_val<uint32_t>(3);
    std::uint32_t l1_buffer_dst_addr_base    = dataflow::get_arg_val<uint32_t>(4);
    std::uint32_t l1_dst_noc_x        = dataflow::get_arg_val<uint32_t>(5);
    std::uint32_t l1_dst_noc_y        = dataflow::get_arg_val<uint32_t>(6);
    std::uint32_t num_tiles                  = dataflow::get_arg_val<uint32_t>(7);
    std::uint32_t single_tile_size_bytes     = dataflow::get_arg_val<uint32_t>(8);
    std::uint32_t total_tiles_size_bytes     = dataflow::get_arg_val<uint32_t>(9);
