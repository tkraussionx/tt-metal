...
void kernel_main() {
    ...
    uint32_t NC         = dataflow::get_arg_val<uint32_t>(9); // should be passed in as 1
    uint32_t Ht         = dataflow::get_arg_val<uint32_t>(10);
    uint32_t Wt         = dataflow::get_arg_val<uint32_t>(11);
    uint32_t nc1        = dataflow::get_arg_val<uint32_t>(12); // if 1 we expect the bcast tensor to have NC=1 and wrap around in NC
    uint32_t start_id   = dataflow::get_arg_val<uint32_t>(13);
    uint32_t HtWt       = dataflow::get_arg_val<uint32_t>(14); // original input tensor HtWt
