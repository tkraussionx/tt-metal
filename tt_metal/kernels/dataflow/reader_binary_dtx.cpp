...
void kernel_main() {
    ...
    // Arguments for in0
    uint32_t in0_addr_base  = get_arg_val<uint32_t>(2);
    uint32_t in0_noc_x = get_arg_val<uint32_t>(3);
    uint32_t in0_noc_y = get_arg_val<uint32_t>(4);
    ...
    // Arguments for in1
    uint32_t src1_addr  = get_arg_val<uint32_t>(8);
    ...
    uint32_t in1_tensor_start_tile_id           = get_arg_val<uint32_t>(12);
    uint32_t in1_tensor_stride_w                = get_arg_val<uint32_t>(13);
    uint32_t in1_tensor_stride_h                = get_arg_val<uint32_t>(14);
    uint32_t in1_tensor_next_block_stride       = get_arg_val<uint32_t>(15);
