...
inline void async_read_from_dram_using_address_map(uint32_t dram_start_addr,
                                            ...
                                            uint32_t zeros_padding_buffer_l1_size_bytes) {
    for(uint32_t i = address_map_group_start_index; i < address_map_group_start_index + address_map_group_size; i+=4) {
        ...
        if(pad == 1) {
            ...
            if (pad_size <= zeros_padding_buffer_l1_size_bytes) {
                ...
                uint32_t zeros_read_size = zeros_padding_buffer_l1_size_bytes;
...
void kernel_main() {
    ...
    uint32_t zeros_padding_buffer_l1_addr = get_arg_val<uint32_t>(10);
    uint32_t zeros_padding_buffer_l1_size_bytes = get_arg_val<uint32_t>(11);
    ...
    uint64_t zeros_base_noc_addr = get_noc_addr(zeros_padding_buffer_l1_addr);
    ...
    for(uint32_t g = 0; g < num_groups; g++) {
        ...
        async_read_from_dram_using_address_map(in0_addr_base, in0_noc_x, in0_noc_y,
            ...
            in0_address_map_index, zeros_base_noc_addr, zeros_padding_buffer_l1_size_bytes);
        ...
        async_read_from_dram_using_address_map(in1_addr_base, in1_noc_x, in1_noc_y,
            ...
            in1_address_map_index, zeros_base_noc_addr, zeros_padding_buffer_l1_size_bytes);
