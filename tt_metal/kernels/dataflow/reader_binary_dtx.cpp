...
void kernel_main() {
    ...
    uint32_t zeros_padding_buffer_l1_addr = get_arg_val<uint32_t>(16);
    uint32_t zeros_padding_buffer_l1_size_bytes = get_arg_val<uint32_t>(17);
    uint32_t scratch_pad_for_address_map_in_l1_addr = get_arg_val<uint32_t>(18);
    ...
    for(uint32_t g = 0; g < num_groups; g++) {
        ...
        async_read_from_dram_using_address_map(in0_addr_base, in0_noc_x, in0_noc_y,
            ...
            zeros_base_noc_addr, zeros_padding_buffer_l1_size_bytes);
        noc_async_read_barrier();
