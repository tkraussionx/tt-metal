...
void kernel_main() {
    ...
    uint32_t in1_mcast_sender_semaphore_addr    = get_arg_val<uint32_t>(3);
    uint32_t in1_mcast_receiver_semaphore_addr  = get_arg_val<uint32_t>(4);
    ...
    // WRITER
    // out tensor args
    constexpr uint32_t out_tensor_addr                    = get_compile_time_arg_val(16);
