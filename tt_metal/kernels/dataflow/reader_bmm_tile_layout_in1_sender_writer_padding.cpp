...
void kernel_main() {
    ...
    // in1 mcast args
    uint32_t in1_mcast_dest_noc_start_x         = get_arg_val<uint32_t>(9);
    uint32_t in1_mcast_dest_noc_start_y         = get_arg_val<uint32_t>(10);
    uint32_t in1_mcast_dest_noc_end_x           = get_arg_val<uint32_t>(11);
    uint32_t in1_mcast_dest_noc_end_y           = get_arg_val<uint32_t>(12);
    uint32_t in1_mcast_num_dests                = get_arg_val<uint32_t>(13);
    uint32_t in1_mcast_sender_semaphore_addr    = get_arg_val<uint32_t>(14);
    uint32_t in1_mcast_receiver_semaphore_addr  = get_arg_val<uint32_t>(15);

    // batch args
    uint32_t KtNt                               = get_arg_val<uint32_t>(16);
    uint32_t batch                              = get_arg_val<uint32_t>(17);
    uint32_t bcast_B                            = get_arg_val<uint32_t>(18);

    // WRITER
    // out tensor args
    uint32_t out_tensor_addr                    = get_arg_val<uint32_t>(19);
    uint32_t out_tensor_start_tile_id           = get_arg_val<uint32_t>(20);
    uint32_t out_tensor_stride_w                = get_arg_val<uint32_t>(21);
    uint32_t out_tensor_stride_h                = get_arg_val<uint32_t>(22);
    uint32_t out_tensor_next_subblock_stride_w  = get_arg_val<uint32_t>(23);
    uint32_t out_tensor_next_subblock_stride_h  = get_arg_val<uint32_t>(24);

    // out subblock args
    uint32_t out_subblock_w                     = get_arg_val<uint32_t>(25);
    uint32_t out_subblock_h                     = get_arg_val<uint32_t>(26);
    uint32_t out_subblock_tile_count            = get_arg_val<uint32_t>(27);

    // batch args
    uint32_t MtNt                               = get_arg_val<uint32_t>(28); // if 0
    // Don't need batch; same as batch from READER args

    // padding args (READER)
    uint32_t last_block_w                       = get_arg_val<uint32_t>(29);
    // padding args (WRITER)
    uint32_t out_num_nonzero_subblocks_h        = get_arg_val<uint32_t>(30);
    uint32_t out_last_subblock_h                = get_arg_val<uint32_t>(31);
    uint32_t padded_block_tiles_h_skip          = get_arg_val<uint32_t>(32);
    uint32_t out_num_nonzero_subblocks_w        = get_arg_val<uint32_t>(33);
    uint32_t out_last_subblock_w                = get_arg_val<uint32_t>(34);
    uint32_t padded_subblock_tiles_addr_skip    = get_arg_val<uint32_t>(35);
    uint32_t padded_block_tiles_w_skip          = get_arg_val<uint32_t>(36);
