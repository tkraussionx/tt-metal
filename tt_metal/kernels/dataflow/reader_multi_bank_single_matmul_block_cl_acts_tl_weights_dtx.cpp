...
void kernel_main() {
    ...
    for(uint32_t in1_tile = 0; in1_tile < in1_tiles; in1_tile+=1) {
        uint64_t in1_tile_noc_addr = dataflow::get_noc_addr(in1_tile, s1);
    ...
    for (uint32_t index = 0; index < address_map_size; index+=4) {
        ...
        uint64_t in0_row_noc_addr = dataflow::get_noc_addr(row_bank_id, s0);
        dataflow::noc_async_read(in0_row_noc_addr, l1_write_addr_in0, size);
        l1_write_addr_in0 += size;
    }
    // Height padding
    for (uint32_t z = 0; z < num_transfers_of_zeroes; z++) {
        dataflow::noc_async_read(zeros_base_noc_addr, l1_write_addr_in1, num_bytes_of_zeroes_per_transfer);
