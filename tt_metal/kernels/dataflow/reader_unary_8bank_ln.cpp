...
void generate_bcast_scaler() {
    ...
    union { float f; uint32_t u; } u; u.u = dataflow::get_arg_val<uint32_t>(5);
...
void generate_epsilon() {
    ...
    union { float f; uint32_t u; } u; u.u = dataflow::get_arg_val<uint32_t>(6);
...
void kernel_main() {
    ...
    uint32_t NCHt      = dataflow::get_arg_val<uint32_t>(1);
    uint32_t Wt        = dataflow::get_arg_val<uint32_t>(2);

    uint32_t num_gamma_tiles = dataflow::get_arg_val<uint32_t>(7); // gamma, beta are (1,1,32,W), bcast from W->NCH , result = x*gamma+beta
    uint32_t gamma_addr = dataflow::get_arg_val<uint32_t>(8);
    uint32_t num_beta_tiles = dataflow::get_arg_val<uint32_t>(9);
    uint32_t beta_addr = dataflow::get_arg_val<uint32_t>(10);
    uint32_t b_addr = dataflow::get_arg_val<uint32_t>(11);
