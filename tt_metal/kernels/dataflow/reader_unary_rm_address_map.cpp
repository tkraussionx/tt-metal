...
void kernel_main() {
    ...
    for (uint32_t t = 0; t < num_tiles - block_size_tiles; t += block_size_tiles) {
        ...
        for (uint32_t i = 0; i < 32; i++) {
            ...
            while(bytes_written < row_size_bytes) {
                ...
                dataflow::noc_async_read(src_noc_addr, l1_write_addr, size);
                l1_write_addr += size;
                bytes_written += size;
            }
        }
    ...
    for (uint32_t z = 0; z < num_transfers_of_zeroes; z++) {
        dataflow::noc_async_read(zeros_base_noc_addr, l1_write_addr, num_bytes_of_zeroes_per_transfer);
        l1_write_addr += num_bytes_of_zeroes_per_transfer;
    }
    if(remainder_zeroes > 0) {
        dataflow::noc_async_read(zeros_base_noc_addr, l1_write_addr, remainder_zeroes);
