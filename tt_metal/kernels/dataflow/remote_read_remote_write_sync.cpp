...
void kernel_main() {
    ...
    std::uint32_t buffer_dst_addr             = dataflow::get_arg_val<uint32_t>(3);
    std::uint32_t dst_noc_x                   = dataflow::get_arg_val<uint32_t>(4);
    std::uint32_t dst_noc_y                   = dataflow::get_arg_val<uint32_t>(5);
    std::uint32_t l1_buffer_address           = dataflow::get_arg_val<uint32_t>(6);
    std::uint32_t stream_register_address     = dataflow::get_arg_val<uint32_t>(7);
    std::uint32_t num_tiles                   = dataflow::get_arg_val<uint32_t>(8);
    std::uint32_t transient_buffer_size_tiles = dataflow::get_arg_val<uint32_t>(9);
    std::uint32_t transient_buffer_size_bytes = dataflow::get_arg_val<uint32_t>(10);
    ...
    *(constant_ptr) = INVALID_VAL;
    ...
    while(counter < num_tiles) {
        ...
        dataflow::noc_async_read(src_noc_addr, l1_buffer_address, transient_buffer_size_bytes);
        dataflow::noc_async_read_barrier();

        // DRAM NOC dst address
        std::uint64_t dst_noc_addr = dataflow::get_noc_addr(dst_noc_x, dst_noc_y, dst_buffer_addr);
        dataflow::noc_async_write(l1_buffer_address, dst_noc_addr, transient_buffer_size_bytes);
