...
void kernel_main() {
    ...
    std::uint32_t   buffer_src_addr  = dataflow::get_arg_val<uint32_t>(1);
    ...
    for (std::uint32_t i=0; i<num_repetitions; i++) {
        ...
        buffer_dst_addr  = dataflow::get_arg_val<uint32_t>(0);
        buffer_src_addr  = dataflow::get_arg_val<uint32_t>(1);
