#include <stdint.h>
#include <array>
#include "dataflow_kernel_api.h"

void kernel_main() {
    // WRITER RUNTIME ARGS
    uint32_t out_tensor_addr                     = dataflow::get_arg_val<uint32_t>(0);
    uint32_t out_tensor_tile_id                  = dataflow::get_arg_val<uint32_t>(1);

    // COMPILE TIME ARGS
    // dataflow::Interleaved accessor args
    constexpr uint32_t out_is_dram               = get_compile_time_arg_val(1);
    // WRITER COMPILE TIME ARGS
    constexpr uint32_t out_w_tiles               = get_compile_time_arg_val(2);
    constexpr uint32_t out_h_tiles               = get_compile_time_arg_val(3);
    constexpr uint32_t out_c                     = get_compile_time_arg_val(4);
    constexpr uint32_t out_HtWt                  = get_compile_time_arg_val(5);


    #define out_transpose_hw get_compile_time_arg_val(6) == 1
    #if (out_transpose_hw)
    constexpr uint32_t cb_id_out0 = 16;
    #else
    constexpr uint32_t cb_id_out0 = 0; // same as cb_id_in0
    #endif

    uint32_t single_tile_size_bytes = dataflow::get_tile_size(cb_id_out0);

    constexpr bool out_is_dram_bool = out_is_dram == 1;
    #define tile_dtype_is_bfloat16 get_compile_time_arg_val(0) == 1
    #if (tile_dtype_is_bfloat16)
    const dataflow::InterleavedAddrGenFast<out_is_dram_bool> s = {
        .bank_base_address = out_tensor_addr,
        .page_size = single_tile_size_bytes,
        .data_format = DataFormat::Float16
    };
    #else
    const dataflow::InterleavedAddrGenFast<out_is_dram_bool> s = {
        .bank_base_address = out_tensor_addr,
        .page_size = single_tile_size_bytes,
        .data_format = DataFormat::Bfp8_b
    };
    #endif

    uint32_t l1_read_addr = dataflow::get_read_ptr(cb_id_out0);
    uint32_t out_num_tiles_read = out_w_tiles;
    uint32_t out_tensor_current_tile_id = out_tensor_tile_id;

    for (uint32_t c_dim = 0; c_dim < out_c; c_dim++) {
        dataflow::cb_wait_front(cb_id_out0, out_num_tiles_read);

        #if (out_transpose_hw)
        for (uint32_t w_dim = 0; w_dim < out_w_tiles; w_dim++) {
            dataflow::noc_async_write_tile(out_tensor_current_tile_id, s, l1_read_addr);
            l1_read_addr += single_tile_size_bytes;
            out_tensor_current_tile_id += out_h_tiles;
        }
        #else
        out_tensor_current_tile_id = out_tensor_tile_id;
        for (uint32_t w_dim = 0; w_dim < out_w_tiles; w_dim++) {
            dataflow::noc_async_write_tile(out_tensor_current_tile_id, s, l1_read_addr);
            l1_read_addr += single_tile_size_bytes;
            out_tensor_current_tile_id++;
        }
        out_tensor_tile_id += out_HtWt;
        #endif

        out_num_tiles_read += out_w_tiles;
    }

    dataflow::noc_async_write_barrier();
    dataflow::cb_pop_front(cb_id_out0, out_num_tiles_read);
}
