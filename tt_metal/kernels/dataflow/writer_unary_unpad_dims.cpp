...
void kernel_main() {
    ...
    const uint32_t num_total_W              = dataflow::get_arg_val<uint32_t>(2);
    const uint32_t num_unpadded_Z           = dataflow::get_arg_val<uint32_t>(3);
    const uint32_t num_total_Z              = dataflow::get_arg_val<uint32_t>(4);
    const uint32_t num_unpadded_Y           = dataflow::get_arg_val<uint32_t>(5);
    const uint32_t num_total_Y              = dataflow::get_arg_val<uint32_t>(6);
    const uint32_t num_unpadded_X           = dataflow::get_arg_val<uint32_t>(7);
    const uint32_t num_total_X              = dataflow::get_arg_val<uint32_t>(8);
    ...
    const uint32_t cache_buffer_l1_addr     = dataflow::get_arg_val<uint32_t>(11);
    const uint32_t temp_buffer_l1_addr      = dataflow::get_arg_val<uint32_t>(12);
    ...
    const uint32_t log_base_2_of_page_size = dataflow::get_arg_val<uint32_t>(13);
            ...
            for (uint32_t y_t = 0; y_t < num_total_Y / 32; y_t++) {
                ...
                if (row_id_in_face < num_unpadded_Y) {
                    ...
                    for (uint32_t k = 0; k < 32; k++) {
                        if (row_id_in_face < num_unpadded_Y) {
                            uint64_t dst_noc_addr = dataflow::get_noc_addr(stick_id, s);
                            ...
                            dataflow::noc_async_write(temp_buffer_l1_addr, round_down_addr, unpadded_X_size + diff_addr);
