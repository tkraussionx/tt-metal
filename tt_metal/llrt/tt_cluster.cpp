...
Cluster::Cluster() {
    ...
    for (const auto &[chip_id, mapping]: this->device_eth_routing_info_) {
        for(const auto &[eth_core, mode]: mapping) {
          std::cout << " chip id " << chip_id << " eth core " << eth_core.str() << " phys " << ethernet_core_from_logical_core(chip_id, eth_core).str()<< " mode " << (uint32_t)mode << std::endl;
        }
    }
    std::cout << " checking sockets " << this->get_ethernet_sockets(0, 7).size() << this->get_ethernet_sockets(0, 7)[0].str() << std::endl;
    std::cout << " checking sockets " << this->get_ethernet_sockets(7, 0).size() << this->get_ethernet_sockets(7, 0)[0].str() << std::endl;
