...
Tensor matmul_multi_core_reuse_padding (const Tensor &A, const Tensor &B, bool profile_device=false); // Only supports 2D matmul expects N=1 for now
Tensor bmm_multi_core_reuse_padding  (const Tensor &A, const Tensor &B, bool profile_device=false); // Only supports 2D matmul expects N=1 for now
