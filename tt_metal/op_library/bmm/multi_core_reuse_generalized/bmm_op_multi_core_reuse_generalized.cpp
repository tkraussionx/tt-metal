...
Tensor matmul_multi_core_reuse_generalized_(const Tensor &a, const Tensor &b, bool bcast_batch, bool profile_device) {
    ...
    tt_metal::Program * program = reuse_generalized_helpers::create_program(
        device,
        single_tile_size,
        num_cores_x,
        Mt, Nt, Kt,
    ...
    tt_metal::DumpHostProfileResults("\"{'parallelization': 'multi_core_reuse_generalized_" + matmul_params_str + "','num_cores':" + std::to_string((std::int32_t) num_blocks_total) + "}\"");
    if (profile_device){
...
Tensor matmul_multi_core_reuse_generalized(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_generalized_(a, b, true, profile_device);
}

Tensor bmm_multi_core_reuse_generalized(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_generalized_(a, b, false, profile_device);
}
