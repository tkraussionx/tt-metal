...
Tensor matmul_multi_core_reuse_mcast_generalized_(const Tensor &a, const Tensor &b, bool bcast_batch, bool profile_device) {
    ...
    std::string matmul_params_str = std::to_string(per_core_M) + "_" + std::to_string(per_core_N) + "_" + std::to_string(out_subblock_h) + "_" + std::to_string(out_subblock_w) + "_" + std::to_string(in0_block_w);
    if (core_range.x > 1 && core_range.y > 1) {
        ...
        parallelization = "multi_core_reuse_mcast_in0_in1_generalized_" + matmul_params_str;
        ...
        parallelization = "multi_core_reuse_mcast_in0_generalized_" + matmul_params_str;
    } else {
        ...
        parallelization = "multi_core_reuse_mcast_in1_generalized_" + matmul_params_str;
    }
...
Tensor matmul_multi_core_reuse_mcast_generalized(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_mcast_generalized_(a, b, true, profile_device);
}

Tensor bmm_multi_core_reuse_mcast_generalized(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_mcast_generalized_(a, b, false, profile_device);
}
