    ...
    for(int output_idx_y = 0; output_idx_y < num_blocks_y; output_idx_y++) {
        for(int output_idx_x = 0; output_idx_x < num_blocks_x; output_idx_x++) {
            ...
            auto mm_reader_kernel = tt_metal::CreateDataMovementKernel(
                program,
                "kernels/dataflow/reader_bmm_tile_layout_padding.cpp",
                core,
...
Tensor matmul_multi_core_reuse_padding_(const Tensor &a, const Tensor &b, bool bcast_batch, bool profile_device) {
    ...
    tt_metal::DumpHostProfileResults("\"{'parallelization': 'multi_core_reuse_padding', 'num_cores':" + std::to_string((std::int32_t) num_blocks_total) + "}\"");
    if (profile_device){
...
Tensor matmul_multi_core_reuse_padding(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_padding_(a, b, true, profile_device);
}

Tensor bmm_multi_core_reuse_padding(const Tensor& a, const Tensor& b, bool profile_device) {
    return matmul_multi_core_reuse_padding_(a, b, false, profile_device);
}
