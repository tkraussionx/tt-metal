...
void MAIN {
        ...
        const uint32_t this_core_Mt = get_arg_val<uint32_t>(0);
        const uint32_t this_core_Nt = get_arg_val<uint32_t>(1);

        mm_init();

        // int x = src1_addr + 2;
        for(uint32_t m = 0; m<this_core_Mt; m++)
        for(uint32_t n = 0; n<this_core_Nt; n++)
        {
