...
void golden_matmul(
    vector<bfloat16>& a, vector<bfloat16>& b, vector<bfloat16>& output, uint32_t M, uint32_t N, uint32_t K) {
...
void matmul_multicore_reuse(
    ...
    log_info(
        tt::LogVerif,
        " -- per_core_Mt= {} -- per_core_Nt= {} -- out_subblock_h= {} -- out_subblock_w= {} --",
        per_core_Mt,
        per_core_Nt,
        out_subblock_h,
        out_subblock_w);
    ...
    uint32_t in0_block_tiles = per_core_Mt * in0_block_w;  // 16 * 2
    uint32_t in0_CB_tiles = in0_block_tiles * 2;           // double buffer
    uint32_t in0_CB_size = in0_CB_tiles * single_tile_size;
    uint32_t in1_block_tiles = per_core_Nt * in0_block_w;  // 16 * 2
    uint32_t in1_CB_tiles = in1_block_tiles * 2;           // double buffer
    uint32_t in1_CB_size = in1_CB_tiles * single_tile_size;
    uint32_t out_block_tiles = per_core_Mt * per_core_Nt;  // 16 * 16
    ...
    uint32_t in0_num_subblocks = (per_core_Mt / out_subblock_h);                      // 16/4
    uint32_t in0_block_num_tiles = out_subblock_h * in0_block_w * in0_num_subblocks;  //  in0_block_tiles
    uint32_t in0_subblock_num_tiles = out_subblock_h * in0_block_w;                   // 4 * 2

    uint32_t in1_num_subblocks = (per_core_Nt / out_subblock_w);                      // 16/2
    uint32_t in1_block_num_tiles = out_subblock_w * in0_block_w * in1_num_subblocks;  // in1_block_tiles
    uint32_t in1_per_core_w = out_subblock_w * in1_num_subblocks;                     // 2*8
    ...
    CoreRangeSet all_cores(
        tt::tt_metal::num_cores_to_corerange_set(num_blocks_x * num_blocks_y, compute_with_storage_grid_size, true));
    log_info(tt::LogVerif, " --  num_blocks_y {} num_blocks_x {} ", num_blocks_y, num_blocks_x);
    log_info(tt::LogVerif, " --  all_cores {}", all_cores);
    ...
    auto reader_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_exercise/kernels/dataflow/reader_matmul_multicore_reuse.cpp",
        all_cores,
    ...
    auto writer_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_exercise/kernels/dataflow/writer_matmul_multicore_reuse.cpp",
        all_cores,
        tt_metal::DataMovementConfig{
    ...
    auto mm_kernel_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_exercise/kernels/compute/matmul_multicore_reuse.cpp",
        all_cores,
    ...
    for (int output_idx_y = 0; output_idx_y < num_blocks_y; output_idx_y++) {
        for (int output_idx_x = 0; output_idx_x < num_blocks_x; output_idx_x++) {
            ...
            std::vector<uint32_t> mm_reader_args = {
                ...
                (std::uint32_t)Kt * per_core_Mt * output_idx_y,  // in0_tensor_start_tile_id
                ...
                (std::uint32_t)per_core_Nt * output_idx_x,   // in1_tensor_start_tile_id
            ...
            std::vector<uint32_t> writer_args = {
                ...
                (std::uint32_t)output_idx_x * per_core_Nt +
                    output_idx_y * per_core_Mt * Nt,  // out_tensor_start_tile_id
...
int main(int argc, char** argv) {
    ...
    log_info(tt::LogVerif, "M {} N {} K {}", M, N, K);
    ...
    try {
        ...
        matmul_multicore_reuse(src0_vec, src1_vec, result_vec, M, N, K, device);
