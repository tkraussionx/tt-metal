...
void kernel_main() {
	...
	uint64_t semaphore_write_noc_addr = get_noc_addr(core_x_cord, core_y_cord, semaphore_write_local_addr);
    ...
    noc_semaphore_set(semaphore_read_l1_address, 0); // reset reader semaphore to 0 so it doesnt start reading early, before generator kernel writes integers
