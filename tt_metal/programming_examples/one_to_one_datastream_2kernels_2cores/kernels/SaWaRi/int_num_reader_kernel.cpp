...
void kernel_main() {
	...
	int core_gen_x_cord = get_arg_val<uint32_t>(3);
	int core_gen_y_cord = get_arg_val<uint32_t>(4);
	int core_read_x_cord = get_arg_val<uint32_t>(5);
	int core_read_y_cord = get_arg_val<uint32_t>(6);
	...
	uint64_t semaphore_write_noc_addr = get_noc_addr(core_gen_x_cord, core_gen_y_cord, semaphore_write_local_addr); // changed this from read to write
	...
	constexpr uint32_t cb_read = tt::CB::c_in1; // index=0
	...
	read_ptr = 0;
 	...
 	noc_semaphore_inc(semaphore_write_noc_addr, 1);	 // signals to generator kernel to write value to DRAM
	...
	for (int integer = 1; integer < 1001; integer++) {
		...
		noc_async_read(dram_noc_addr, l1_read_addr, 4);
 		...
 		noc_semaphore_inc(semaphore_write_noc_addr, 1); // tells generator kernel to write next value in stream to dram
