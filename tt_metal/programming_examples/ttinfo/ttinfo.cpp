...
std::string bufferTypeToString(tt::tt_metal::BufferType type) {
    switch (type) {
        case tt::tt_metal::BufferType::DRAM: return "DRAM";
        case tt::tt_metal::BufferType::L1: return "L1";
...
int main() {
    ...
    log_info(
        tt::LogTest,
        "num_dram_channels {}, dram_size_per_channel {}, l1_size_per_core {}",
        num_dram_channels,
        dram_size_per_channel,
        l1_size_per_core);
    ...
    log_info(
        tt::LogTest,
        "Number of banks for type {} is {} and each bank size is {} bytes ({:.3f} MBs)",
        bufferTypeToString(tt::tt_metal::BufferType::DRAM),
        num_dram_banks,
        dram_bank_size,
        static_cast<double>(dram_bank_size) / (1024 * 1024));
    log_info(
        tt::LogTest,
        "Number of banks for type {} is {} and each bank size is {} bytes ({:.3f} MBs)",
        bufferTypeToString(tt::tt_metal::BufferType::L1),
        num_l1_banks,
        l1_bank_size,
        static_cast<double>(l1_bank_size) / (1024 * 1024));
    ...
    for (int x = 0; x < logical_grid_size.x; ++x) {
        for (int y = 0; y < logical_grid_size.y; ++y) {
            ...
            log_info(
                tt::LogTest,
                "Logical core ({}, {}) mapped to physical core ({}, {})",
                logical_core.x,
                logical_core.y,
                physical_core.x,
                physical_core.y);
