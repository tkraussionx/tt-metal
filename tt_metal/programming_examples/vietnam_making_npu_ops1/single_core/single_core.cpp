...
int main() {
    ...
    auto page_size = 1024 * 2 /*TODO*/;
    auto device_buffer_size = num_tiles * page_size /*TODO*/;

    auto device_buffer0_config = InterleavedBufferConfig{
        .device = device, .size = device_buffer_size, .page_size = page_size, .buffer_type = BufferType::DRAM};
    ...
    EnqueueWriteBuffer(cq, device_buffer0, host_buffer0.get(), false /*blocking*/);
    ...
    auto target_cores = CoreCoord{1, 0};
    // auto target_cores = CoreRange({0,0}, {1,0});
    /////////////////////////////////////////////////////////////////////////////////
    ...
    auto cb0_config =
        CircularBufferConfig(cb_num_tiles * page_size, {{cb0_id, cb0_data_format}}).set_page_size(cb0_id, page_size);
    ...
    const uint32_t device_buffer0_is_dram =
        static_cast<uint32_t>(device_buffer0_config.buffer_type == BufferType::DRAM);
    const uint32_t device_buffer1_is_dram =
        static_cast<uint32_t>(device_buffer1_config.buffer_type == BufferType::DRAM);
    ...
    KernelHandle writer_kernel_id = CreateKernel(
        program,
        "/home/ubuntu/tt-metal/tt_metal/programming_examples/"
        "vietnam_making_npu_ops1/single_core/kernels/writer.cpp" /* TODO path to writer.cpp*/,
        target_cores,
        WriterDataMovementConfig({device_buffer1_is_dram}, {} /* TODO */));
    ...
    const std::vector<uint32_t> reader_runtime_args = {
        device_buffer0->address(), static_cast<uint32_t>(cb0_id), num_tiles};

    SetRuntimeArgs(program, reader_kernel_id, target_cores, reader_runtime_args);

    const std::vector<uint32_t> writer_runtime_args = {
        device_buffer1->address(), static_cast<uint32_t>(cb1_id), num_tiles
        /*TODO*/};
    ...
    SetRuntimeArgs(program, compute_kernel_id, target_cores, compute_runtime_args);
