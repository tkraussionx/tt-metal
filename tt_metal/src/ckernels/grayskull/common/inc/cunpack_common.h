...
inline uint unpack_16B_address(const uint addr) { return (addr << FIFO_BASE_ADDRESS_ALIGN_BITS) >> 4; }
...
inline void configure_unpack_AB(
    ...
    uint unpA_ch1_x_stride = (uint)(unpack_dst_format[unpA_operand] & 0x3) == (uint)DataFormat::Float32   ? 4
                             : (uint)(unpack_dst_format[unpA_operand] & 0x3) == (uint)DataFormat::Float16 ? 2
                                                                                                          : 1;
    uint unpB_ch1_x_stride = (uint)(unpack_dst_format[unpB_operand] & 0x3) == (uint)DataFormat::Float32   ? 4
                             : (uint)(unpack_dst_format[unpB_operand] & 0x3) == (uint)DataFormat::Float16 ? 2
                                                                                                          : 1;
   ...
   //  DPRINT << "UNPA: " << unpA_ch1_x_stride << "UNPB: " << unpB_ch1_x_stride <<
    ...
    // Strides
    cfg[UNP0_ADDR_CTRL_XY_REG_1_Xstride_ADDR32] =
        (unpA_ch1_y_stride << UNP0_ADDR_CTRL_XY_REG_0_Ystride_SHAMT) |
    ...
    cfg[UNP1_ADDR_CTRL_XY_REG_1_Xstride_ADDR32] =
        (unpB_ch1_y_stride << UNP1_ADDR_CTRL_XY_REG_0_Ystride_SHAMT) |
...
inline void reconfig_unpacker_data_format(
    const uint src_operand_id, const uint32_t tile_addr, const uint32_t out_df_addr) {
    // volatile uint *cfg = get_cfg_pointer();
