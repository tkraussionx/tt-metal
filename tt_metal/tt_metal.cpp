    ...
    void WriteRuntimeArgsToDevice(Device *device, const Program &program) {
        ...
        for (size_t kernel_id = 0; kernel_id < program.num_kernels(); kernel_id++) {
            ...
            for (const auto &logical_core : kernel->cores_with_runtime_args()) {
                ...
                log_info(tt::LogMetal, "{} - Writing {} unique rtargs to core {} (physical: {}) addr 0x{:x} => args: {}",
            ...
            if (common_rt_args.size() > 0) {
                log_info(tt::LogMetal, "Has {} common runtime args for kernel {}", common_rt_args.size(), kernel->name());
                for (auto &core_range : kernel->logical_coreranges()) {
                    for (auto x = core_range.start.x; x <= core_range.end.x; x++) {
                        for (auto y = core_range.start.y; y <= core_range.end.y; y++) {
                            ...
                            log_info(tt::LogMetal, "{} - Writing {} common rtargs to core {} (physical: {}) addr 0x{:x} => args: {}",
