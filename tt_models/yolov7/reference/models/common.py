...
from tt_models.yolov7.reference.utils.datasets import letterbox
from tt_models.yolov7.reference.utils.general import (
    non_max_suppression,
    make_divisible,
    scale_coords,
    increment_path,
...
from tt_models.yolov7.reference.utils.plots import (
    color_list,
    plot_one_box,
)
from tt_models.yolov7.reference.utils.torch_utils import (
    time_synchronized,
)
...
class RepConv(nn.Module):
    ...
    def fuse_repvgg_block(self):
        if self.deploy:
            return
        logger.info(f"RepConv.fuse_repvgg_block")
        ...
        if isinstance(self.rbr_identity, nn.BatchNorm2d) or isinstance(
            ...
            # logger.info(f"fuse: rbr_identity == BatchNorm2d or SyncBatchNorm")
            ...
            # logger.info(f" identity_conv_1x1.weight = {identity_conv_1x1.weight.shape}")
            ...
            # logger.info(f"fuse: rbr_identity != BatchNorm2d, rbr_identity = {self.rbr_identity}")
            bias_identity_expanded = torch.nn.Parameter(torch.zeros_like(rbr_1x1_bias))
            weight_identity_expanded = torch.nn.Parameter(
                torch.zeros_like(weight_1x1_expanded)
            )

        # logger.info(f"self.rbr_1x1.weight = {self.rbr_1x1.weight.shape}, ")
        # logger.info(f"weight_1x1_expanded = {weight_1x1_expanded.shape}, ")
        # logger.info(f"self.rbr_dense.weight = {self.rbr_dense.weight.shape}, ")
...
class Detections:
    ...
    def print(self):
        self.display(pprint=True)  # logger.info results
        logger.info(
    ...
    def pandas(self):
        # return detections as pandas DataFrames, i.e. logger.info(results.pandas().xyxy[0])
...
class RepConv_OREPA(nn.Module):
    ...
    def switch_to_deploy(self):
        if hasattr(self, "rbr_reparam"):
            return
        logger.info(f"RepConv_OREPA.switch_to_deploy")
        kernel, bias = self.get_equivalent_kernel_bias()
        self.rbr_reparam = nn.Conv2d(
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # logger.info(attn.dtype, v.dtype)
        try:
...
class SwinTransformerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            ...
            Padding = True
            # logger.info(f'img_size {min(H_, W_)} is less than (or not divided by) window_size {self.window_size}, Padding.')
            pad_r = (self.window_size - W_ % self.window_size) % self.window_size
            pad_b = (self.window_size - H_ % self.window_size) % self.window_size
            x = F.pad(x, (0, pad_r, 0, pad_b))

        # logger.info('2', x.shape)
        B, C, H, W = x.shape
        L = H * W
