...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        img_path = "webcam.jpg"
        logger.info(f"webcam {self.count}: ", end="")
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources="streams.txt", img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            logger.info(f" success ({w}x{h} at {self.fps:.2f} FPS).")
            thread.start()
        logger.info("")  # newline
...
def pastein(image, labels, sample_labels, sample_images, sample_masks):
    ...
    for s in scales:
        ...
        if (
            ...
            # logger.info(len(sample_labels))
            # logger.info(sel_ind)
            # logger.info((xmax-xmin, ymax-ymin))
            # logger.info(image[ymin:ymax, xmin:xmax].shape)
            # logger.info([[sample_labels[sel_ind], *box]])
            ...
            if (r_w > 10) and (r_h > 10):
                ...
                if m_ind.astype(np.int32).sum() > 60:
                    ...
                    # logger.info(sample_labels[sel_ind])
                    # logger.info(sample_images[sel_ind].shape)
                    # logger.info(temp_crop.shape)
                    box = np.array(
