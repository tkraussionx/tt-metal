...
from tt_models.yolov7.reference.utils.google_utils import gsutil_getsize
from tt_models.yolov7.reference.utils.metrics import fitness
...
def check_git_status():
    ...
    logger.info(colorstr("github: "), end="")
    try:
...
def check_requirements(requirements="requirements.txt", exclude=()):
    ...
    if isinstance(requirements, (str, Path)):  # requirements.txt file
        ...
        if not file.exists():
            logger.info(f"{prefix} {file.resolve()} not found, check failed.")
            return
        requirements = [
...
def print_mutation(hyp, results, yaml_file="hyp_evolved.yaml", bucket=""):
    # logger.info mutation results to evolve.txt (for use with train.py --evolve)
