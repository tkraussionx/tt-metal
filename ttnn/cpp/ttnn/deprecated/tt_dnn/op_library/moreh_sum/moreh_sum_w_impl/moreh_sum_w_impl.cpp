...
operation::ProgramWithCallbacks moreh_sum_w_impl(
    const Tensor &a, const Tensor &output, const ttnn::DeviceComputeKernelConfig &compute_kernel_config) {
    ...
    string compute_kernel_name =
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/moreh_sum/moreh_sum_w_impl/kernels/moreh_sum_w.cpp";
    ...
    tt_metal::CircularBufferConfig cb_zero_config =
        tt_metal::CircularBufferConfig(zero_single_tile_size, {{CB::c_in1, zero_cb_data_format}})
            .set_page_size(CB::c_in1, zero_single_tile_size);
    auto cb_zero = tt_metal::CreateCircularBuffer(program, all_cores, cb_zero_config);

    tt_metal::CircularBufferConfig cb_scaler_config =
        tt_metal::CircularBufferConfig(scaler_single_tile_size, {{CB::c_in2, scaler_cb_data_format}})
