...
void kernel_main() {
    ...
    constexpr uint32_t org_batch = get_compile_time_arg_val(10);
    ...
    uint64_t output_write_addr = get_noc_addr(out_addr);
    uint32_t out_element_size = tile_bytes / (32 * 32);
    for (uint32_t group =0; group< num_groups; group ++){
        for(uint32_t dim_chunk =0; dim_chunk<DHt; dim_chunk++){

            uint32_t read_col_offset = dim_chunk*tile_bytes;
            uint32_t read_row_offset = group*32*out_element_size;
            uint32_t write_col_offset = ((cur_batch/org_batch)* DHt*num_groups + DHt*group + dim_chunk)*tile_bytes;
            uint32_t write_row_offset = (cur_batch%org_batch)*32*out_element_size;
            uint32_t num_bytes_written = 32*out_element_size;

            noc_async_write(read_ptr + read_col_offset + read_row_offset,
                            output_write_addr + write_col_offset + write_row_offset,
                            num_bytes_written);
            noc_async_write_barrier();
        }
    }
