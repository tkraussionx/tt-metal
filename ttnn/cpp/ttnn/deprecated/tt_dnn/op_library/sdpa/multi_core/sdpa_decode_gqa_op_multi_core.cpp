...
operation::ProgramWithCallbacks sdpa_decode_gqa_multi_core(
    ...
    */
    ...
    uint32_t org_B = k_shape[0];
    uint32_t q_heads = q_shape[1], k_heads = k_shape[1], num_groups = q_heads / k_heads;
    uint32_t B = k_shape[0] * k_shape[1], PNH = q_shape[1], S = k_shape[2], DH = k_shape[3];
    ...
    std::vector<uint32_t> reader_compile_time_args_common = {
        B, PNHt, St, DHt, Sk_chunk_t, num_active_cores, num_groups, org_B
    };
    ...
    auto reader_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/sdpa/kernels/dataflow/reader_decode_gqa_all.cpp",
        core_grid,
    ...
    auto writer_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/sdpa/kernels/dataflow/writer_decode_gqa_all.cpp",
        core_grid,
