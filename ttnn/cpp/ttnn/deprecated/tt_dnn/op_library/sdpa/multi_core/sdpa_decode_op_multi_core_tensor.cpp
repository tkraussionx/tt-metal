...
operation::ProgramWithCallbacks sdpa_decode_multi_core_tensor(
    ...
    */
    ...
    log_info("pos_data_format: {}", pos_df);
    ...
    std::vector<uint32_t> reader_compile_time_args_common = {
        B, PNHt, St, DHt, Sk_chunk_t, num_active_cores, is_q_sharded, num_cores_per_batch, k_chunk_size, log2_page_size, index_stick_size
    };
    ...
    auto reader_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/sdpa/kernels/dataflow/reader_decode_all_tensor.cpp",
        core_grid,
    ...
    auto writer_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/sdpa/kernels/dataflow/writer_decode_all_tensor.cpp",
        core_grid,
    ...
    for (uint32_t i = 0; i < num_active_cores; ++i) {
        ...
        SetRuntimeArgs(program, compute_kernels_id, core, {do_reduce, core_num, cur_batch});
    }
    ...
    (
        ...
        auto pos_buffer = input_tensors.at(3).buffer();
