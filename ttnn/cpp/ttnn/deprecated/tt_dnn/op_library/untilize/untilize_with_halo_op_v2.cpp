...
operation::ProgramWithCallbacks untilize_with_halo_multi_core_v2(
    ...
    KernelHandle reader_kernel_id0 = CreateKernel(
        program,
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/untilize/kernels/dataflow/halo_gather.cpp",
        all_cores,
        DataMovementConfig{
    ...
    vector<CoreCoord> cores = grid_to_cores_with_noop(bounding_box.end_coord.x, bounding_box.end_coord.y, num_cores_x, num_cores_y);
    std::vector<std::vector<uint32_t>> reader_kernel_rt_args(cores.size(), vector<uint32_t>(1, input_tensor.buffer()->aligned_page_size()));

    tt_metal::SetRuntimeArgs(program, reader_kernel_id1, cores, reader_kernel_rt_args);
    tt_metal::SetRuntimeArgs(program, reader_kernel_id0, cores, reader_kernel_rt_args);
