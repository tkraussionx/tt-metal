...
void launch_in_worker_thread(auto cq_id, auto operation_id, const auto& operation_attributes, const auto& tensor_args, auto &tensor_return_value, auto& device) {
    ...
    const auto enqueue_or_launch_program = [=](auto& program) {
...
typename device_operation_t::tensor_return_value_t launch_on_single_device(
    ...
    // device->work_executor_v2.push_work([=]() mutable {
        tt::log_debug(tt::LogAsync, "Device {}: launching operation {} on device", device->id(), get_type_name<device_operation_t>(operation_attributes));
        launch_in_worker_thread<device_operation_t>(
            cq_id, operation_id, operation_attributes, tensor_args, tensor_return_value, device);
        tt::log_debug(tt::LogAsync, "Device {}: launched operation {} on device", device->id(), get_type_name<device_operation_t>(operation_attributes));
    // });
...
typename device_operation_t::tensor_args_t get_shard_tensor_args(std::size_t index, auto device, const typename device_operation_t::tensor_args_t& tensor_args) {
    auto get_shard = [=](const auto& tensor) {
...
typename device_operation_t::tensor_return_value_t launch_on_multi_device(
    ...
    constexpr bool launch_shards_in_parallel = false;
    ...
    auto output = make_tensor_return_value_from_shards(storage, outputs);
    for (auto shard_index = 0; shard_index < num_shards; shard_index++) {
        auto device = storage.get_buffer_for_device_id(shard_index)->device();
        device->work_executor_v2.synchronize();
    }
    return output;
}
...
typename device_operation_t::tensor_return_value_t run(
    ...
    return std::visit(
        [&](auto&& storage) -> tensor_return_value_t {
            using storage_t = std::remove_cvref_t<decltype(storage)>;
            if constexpr (std::is_same_v<storage_t, tt::tt_metal::DeviceStorage>) {
                auto output = detail::launch_on_single_device<device_operation_t>(cq_id, operation_attributes, tensor_args);
                auto device = tt::stl::reflection::get_first_object_of_type<Tensor>(tensor_args).device();
                device->work_executor_v2.synchronize();
                return output;
