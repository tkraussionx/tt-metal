...
std::vector<ttnn::Tensor> get_device_tensors(const ttnn::Tensor& tensor) {
    if (std::holds_alternative<tt::tt_metal::MultiDeviceHostStorage>(tensor.get_storage())) {
        ...
        for (auto& device_buf_pair : device_storage.buffers)
        {
            ...
            tensors[device->id()] = Tensor{DeviceStorage{buffer}, device_storage.shapes.at(device), tensor.get_dtype(), tensor.get_layout()};
        }
        return tensors;
    }
...
Tensor aggregate_as_tensor(std::vector<Tensor>& tensor_shards)
{
    ...
    if (storage_type == StorageType::OWNED) {
        ...
        for (const auto &shard : tensor_shards) {
            ...
            device_buffers.insert({device, std::get<DeviceStorage>(shard.get_storage()).buffer});
            shapes.insert({device, shard.get_legacy_shape()});
        }
