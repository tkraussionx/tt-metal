    ...
    for (uint32_t i = 0; i < num_transfers; ++i) {
        if constexpr (num_full_chunks > 0) {
            for (uint32_t c = 0; c < num_full_chunks; ++c) {
                uint64_t eth_receiver_l1_curr_noc_addr = eth_buffer_addrs[buffer_index];
                {
                ...
                fetch_chunk(cb_id_in0, num_pages, page_size, eth_buffer_addrs[buffer_index]);
        ...
        if constexpr (rem_num_pages > 0) {
            ...
            noc_semaphore_set(receiver_read_semaphore_addr_ptr, 0);
            fetch_chunk(cb_id_in0, rem_num_pages, page_size, eth_buffer_addrs[buffer_index]);
            noc_semaphore_inc(eth_receiver_l1_semaphore_noc_addr, 1);
