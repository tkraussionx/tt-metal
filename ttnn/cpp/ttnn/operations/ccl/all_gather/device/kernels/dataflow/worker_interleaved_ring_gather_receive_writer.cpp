...
void kernel_main() {
    ...
    #ifdef SHARDED_MEM_LAYOUT
    constexpr tt::tt_metal::TensorMemoryLayout output_tensor_memory_layout = static_cast<tt::tt_metal::TensorMemoryLayout>(get_compile_time_arg_val(6));
    constexpr uint32_t output_tensor_shard_grid_height = get_compile_time_arg_val(7);
    constexpr uint32_t output_tensor_shard_grid_width = get_compile_time_arg_val(8);
    constexpr uint32_t output_tensor_shard_grid_start_y_logical = get_compile_time_arg_val(9);
    constexpr uint32_t output_tensor_shard_grid_start_x_logical = get_compile_time_arg_val(10);
    constexpr uint32_t output_tensor_shard_pages_per_shard_y = get_compile_time_arg_val(11);
    constexpr uint32_t output_tensor_shard_pages_per_shard_x = get_compile_time_arg_val(12);
    constexpr bool output_tensor_shard_grid_transposed = get_compile_time_arg_val(13) != 0;
    #endif
