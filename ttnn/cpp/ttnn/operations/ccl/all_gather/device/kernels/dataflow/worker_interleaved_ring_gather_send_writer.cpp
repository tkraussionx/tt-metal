...
void kernel_main() {
    ...
    const uint32_t output_page_size = get_arg_val<uint32_t>(arg_idx++);
    ...
    const uint32_t writer_send_sem_addr = get_arg_val<uint32_t>(arg_idx++);
    ...
    constexpr uint32_t half_cb_n_pages = get_compile_time_arg_val(1);
    ...
    if (num_full_chunks > 0) {
        for (uint32_t c = 0; c < num_full_chunks; ++c) {
            {
                DeviceZoneScopedN("WAIT-FOR-EDM");
