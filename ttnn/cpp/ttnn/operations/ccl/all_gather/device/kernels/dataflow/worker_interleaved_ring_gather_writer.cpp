...
void kernel_main() {
    ...
    const uint32_t sender_num_transfers = get_arg_val<uint32_t>(arg_idx++);
    const uint32_t input_start_idx = get_arg_val<uint32_t>(arg_idx++);
    const uint32_t output_start_idx = get_arg_val<uint32_t>(arg_idx++);
    const uint32_t sender_output_start_addr_offset = get_arg_val<uint32_t>(arg_idx++);
    ...
    const uint32_t receiver_num_transfers = get_arg_val<uint32_t>(arg_idx++);
    const uint32_t output_start_page_idx = get_arg_val<uint32_t>(arg_idx++);
    const uint32_t receiver_output_start_addr_offset = get_arg_val<uint32_t>(arg_idx++);
    ...
    const uint32_t receiver_input_start_ring_idx = get_arg_val<uint32_t>(arg_idx++);
    ...
    ASSERT(!(sender_enabled && receiver_enabled))
    if (!sender_enabled && !receiver_enabled) {
        return;
    }

    uint32_t output_start_addr_offset;
    if (sender_enabled){
        output_start_addr_offset = sender_output_start_addr_offset;
    }
    else {
        output_start_addr_offset = receiver_output_start_addr_offset;
    }
    ...
    uint32_t sem_addr = get_semaphore(get_compile_time_arg_val(7));
    constexpr uint32_t ring_size = get_compile_time_arg_val(8);

    #ifdef SHARDED_MEM_LAYOUT
    constexpr tt::tt_metal::TensorMemoryLayout output_tensor_memory_layout = static_cast<tt::tt_metal::TensorMemoryLayout>(get_compile_time_arg_val(9));
    constexpr uint32_t output_tensor_shard_grid_height = get_compile_time_arg_val(10);
    constexpr uint32_t output_tensor_shard_grid_width = get_compile_time_arg_val(11);
    constexpr uint32_t output_tensor_shard_grid_start_y_logical = get_compile_time_arg_val(12);
    constexpr uint32_t output_tensor_shard_grid_start_x_logical = get_compile_time_arg_val(13);
    constexpr uint32_t output_tensor_shard_pages_per_shard_y = get_compile_time_arg_val(14);
    constexpr uint32_t output_tensor_shard_pages_per_shard_x = get_compile_time_arg_val(15);
    constexpr bool output_tensor_shard_grid_transposed = get_compile_time_arg_val(16) != 0;
    #endif
    ...
    if (receiver_enabled) {
        ...
        uint32_t input_ring_idx = receiver_input_start_ring_idx;
        ...
        for (uint32_t i = 0; i < receiver_num_transfers; ++i) {
    ...
    ccl::edm::WorkerToEdmSender<ttnn::ccl::EriscDataMoverTerminationMode::MESSAGE_COUNT_REACHED> sender(
        ...
        (num_full_chunks > 0 ? num_pages : rem_num_pages) * page_size,
        writer_send_sem_ptr);
    ...
    if (num_full_chunks > 0) {
        for (uint32_t c = 0; c < num_full_chunks; ++c) {
            ...
            write_and_send_chunk(output_page_idx, col_idx, row_idx, cb_id_in0, d, num_cols, num_rows, col_offset, row_offset, num_pages, page_size, sender);
        }
    }
    ...
    // sender_num_transfers = num_devices - 1
    for (uint32_t i = 1; i < sender_num_transfers; ++i) {
        if (num_full_chunks > 0) {
            for (uint32_t c = 0; c < num_full_chunks; ++c) {
                sender.wait_for_empty_write_slot();
                sender.send_payload_blocking(cb_id_in0, num_pages, page_size);
            }
        }
        if (rem_num_pages > 0) {
            sender.wait_for_empty_write_slot();
