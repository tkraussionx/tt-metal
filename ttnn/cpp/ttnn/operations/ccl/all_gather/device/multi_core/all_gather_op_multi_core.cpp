...
static std::tuple<CoreRangeSet,CoreRangeSet,CoreRangeSet> get_all_worker_cores(AllGatherConfig const& all_gather_config, uint32_t num_links, uint32_t num_full_send_directions) {
    ...
    for (auto const& core : receiver_worker_cores) {
    ...
    return {CoreRangeSet(receiver_worker_cores), CoreRangeSet(sender_worker_cores), CoreRangeSet(all_cores)};
}
...
operation::ProgramWithCallbacks all_gather_multi_core_with_workers(const Tensor& input_tensor, Tensor& output_tensor, const uint32_t dim, const uint32_t num_links, const uint32_t ring_size, const uint32_t ring_index, const std::optional<chip_id_t> receiver_device_id, const std::optional<chip_id_t> sender_device_id, all_gather_op::Topology topology) {
    ...
    // KERNEL CREATION
    auto const& [all_receiver_workers, all_sender_workers, all_workers] = get_all_worker_cores(all_gather_config, num_links, num_full_send_directions);
    ...
    std::string const& reader_kernel_path = "ttnn/cpp/ttnn/operations/ccl/all_gather/device/kernels/dataflow/worker_interleaved_ring_gather_reader.cpp";
    KernelHandle worker_reader_kernel_id = tt::tt_metal::CreateKernel(
        program,
        reader_kernel_path,
        all_workers,
        tt::tt_metal::ReaderDataMovementConfig(worker_reader_ct_args, worker_defines));
    ...
    auto build_worker_writer_ct_args = [&]() {
        ...
        if (is_sharded) {
            emit_sharded_tensor_kernel_ct_args(device, output_tensor, worker_writer_ct_args, output_pages_per_shard_y, output_pages_per_shard_x);
        }
    ...
    std::string const& writer_kernel_path = "ttnn/cpp/ttnn/operations/ccl/all_gather/device/kernels/dataflow/worker_interleaved_ring_gather_writer.cpp";
    KernelHandle worker_writer_kernel_id = tt::tt_metal::CreateKernel(
        program,
        writer_kernel_path,
        all_workers,
        tt::tt_metal::WriterDataMovementConfig(worker_writer_ct_args, worker_defines));
    ...
    for (uint32_t direction = 0; direction < num_full_send_directions; direction++) {
        ...
        for (uint32_t i = 0; i < num_links; ++i) {
            ...
            for (uint32_t b = 0; b < all_gather_config.get_num_workers_per_link(); ++b) {
                ...
                // RT Arg builder
                auto build_worker_reader_rt_args = [&](bool is_sender, bool is_receiver) {

                    std::vector<uint32_t> args = {
                        static_cast<uint32_t>(input_buffer->address()),
                        static_cast<uint32_t>(output_buffer->address()),
                        static_cast<uint32_t>(is_sender),
                        static_cast<uint32_t>(is_receiver),
                    ...
                    if (sender_enabled) {
                        ...
                        sender_args = std::vector<uint32_t>(16, 0);
                    }
                    ...
                    if (receiver_enabled) {
                        ...
                        receiver_args = std::vector<uint32_t>(6, 0);
                    }
                    args.insert(args.end(), sender_args.begin(), sender_args.end());
                    args.insert(args.end(), receiver_args.begin(), receiver_args.end());
                    if (is_sharded) {
                ...
                auto build_worker_writer_rt_args = [&](bool is_sender, bool is_receiver) {
                    ...
                    if (sender_enabled) {
                        ...
                        sender_args = std::vector<uint32_t>(8, 0);
                    }
                    std::vector<uint32_t> receiver_args;
                    if (receiver_enabled) {
                        TT_ASSERT(!is_linear ||
                        ...
                        receiver_args = std::vector<uint32_t>(11, 0);
                    }
                    worker_writer_rt_args.insert(worker_writer_rt_args.end(), sender_args.begin(), sender_args.end());
                    worker_writer_rt_args.insert(worker_writer_rt_args.end(), receiver_args.begin(), receiver_args.end());

                    if (is_sharded) {
                        emit_sharded_tensor_kernel_rt_args(device, output_tensor, worker_writer_rt_args);
                    }
                ...
                // SEND WORKERS
                if (!sender_enabled) {
                    std::vector<uint32_t> const& worker_reader_rt_args = build_worker_reader_rt_args(false, false); // is_sender = false, is_receiver = false
                    std::vector<uint32_t> const& worker_writer_rt_args = build_worker_writer_rt_args(false, false); // is_sender = false, is_receiver = false
                    ...
                    //// Reader
                    std::vector<uint32_t> const& worker_reader_rt_args = build_worker_reader_rt_args(true, false); // is_sender = true, is_receiver = false
                    tt::tt_metal::SetRuntimeArgs(
                        program,
                        worker_reader_kernel_id,
                        sender_worker_cores.at(b),
                    ...
                    //// Writer
                    std::vector<uint32_t> const& worker_writer_rt_args = build_worker_writer_rt_args(true, false); // is_sender = true, is_receiver = false
                    tt::tt_metal::SetRuntimeArgs(
                        program,
                        worker_writer_kernel_id,
                        sender_worker_cores.at(b),
                ...
                if (!receiver_enabled){
                    ...
                    //// Reader
                    std::vector<uint32_t> worker_reader_rt_args = build_worker_reader_rt_args(false, true); // is_sender = false, is_receiver = true
                    tt::tt_metal::SetRuntimeArgs(
                        program,
                        worker_reader_kernel_id,
                        receiver_worker_cores.at(b),
                    ...
                    //// Writer
                    std::vector<uint32_t> worker_writer_rt_args = build_worker_writer_rt_args(false, true); // is_sender = false, is_receiver = true
                    tt::tt_metal::SetRuntimeArgs(
                        program,
                        worker_writer_kernel_id,
                        receiver_worker_cores.at(b),
    ...
    auto override_runtime_arguments_callback =
        ...
        // update receivers and senders
        auto &worker_reader_runtime_args_by_core = GetRuntimeArgs(program, worker_reader_kernel_id);
        auto &worker_writer_runtime_args_by_core = GetRuntimeArgs(program, worker_writer_kernel_id);
        // receivers
        for (auto const& core : all_receiver_worker_cores) {
            //writer
            auto &worker_writer_receiver_runtime_args = worker_writer_runtime_args_by_core[core.x][core.y];
            worker_writer_receiver_runtime_args.at(0) = output.buffer()->address();
        }
        // senders
        for (auto const& core : all_sender_worker_cores) {
            // reader
            auto& worker_reader_sender_runtime_args = worker_reader_runtime_args_by_core[core.x][core.y];
            worker_reader_sender_runtime_args.at(0) = input.buffer()->address();
            worker_reader_sender_runtime_args.at(1) = output.buffer()->address();
            // writer
            auto& worker_writer_sender_runtime_args = worker_writer_runtime_args_by_core[core.x][core.y];
