...
auto build_source_address_generator(std::size_t &arg_idx, address_t tensor_address, std::size_t page_size, uint32_t cb_id_in0) -> typename source_tensor_addrgen<tensor_layout, buffer_type, page_layout>::type {
    ...
    if constexpr (is_row_major_layout) {
        ...
        return addrgen_type{
            .bank_base_address = tensor_address + tensor_address, .page_size = page_size};
        ...
        if constexpr (is_interleaved) {
            return addrgen_type{
                .bank_base_address = tensor_address, .page_size = tensor_address, .data_format = get_dataformat(cb_id_in0)};
        } else if constexpr (is_sharded) {
            return tt::tt_metal::address_generators::build_sharded_addr_gen<tensor_layout>(
                tt::tt_metal::address_generators::HarvestedWormholeWorkerToNocLookup(
                    0,//output_shard_grid_nrows,
...
\
    ...
    DPRINT << "CCL Send to EDM x=" << (uint32_t)edm_args.edm_noc_x << "\n";
    DPRINT << "CCL Send to EDM y=" << (uint32_t)edm_args.edm_noc_y << "\n";
    DPRINT << "CCL Send to EDM edm_args.edm_buffer_base_address=" << (uint32_t)edm_args.edm_buffer_base_address << "\n";
    DPRINT << "CCL Send to EDM edm_args.num_buffers_per_channel=" << (uint32_t)edm_args.num_buffers_per_channel << "\n";
    DPRINT << "CCL Send to EDM edm_args.edm_semaphore_address=" << (uint32_t)edm_args.edm_semaphore_address << "\n";
    DPRINT << "CCL Send to EDM packet_size_in_pages * page_size=" << (uint32_t)packet_size_in_pages * page_size << "\n";
    ...
    for (std::size_t i = 0; i < num_commands; ++i) {
...

        {
            ...
            for (uint32_t p = 0; p < command_tensor.worker_pages_per_slice; p += packet_size_in_pages) {
                ...
                DPRINT << "CCL send: Waiting for slot from EDM\n";
                // Not optimal (doesn't overlap read/write) - but good for functional
                // bringup
                sender.wait_for_empty_write_slot();
                DPRINT << "CCL send: write to EDM: " << n_pages << " pages\n";
