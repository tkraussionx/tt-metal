...
struct CclCommandArg<CclCommandArgCode::SET_FULL_TENSOR_SLICE_SPEC_IN_PAGES>
    ...
    static void pack_to(args_elem_t* args, field_type const& command_tensor) {
        ...
        args[i++] = command_tensor.tensor_slice_shape.y;
        args[i++] = command_tensor.tensor_slice_shape.x;

        args[i++] = command_tensor.tensor_slice_offset.w;
        args[i++] = command_tensor.tensor_slice_offset.z;
        args[i++] = command_tensor.tensor_slice_offset.y;
        args[i++] = command_tensor.tensor_slice_offset.x;

        args[i++] = command_tensor.worker_start_offset_in_slice.w;
        args[i++] = command_tensor.worker_start_offset_in_slice.z;
        args[i++] = command_tensor.worker_start_offset_in_slice.y;
        args[i++] = command_tensor.worker_start_offset_in_slice.x;
...
struct CclCommandHeader {
    ...
    static CclCommandHeader from_uint32(uint32_t const& cmd_header) {
        return *reinterpret_cast<CclCommandHeader const*>(&cmd_header);
    }

    static uint32_t to_uint32(CclCommandHeader const& cmd_header) {
        return *reinterpret_cast<uint32_t const*>(&cmd_header);
    }
