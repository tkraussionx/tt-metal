...
template <typename EDM_CONFIG>
FORCE_INLINE constexpr typename ChannelBuffer<EDM_CONFIG>::STATE get_receiver_state_that_can_advance_without_eth() {
    return ChannelBuffer<EDM_CONFIG>::RECEIVER_SIGNALING_WORKER;
}

template <typename EDM_CONFIG>
FORCE_INLINE bool state_has_no_dependence_on_eth(ChannelBuffer<EDM_CONFIG> const& channel) {
    return channel.get_state() == ChannelBuffer<EDM_CONFIG>::SENDER_WAITING_FOR_ETH || channel.get_state() == get_receiver_state_that_can_advance_without_eth<EDM_CONFIG>();
}
...
FORCE_INLINE bool channel_can_make_progress(ChannelBuffer<EDM_CONFIG> const &edm_channel) {
    switch (edm_channel.get_state()) {
        ...
        case ChannelBuffer<EDM_CONFIG>::STATE::SENDER_WAITING_FOR_WORKER:
            if constexpr (EDM_CONFIG::TERMINATION_MODE == EriscDataMoverTerminationMode::WORKER_INITIATED) {
                return edm_channel.is_local_semaphore_full() || *edm_channel.local_semaphore_address == EriscDataMoverWorkerSignal::TERMINATE_IMMEDIATELY;
            } else {
...
/////////////////////////////////////////////
template <typename EDM_CONFIG>
FORCE_INLINE void sender_eth_send_data_sequence(ChannelBuffer<EDM_CONFIG> &sender_buffer_channel) {
...
template <typename EDM_CONFIG>
FORCE_INLINE void sender_noc_receive_payload_ack_check_sequence(
 ...
 */
template <typename EDM_CONFIG>
FORCE_INLINE void receiver_eth_notify_workers_payload_available_sequence(ChannelBuffer<EDM_CONFIG> &buffer_channel) {
 ...
 */
template <typename EDM_CONFIG>
FORCE_INLINE void receiver_eth_accept_payload_sequence(
 ...
 */
template <typename EDM_CONFIG>
FORCE_INLINE void receiver_noc_read_worker_completion_check_sequence(
    ChannelBuffer<EDM_CONFIG> &buffer_channel,
    uint32_t &num_receivers_complete) {
