...
static std::size_t decide_number_of_edm_channels(
   ttnn::ccl::CCLOpConfig const& ccl_op_config, std::size_t max_num_workers, bool enable_bidirectional) {
    return std::min<std::size_t>(max_num_workers, enable_bidirectional ? 8 : 4);
        // ccl_op_config.is_input_sharded() ? std::min<uint32_t>(ccl_op_config.get_shard_grid_size())
...
// TODO: need to make this take the packet size as arg instead of `edm_channel_buffer_size` because
// `edm_channel_buffer_size` can be shrunk
