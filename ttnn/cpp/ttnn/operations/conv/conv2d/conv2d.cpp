...
static TensorMemoryLayout select_shard_spec(
    ...
    if (!is_conv1d && !is_width_or_block_sharding_valid) {
        log_info(LogOp, "2d conv which can only be supported by Shard layout: HEIGHT_SHARDED");
    ...
    if (conv_config.reshard_if_not_optimal || needs_shard_or_reshard) {
       auto block_shard_orientation = conv_config.transpose_shards ? ShardOrientation::COL_MAJOR : ShardOrientation::ROW_MAJOR;
        const ParallelConfig& optimal_parallel_config = determine_parallel_config(
            select_shard_spec(conv_config, batch_size, height, width, in_channels, kernel_size, stride),
            batch_size,
            in_channels,
            height,
            width,
