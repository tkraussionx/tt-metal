...
Tensor optimized_conv_new(const Tensor& a, const Tensor &b, std::optional<const Tensor> bias,
    ...
    operation::launch_op(
        [sliding_window_config, output_channels, groups, untilize_out, fuse_relu, math_fidelity, parallelization_config, block_config, out_memory_config, dtype, input_tensor_shape, use_shallow_conv_variant, compute_kernel_config, enable_act_double_buffer, enable_split_reader, enable_subblock_padding, use_non_tile_height]
            (const std::vector<Tensor>& input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors, const std::vector<std::optional<Tensor>>& optional_output_tensors) mutable -> std::vector<Tensor> {
                ...
                return operation::run_without_autoformat(
                    OptimizedConvNew(sliding_window_config, output_channels, groups, untilize_out, bias.has_value(), fuse_relu, math_fidelity, parallelization_config, block_config, out_memory_config, dtype, input_tensor_shape, use_shallow_conv_variant, kernel_config_val, enable_act_double_buffer, enable_split_reader, enable_subblock_padding, use_non_tile_height
                    ),
                    input_tensors,
...
void OptimizedConvNew::validate(const std::vector<Tensor>& input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    ...
    if (this->out_memory_config.is_sharded()) {
        ...
        if(this->out_memory_config.memory_layout == TensorMemoryLayout::HEIGHT_SHARDED) {
        ...
        } else if (this->out_memory_config.memory_layout == TensorMemoryLayout::BLOCK_SHARDED) {
            ...
            if (this->out_memory_config.shard_spec.value().orientation == ShardOrientation::COL_MAJOR) {
...
std::vector<tt::tt_metal::LegacyShape> OptimizedConvNew::compute_output_shapes(const std::vector<Tensor>& input_tensors) const {
    ...
    auto padded_shape_w = this->use_non_tile_height ?  parallelization_config.num_cores_nhw * parallelization_config.per_core_out_matrix_height : parallelization_config.num_cores_nhw * parallelization_config.per_core_out_matrix_height_ntiles * TILE_HEIGHT;
...
std::vector<Tensor> OptimizedConvNew::create_output_tensors(const std::vector<Tensor>& input_tensors) const {
    ...
    if (this->out_memory_config.is_sharded()) {
        ...
        if (this->out_memory_config.memory_layout == TensorMemoryLayout::HEIGHT_SHARDED) {
            ...
            auto mem_config = this->out_memory_config;
            mem_config.shard_spec = shard_spec;
            return {create_device_tensor(output_shape, this->dtype, output_layout, input_tensor.device(), mem_config)};
        } else if(this->out_memory_config.memory_layout == TensorMemoryLayout::WIDTH_SHARDED) {
            uint32_t total_height_tiles = tt::tt_metal::compute_volume(output_shape) / output_shape[-1] / TILE_HEIGHT;
            ...
            auto shard_grid = this->out_memory_config.shard_spec.value().grid;
            auto shard_spec = ShardSpec{shard_grid, shard_shape, this->out_memory_config.shard_spec.value().orientation};
            auto mem_config = this->out_memory_config;
            mem_config.shard_spec = shard_spec;
            return{create_device_tensor(output_shape, this->dtype, output_layout, input_tensor.device(), mem_config)};
    ...
    return operation::generic_create_output_tensors(*this, input_tensors, this->dtype, output_layout, this->out_memory_config);
}
