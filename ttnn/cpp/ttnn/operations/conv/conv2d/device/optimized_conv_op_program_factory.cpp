...
void OptimizedConvNew::validate(const std::vector<Tensor>& input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    ...
    if (this->memory_config.is_sharded()) {
        ...
        if (!this->untilize_out) {
            TT_FATAL(parallelization_config.per_core_out_matrix_height % TILE_HEIGHT == 0, "Error");
            TT_FATAL(parallelization_config.per_core_out_matrix_width % TILE_WIDTH == 0, "Error");
        }
        auto [act_matrix_shape, act_matrix_shape_unpadded] = optimized_conv_op_utils::compute_opt_conv_activation_as_mm_shape(input_tensor_a.get_legacy_shape(), sliding_window_config, parallelization_config.per_core_out_matrix_height);
