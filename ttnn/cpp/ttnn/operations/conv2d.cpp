...
ParallelConfig determine_parallel_config(
    ...
    std::vector<uint32_t> device_grid_size = {(uint32_t) compute_with_storage_grid_size.x,(uint32_t) compute_with_storage_grid_size.y};
    uint32_t max_num_cores = device_grid_size[0] * device_grid_size[1];

    auto calculate_num_cores_nhw = [&]() {
        uint32_t num_cores_nhw = height_sharding ?
...
uint32_t get_num_cores_nhw_from_parallel_config(const ParallelConfig& pconfig) {
    ...
    TT_ASSERT(pconfig.shard_scheme == TensorMemoryLayout::HEIGHT_SHARDED || pconfig.shard_scheme == TensorMemoryLayout::BLOCK_SHARDED);
...
tt::tt_metal::OptimizedConvBlockConfig determine_per_core_conv_block_config(const ParallelConfig& parallel_config, const tt::tt_metal::OptimizedConvParallelizationConfig& conv_op_parallel_config, uint32_t padded_in_channels, uint32_t act_block_h_override, uint32_t window_w, bool fp32_accum, bool use_shallow_conv_variant) {
    ...
    uint32_t act_c_num_blocks = parallel_config.shard_scheme == TensorMemoryLayout::HEIGHT_SHARDED ? 1 : parallel_config.shard_orientation == ShardOrientation::COL_MAJOR ? grid_size.y : grid_size.x;
...
std::tuple<ttnn::Tensor, ParallelConfig, bool>  shard_or_reshard_tensor_if_required(Device& device, const ttnn::Tensor& input_tensor_, const ConvConfig& conv_config, uint32_t batch_size, uint32_t height, uint32_t width, uint32_t in_channels, uint32_t out_channels) {
    ...
    if (conv_config.reshard_if_not_optimal || needs_shard_or_reshard) {
        auto block_shard_orientation = conv_config.transpose_shards ?  ShardOrientation::COL_MAJOR :  ShardOrientation::ROW_MAJOR;
        const ParallelConfig & optimal_parallel_config = determine_parallel_config(conv_config.height_sharding, batch_size, in_channels, height, width, out_channels, device, block_shard_orientation);
        ...
        if (conv_config.override_sharding_config) {
            if (conv_config.core_grid.ranges().empty()) {
                ...
                auto shard_scheme = conv_config.height_sharding ? TensorMemoryLayout::HEIGHT_SHARDED : TensorMemoryLayout::BLOCK_SHARDED;
                auto shard_orientation = conv_config.height_sharding ? ShardOrientation::ROW_MAJOR : block_shard_orientation;
                parallel_config = {.grid = conv_config.core_grid, .shard_scheme = shard_scheme, .shard_orientation = shard_orientation};
            }
        } else {
            parallel_config = optimal_parallel_config;
        }
    ...
    if (needs_shard_or_reshard) {
        ...
        if(input_tensor.get_shape()[0] != 1 or input_tensor.get_shape()[1] != 1) {
            ...
            input_tensor = ttnn::operations::core::reshape(input_tensor, Shape({1, 1, input_tensor.get_shape()[0]*input_tensor.get_shape()[1]*input_tensor.get_shape()[2], input_tensor.get_shape()[3]}));
        }
...
std::pair<ttnn::Tensor, std::optional<ttnn::Tensor>> prepare_conv_weights_biases_and_move_to_device(const ttnn::Tensor& weight_tensor, std::optional<const ttnn::Tensor>& bias_tensor, uint32_t input_channels_alignment, DataType weights_bias_dtype, uint32_t weight_block_h_ntiles, uint32_t weight_block_w_ntiles, const ParallelConfig& parallel_config, Device& device) {
    ...
    tt::tt_metal::Shape weights_channels_padded_shape = tt::tt_metal::Shape(std::array<uint32_t, 4>({
        round_up(out_channels, 32),
        round_up(in_channels, input_channels_alignment),
        window_h,
        window_w
    }));
    ...
    weight_tensor_ = tt::tt_metal::pad_on_host(
        weight_tensor,
    ...
    if(bias_tensor.has_value()) {
        ...
        tt::tt_metal::Shape bias_channels_padded_shape = tt::tt_metal::Shape(std::array<uint32_t, 4>({
            1,
            1,
            32,
            round_up(out_channels, weight_block_w_ntiles * 32)
        }));
        bias_tensor_ = tt::tt_metal::pad_on_host(
            bias_tensor_,
            bias_channels_padded_shape,
            {0, 0, 0, 0},
            0
        );
...
std::tuple<ttnn::Tensor, uint32_t, uint32_t, ttnn::Tensor, std::optional<ttnn::Tensor>> conv2d(
    ...
    auto opt_conv_op_block_config = determine_per_core_conv_block_config(parallel_config, opt_conv_op_parallel_config, round_up(in_channels, conv_config.input_channels_alignment), conv_config.act_block_h_override, kernel_size[1], conv_config.fp32_dest_acc_enabled, conv_config.input_channels_alignment == 16);
    ...
    if (device.arch() == ARCH::WORMHOLE_B0) {
        compute_kernel_config = WormholeComputeKernelConfig({.math_fidelity = conv_config.math_fidelity, .math_approx_mode = conv_config.math_approx_mode_enabled, .fp32_dest_acc_en = conv_config.fp32_dest_acc_enabled, .packer_l1_acc = conv_config.packer_l1_accum_enabled});
    } else {
