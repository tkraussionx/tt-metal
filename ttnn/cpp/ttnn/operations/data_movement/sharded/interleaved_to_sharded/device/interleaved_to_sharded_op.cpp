...
void InterleavedToShardedDeviceOperation::validate(const std::vector<Tensor>& input_tensors) const {
    ...
    if (input_tensor.get_layout() == Layout::ROW_MAJOR) {
        auto page_size = (*this->output_mem_config.shard_spec).shape[1] * input_tensor.element_size();
        TT_FATAL(page_size % L1_ALIGNMENT == 0, "Shard page size {} is not divisible by {} as required for alignment on L1.", page_size, L1_ALIGNMENT);
    }
