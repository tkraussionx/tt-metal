...
std::vector<ttnn::SimpleShape> InterleavedToShardedPartialDeviceOperation::compute_output_shapes(const std::vector<Tensor> &input_tensors) const {
    const auto& input_tensor = input_tensors.at(0);
    ttnn::SimpleShape shape = input_tensor.get_logical_shape();
