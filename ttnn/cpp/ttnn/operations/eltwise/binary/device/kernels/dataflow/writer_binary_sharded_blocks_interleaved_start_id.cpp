...
void kernel_main() {
    ...
    const uint32_t unpadded_block_width_tiles = get_compile_time_arg_val(5);
    const uint32_t output_width_tiles = get_compile_time_arg_val(6); // input width in tiles - block width in tiles
    const uint32_t block_num_tiles = get_compile_time_arg_val(7); // block_height_tiles * block_width_tiles
    const uint32_t start_id_offset = get_compile_time_arg_val(8);
    const uint32_t start_id_base = get_compile_time_arg_val(9);
