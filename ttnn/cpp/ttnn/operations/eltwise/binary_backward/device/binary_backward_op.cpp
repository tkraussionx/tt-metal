...
std::vector<std::optional<Tensor>> ExecuteBackwardLt::invoke(
    ...
    input_grad = ttnn::zeros_like(grad, grad.get_dtype(), grad.get_layout(), std::nullopt, output_mem_config);
    result.push_back(input_grad.value());
    input_grad = ttnn::zeros_like(input, input.get_dtype(), input.get_layout(), std::nullopt, output_mem_config);
    result.push_back(input_grad.value());
    return result;
}
