...
operation::ProgramWithCallbacks eltwise_unary_sharded(const Tensor &input, Tensor &output, const std::vector<UnaryWithParam> op_chain, bool fp32_dest_acc_en){
    ...
    if (input.get_dtype() == DataType::BFLOAT8_B) {
        ...
        TT_FATAL((shard_spec.shape[1] * tt::datum_size(act_df)) % L1_ALIGNMENT == 0, "Shard width should be multiple of L1_ADRESS_ALIGNMENT");
        ...
        size_t shard_size_in_bytes = shard_height * shard_width * tt::datum_size(act_df);
    ...
    tt::log_debug(tt::LogOp, "input_cb: {}, npages: {}, pagesize: {}", in_cb_id, in_cb_npages, in_cb_pagesize);
    tt::log_debug(tt::LogOp, "input_tile_size: {}", input_tile_size);
