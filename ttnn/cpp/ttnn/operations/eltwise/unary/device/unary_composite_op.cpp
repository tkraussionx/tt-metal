...
Tensor _acosh(const Tensor& input_a, const std::optional<MemoryConfig>& output_mem_config) {
   ...
   Tensor t_result(input_a);
   {
       ...
       Tensor nan_res = ttnn::multiply(
           ttnn::le(input_a, t_one, std::nullopt, output_mem_config), ttnn::sfpu_positive_nan(input_a.get_dtype()), std::nullopt, output_mem_config);
...
Tensor _logit(const Tensor& input_a, float eps, const std::optional<MemoryConfig>& output_mem_config) {
    ...
    Tensor t_inf = ttnn::multiply(ttnn::sign(input_a, output_mem_config), ttnn::sfpu_positive_inf(input_a.get_dtype()), std::nullopt, output_mem_config);
    Tensor logit_result = ttnn::where(
        ttnn::eq(logit_input, 1.0, std::nullopt, output_mem_config),
        t_inf,
        ttnn::where(ttnn::ltz(log_input, output_mem_config), ttnn::sfpu_positive_nan(input_a.get_dtype()), ttnn::log(log_input, output_mem_config))
        );
    return logit_result;
}
