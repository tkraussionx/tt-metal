...
std::vector<Tensor> _hardtanh_bw(
    ...
    Tensor grad_result = ttnn::where(
        ...
        0.0,
        ttnn::where(ttnn::ge(input, ttnn::operations::creation::full_like(input, max), std::nullopt, output_mem_config), 0.0, grad),
        output_mem_config);
...
std::vector<Tensor> _multigammaln_bw(const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config) {
    std::vector<Tensor> grad_tensor;
    auto output_memory_config = output_mem_config.value_or(input.memory_config()); //TODO: Remove after removing digamma from tt_eager
