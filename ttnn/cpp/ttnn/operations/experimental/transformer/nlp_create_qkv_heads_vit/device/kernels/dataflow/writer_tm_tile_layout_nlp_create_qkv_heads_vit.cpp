...
void kernel_main() {
    ...
    uint32_t k_out_tensor_tile_id  =  q_out_tensor_tile_id; // + 2;   // = get_arg_val<uint32_t>(6);
    uint32_t v_out_tensor_tile_id  =  q_out_tensor_tile_id; // + 4;   // = get_arg_val<uint32_t>(7);
    ...
    uint32_t k_out_tensor_current_tile_id; // need this to update q_out_tensor_tile_id
    uint32_t v_out_tensor_current_tile_id; // need this to update q_out_tensor_tile_id
    ...
    for (uint32_t block = 0; block < num_blocks; block++) {
        ...
        // k + create k head --> outputs: [B, num_heads, S, head_dim]
        ...
        // v + create v head --> outputs: [B, num_heads, S, head_dim]
        out_tensor_current_tile_id_along_c = v_out_tensor_tile_id*2;
        for (uint32_t c_dim = 0; c_dim < q_out_c; c_dim++) {
