...
operation::ProgramWithCallbacks multi_core_nlp_create_qkv_heads_vit(const Tensor &a, std::vector<Tensor>& output, CoreCoord compute_with_storage_grid_size) {
    ...
    uint32_t q_num_tiles_per_tensor = 24;
    uint32_t kv_num_tiles_per_tensor = 24;
    ...
    auto writer_kernel_id = tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/nlp_create_qkv_heads_vit/device/kernels/dataflow/writer_tm_tile_layout_nlp_create_qkv_heads_vit.cpp",
        all_cores,
    ...
    for (uint32_t i = 0, num_blocks_written = 0; i < num_cores; i++){
        ...
        std::cout << "TILE ID -- " << q_out_tensor_tile_id << " -- " <<  num_blocks_written << " -- " << kv_num_tiles_per_tensor << std::endl;
