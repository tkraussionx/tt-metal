...
MorehBinaryDeviceOperation::Add::cached_program_t
MorehBinaryDeviceOperation::Add::create(
    ...
    for (uint32_t x = 0; x < grid.x; ++x) {
      for (uint32_t y = 0; y < grid.y; ++y) {
        ...
        const std::vector<uint32_t> compute_runtime_args = {
            static_cast<uint32_t>(input0_cb), static_cast<uint32_t>(input1_cb),
            static_cast<uint32_t>(output_cb), num_tiles_per_core};
...
void MorehBinaryDeviceOperation::Add::override_runtime_arguments(
