// SPDX-FileCopyrightText: © 2023 Tenstorrent Inc.
//
// SPDX-License-Identifier: Apache-2.0

#pragma once

#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

#include "ttnn/cpp/pybind11/decorators.hpp"
#include "ttnn/operations/moreh_eltwise/moreh_relu/moreh_relu.hpp"
#include "ttnn/types.hpp"

namespace py = pybind11;

namespace ttnn::operations::moreh_eltwise {

void bind_moreh_relu(py::module &module) {
  bind_registered_operation(
      module, ttnn::moreh_relu,
      R"doc(moreh_relu(input_tensor: ttnn.Tensor, output_tensor: ttnn.Tensor, which_relu : uint8_t, bound : float) -> ttnn.Tensor)doc",

      // Add pybind overloads for the C++ APIs that should be exposed to python
      // There should be no logic here, just a call to `self` with the correct
      // arguments
      ttnn::pybind_overload_t{
          [](const decltype(ttnn::moreh_relu) &self,
             const ttnn::Tensor &input_tensor,
             const ttnn::Tensor &output_tensor, const uint8_t which_relu,
             const float bound, const uint8_t &queue_id) -> ttnn::Tensor {
            return self(queue_id, input_tensor, output_tensor, which_relu,
                        bound);
          },
          py::arg("input_tensor"), py::arg("output_tensor"), py::kw_only(),
          py::arg("which_relu"), py::arg("bound"), py::arg("queue_id") = 0});
}

} // namespace ttnn::operations::moreh_eltwise
