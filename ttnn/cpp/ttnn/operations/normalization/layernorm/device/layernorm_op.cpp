...
void LayerNorm::validate(const std::vector<Tensor> &input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    if (this->distributed_type == LayerNormDistributedType::NOT_DISTRIBUTED)
    {
    ...
    else if (this->distributed_type == LayerNormDistributedType::PRE_ALL_GATHER) // only sharded layernorm pre all gather supported for now
    {
        ...
        TT_FATAL(a.is_sharded(), "Distributed pre all gather op is currently only supported for sharded inputs.");
        ...
        std::visit(
            [&](const auto& program_config) {
                ...
                if constexpr (
                    ...
                    TT_FATAL(not program_config.inplace); // pre all gather distributed op can't be inplace
        ...
        TT_FATAL(false, "Distributed layernorm config is currently only supported for pre all gather op.");
    }
}
    ...
    return std::visit(
        [&](const auto& program_config) -> std::vector<Tensor> {
            ...
            if constexpr (
                ...
                if (program_config.inplace) {
                    return {input_tensor};
                } else if (this->distributed_type == LayerNormDistributedType::PRE_ALL_GATHER)
                {
        ...
        [&](const auto& program_config) -> operation::ProgramWithCallbacks {
            ...
            if constexpr (
                ...
                return layernorm_multi_core_sharded(
                                            a, b, gamma, beta, output_tensor, this->norm_type, this->distributed_type, this->eps,
