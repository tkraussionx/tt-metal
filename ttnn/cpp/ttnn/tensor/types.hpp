...
using ShardGrid = std::pair<std::size_t, std::size_t>; // (y,x)
struct ShardTensor2D {
    ShardGrid shard_grid; // logic 2D grid that defines the mapping of shards to devices
    ShardTensor2D(ShardGrid grid) : shard_grid(std::move(grid)) {}
};
