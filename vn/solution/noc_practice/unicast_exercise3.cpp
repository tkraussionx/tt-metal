...
int main() {
    ...
    tt::tt_metal::KernelHandle reader_kernel = tt::tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/noc_practice/kernels/reader_unicast_exercise3.cpp", /* reader kernel path. */
        core,
    ...
    tt::tt_metal::KernelHandle writer_kernel = tt::tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/noc_practice/kernels/writer_unicast_exercise3.cpp", /* writer kernel path. */
        core,
        tt::tt_metal::DataMovementConfig{
    ...
    const std::vector<uint32_t> reader_runtime_args = {input_dram_buffer->address(), num_tiles};
    tt::tt_metal::SetRuntimeArgs(program, reader_kernel, core, reader_runtime_args);

    const std::vector<uint32_t> writer_runtime_args = {output_dram_buffer->address(), num_tiles};
